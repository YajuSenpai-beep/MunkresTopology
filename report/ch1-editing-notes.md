...
## 十二、索引命令修复（2026.06.01 补充）

第一章编辑中发现的 `\idx{}` 和 `\idxmath{}` 显示问题及修复方案。
...
| B：排序文字混入 | 59 | `\idx{Section: ...}` → `\idx[section]{Section: ...}` |
| C：idxmath 排序 | 9 | display 参数去括号文字 |
| D：重复符号 | 11 | idxmath 后删重 |
| E：section 标题 | 7 | idx 移出 `\section*{}` |
| F：其他 | 6 | 拼写、嵌套混乱、重复 ℝ |
| **合计** | **138** | |

### 核心规则

1. **`\idx[display]{key}`** 双参形式用于 display≠key 的场景
2. **`\section*{...}` 内不得含 `\idx`**：`\@star@section` 的 xstring 解析会冲突
3. **`\begin{env}[...]` 内 `\idx[...]` 需包裹 `{...}`**：防止 `]` 被外层吞掉
4. **`\idxmath{sort}{display}` 的 display 必须是纯数学符号**：不含 `(see ...)` 等文字

详细报告见 [index/idx-system-report.md](index/idx-system-report.md)。
