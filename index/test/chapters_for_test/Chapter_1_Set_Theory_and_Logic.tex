...
The relations \(\subset\) and \(\varsubsetneq\) are called inclusion and proper inclusion, respectively. If \(A \subset  B\) , we also write \(B \supset  A\) , which is read " \(B\) contains \(A\) ."
...
\section*{For at least one \(x \in  A\) , statement \(P\) does not hold.}
...
Definition. A rule of assignment is a subset \(r\) of the cartesian product \(\mathbf{C} \times  \mathbf{D}\) of two sets, having the property that each element of \(C\) appears as the first coordinate of at most one ordered pair belonging to \(r\) .
...
EXAMPLE 9. The interval(-1,1)of real numbers has the same order type as the set \(\mathbb{R}\) of real numbers itself, for the function \(f : \left( {-1,1}\right)  \rightarrow  \mathbb{R}\) given by
...
EXAMPLE II. Consider the dictionary order on the plane \(\mathbb{R} \times  \mathbb{R}\) . In this order, the point \(p\) is less than every point lying above it on the vertical line through \(p\) , and \(p\) is less than every point to the right of this vertical line.

EXAMPLE 12 Consider the set \(\lbrack 0,1)\) of real numbers and the set \({\mathbb{Z}}_{ + }\) of positive integers, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
...
(2) (Principle of induction). If \(A\) is an inductive set of positive integers, then \(A =\)  \({Z}_{ + }\) .
...
Theorem 4.2 (Strong induction principle). Let \(A\) be a set of positive integers. Suppose that for each posiave integer \(n\) , the statement \({S}_{n} \subset  A\) implies the statement \(n \in  A\) . Then \(A = {\mathbb{Z}}_{ + }\) .
...
EXAMPLE 2. How does the cartesian product \(A \times  B \times  C\) differ from the cartesian products \(A \times  \left( {B \times  C}\right)\) and \(\left( {A \times  B}\right)  \times  {C}^{7}\) Very little. There are obvious bijective correspondences between these sets, indicated as follows
...
we also call such a function a sequence, or an infinite sequence, of elements of \(X\) . If \(\mathbf{x}\) is an \(\omega\) -tuple, we often denote the value of \(\mathbf{x}\) at \(i\) by \({x}_{i}\) rather than \(\mathbf{x}\left( i\right)\) , and call it the i th coordinate of \(x\) . We denote \(x\) itself by the symbol
...
Proof. (1) \(\Rightarrow\) (2). Suppose that \(B\) is countable. If \(B\) is countably infinite, there is a bijection \(f : {\mathbb{Z}}_{ + } \rightarrow  B\) by definition, and we are through. If \(B\) is finite, there is a bijection \(h : \{ 1,\ldots ,n\}  \rightarrow  B\) for some \(n \geq  1\) . (Recall that \(B \neq  \varnothing\) .) We can extend \(h\) to a surjection \(f : {\mathbb{Z}}_{ + } \rightarrow  B\) by defining
...
(f) The set \(F\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) that are "eventually zero." [We say that \(f\) is eventually zero if there is a positive integer \(N\) such that \(f\left( n\right)  = 0\) for all \(n \geq  N\) ]
...
that satisfies \(\left( *\right)\) for all \(i\) in its domain. Proof The point of this lemma is that it is a statement that depends on \(n\) ; therefore, it is capable of being proved by induction. Let \(A\) be the set of all \(n\) for which the lemma holds. We show that \(A\) is inductive. It then follows that \(A = {\mathbb{Z}}_{ + }\) .
...
EXAMPLE 2. Given \(a \in  \mathbb{R}\) , we "defined" \({a}^{n}\) , in the exercises of \(\$ 4\) , by the recursion formula
...
Use Theorem 8.4 to define this product rigorously. We sometimes denote the product \(\mathop{\prod }\limits_{{k = 1}}^{n}{b}_{k}\) by the symbol \({b}_{1}{b}_{2}\cdots {b}_{n}\) . 3. Obtain the definitions of \({a}^{n}\) and \(n\) ! for \(n \in  {\mathbb{Z}}_{ + }\) as special cases of Exercise 2. 4. The Fibonacci numbers of number theory are defined recursively by the formula
...
EXAMPLE I. Consider the set \(\{ 1,2\}  \times  {\mathbb{Z}}_{ + }\) in the dictionary ordering. Schernatically, it can be represented as one infinite sequence followed by another infinite sequence:
...
\section*{*\$11 The Maximum Principle \({}^{ \dagger  }\)}
...
Theorem. The choice axiom is equivalent to the well-ordering theorem. Proof. Let \(X\) be a set, let \(c\) be a fixed choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(\left( {T, < }\right)\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) ,
