"""Batch fix OCR errors for Ch3-14, then convert exercises."""
import glob, re

files = sorted(glob.glob("chapters/Chapter_0[3-9]*.tex")) + \
        sorted(glob.glob("chapters/Chapter_1[0-4]*.tex"))
files = [f for f in files if "Chapter_1_Set" not in f]
...
for fp in files:
    ...
    # cdot-for-colon in function definitions: h \cdot B -> h : B
    # Be conservative: only fix known patterns
    cdot_fixes = [
        (r"h \cdot  {\mathbb{Z}}_{ + }", r"h : {\mathbb{Z}}_{ + }"),
        (r"g \cdot  {\mathbb{Z}}_{ + }", r"g : {\mathbb{Z}}_{ + }"),
        (r"k \cdot  {S}_{\alpha }", r"k : {S}_{\alpha }"),
        (r"h \cdot  B \rightarrow", r"h : B \rightarrow"),
    ]
    for old, new in cdot_fixes:
        if old in c:
            c = c.replace(old, new)
            fixes += 1
    ...
    # Isolated rbrack fixes (not part of \right\rbrack)
    # These need manual inspection, skip automated fix
    # But we can fix obvious ones like (a,b\rbrack -> (a, b]
    # Skip for now - handle manually per chapter

    # missing periods - common patterns
    period_fixes = [
        ("is empty On", "is empty. On"),
        ("is empty Not", "is empty. Not"),
        ("disjoint It", "disjoint. It"),
        ("surjective For", "surjective. For"),
        ("countable Since", "countable. Since"),
        ("countable Its", "countable. Its"),
        ("set Given", "set. Given"),
        ("uncountable Then", "uncountable. Then"),
...
for fp in files:
    ...
    if "\\begin{enumerate}[itemsep=0.4em" in c and "label=\\arabic*" in c:
    ...
    if not blocks:
        print(f"{name}: no exercise blocks found")
        continue
    ...
    with open(fp, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"{name}: {len(blocks)} blocks, {total_ex} exercises converted")
