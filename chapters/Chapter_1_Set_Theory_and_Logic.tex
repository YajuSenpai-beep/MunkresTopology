...
The \index{equality symbol} \(=\) is used throughout this book to mean \index{Logical equivalence} logical identity. Thus, when we write \(a = b\) , we mean that `` \(a\) `` and `` \(b\) `` are symbols for the \index{Cardinality: comparability!same}\index{Cardinality: comparability!same}\index{Cardinality: comparability!same}\index{Cardinality: comparability!same}same object. This is what one means in arithmetic, for example, when one writes \(\displaystyle\frac{2}{4} = \frac{1}{2}\) . Similarly, the equation \(A = B\) states that `` \(A\) `` and `` \(B\) `` are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
A \subset  B\text{ . }
\]
Nothing in this definition requires \(A\) to be different from \(B\) ; in fact, if \(A = B\) , it is true that both \(A \subset  B\) and \(B \subset  A\) . If \(A \subset  B\) and \(A\) is different from \(B\) , we say that \(A\) is a \index{proper subset} \index{Restriction: of a covering map!of a relation}\index{Restriction: of a covering map!of a relation}of \(B\) , and we write
\[
...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the \index{Intersection}\index{Intersection}\index{intersection@Intersection}\index{intersection@Intersection}intersection\index{Section: of the positive integers} of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
Now some students are bothered by the notion of an ``\index{Empty set}\index{Empty set}\index{empty set@Empty set}\index{empty set@Empty set}empty set.'' ``How,'' they say, ``can you have a set with nothing in it?'' The problem is similar to that which arose many years ago when the number 0 was first introduced.
...
Again, mathematics cannot tolerate ambiguity, so a \index{Choice axiom (see Axiom of choice)} choice of meanings must be made. Mathematicians have agreed always to use ``if ... then'' in the first sense, so that a statement of the form ``If \(P\) , then \(Q\) `` means that if \(P\) is true, \(Q\) is true also, but if \(P\) is false, \(Q\) may be either true or false.

As an example, consider the following statement about real numbers:
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ . }
\]
It is a statement of the form, ``If \(P\) , then \(Q\) ,'' where \(P\) is the phrase `` \(x > 0\) '' (called the \index{hypothesis} of the statement) and \(Q\) is the phrase `` \({x}^{3} \neq  0\) '' (called the \index{conclusion} of the statement). This is a true statement, for in every case for which the \index{Hypothesis}\index{Hypothesis}\index{hypothesis@Hypothesis}\index{hypothesis@Hypothesis}hypothesis \(x > 0\) holds, the \index{Conclusion}\index{Conclusion}\index{conclusion@Conclusion}\index{conclusion@Conclusion}conclusion \({x}^{3} \neq  0\) holds as well.
...
To return now to the empty set and inclusion, we see that the inclusion \(\varnothing  \subset  A\) does hold for every set \(A\) . Writing \(\varnothing  \subset  A\) is the same as saying, ``If \(x \in  \varnothing\) , then \(x \in  A\) ,'' and this statement is \index{Vacuously true}\index{Vacuously true}\index{vacuously true@Vacuously true}\index{vacuously true@Vacuously true}vacuously true.
...
Given a statement of the form ``If \(P\) , then \(Q\) ,'' its \index{Contrapositive}\index{Contrapositive}\index{contrapositive@Contrapositive}\index{contrapositive@Contrapositive}contrapositive is defined to be the statement ``If \(Q\) is not true, then \(P\) is not true.'' For example, the contrapositive of the statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ . }
\]
is the statement
...
Q \Rightarrow  P
\]
which is called the \index{Converse}\index{Converse}\index{converse@Converse}\index{converse@Converse}converse of \(P \Rightarrow  Q\) . One must be careful to distinguish between a statement's converse and its contrapositive. Whereas a statement and its contrapositive are logically equivalent, the truth of a statement says nothing at all about the truth or falsity of its converse. For example, the true statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ , }
\]
...
\section*{Negation}
If one wishes to form the contrapositive of the statement \(P \Rightarrow  Q\) , one has to know how to form the statement ``not \(P\) ,'' which is called the \index{negation} of \(P\) . In many cases, this causes no difficulty; but sometimes confusion occurs with statements involving the phrases ``for every'' and ``for at least one.'' These phrases are called \index{logical quantifiers@Logical quantifiers}\index{logical quantifiers@Logical quantifiers}logical \index{Quantifiers, logical} quantifiers.

To illustrate, suppose that \(X\) is a set, \(A\) is a subset of \(X\) , and \(P\) is a statement about the \index{Principle of recursive definition!general}\index{Principle of recursive definition!general}\index{Principle of recursive definition!general}\index{Principle of recursive definition!general}general element of \(X\) . Consider the following statement:
\[
( *) \hspace{1em} \text{ For every }x \in  A\text{ , statement }P\text{ holds. }
\]
How does one form the \index{Negation}\index{Negation}\index{negation@Negation}\index{negation@Negation}negation of this statement? Let us translate the problem into the language of sets. Suppose that we let \(B\) denote the set of all those elements \(x\) of \(X\) for which \(P\) holds. Then statement \(( *)\) is just the statement that \(A\) is a subset of \(B\) . What is its negation? Obviously, the statement that \(A\) is not a subset of \(B\) ; that is, the statement that there exists at least one element of \(A\) that does not belong to \(B\) . Translating back into ordinary language, this becomes
\[
...
We return now to our discussion of sets. There is one other \index{Operation, binary} operation on sets that is occasionally useful. It is the \index{difference of two sets}, denoted by \(A - B\) , and defined as the set consisting of those elements of \(A\) that are not in \(B\) . Formally,
\[
A - B = \{ x \mid  x \in  A\text{ and }x \notin  B\} .
\]
It is sometimes called the \index{complement} of \(B\) relative to \(A\) , or the \index{Complement}\index{Complement}\index{complement@Complement}\index{complement@Complement}complement of \(B\) in \(A\) . Our three set operations are represented schematically in Figure 1.1.
...
\]
holds. The equation is illustrated in Figure 1.3; the shaded region represents the set in question, as you can check mentally. This equation can be thought of as a ``\index{Distributive laws for  and} distributive law'' for the operations \(\cap\) and \(\cup\) .
...
Other examples of set-theoretic rules include the \index{Second coordinate of ordered pair} second ``distributive law,''
\[
A \cup  ( {B \cap  C})  = ( {A \cup  B})  \cap  ( {A \cup  C}) ,
\]
and \index{DeMorgan's laws},
\[
\begin{aligned}
...
\end{aligned}
\]
We leave it to you to check these rules. One can state other rules of set theory, but these are the most important ones. \index{DeMorgan's laws}\index{DeMorgan's laws}\index{demorgan's laws@DeMorgan's laws}\index{demorgan's laws@DeMorgan's laws}DeMorgan's laws are easier to remember if you verbalize them as follows:
...
There is yet another way of forming new sets from old ones; it involves the notion of an ``\index{ordered pair}'' of objects. When you studied analytic geometry, the first thing you did was to convince yourself that after one has chosen an \(x\) -axis and a \(y\) -axis in the plane, every point in the plane can be made to correspond to a unique \index{Ordered pair}\index{Ordered pair}\index{ordered pair@Ordered pair}\index{ordered pair@Ordered pair}ordered pair(x, y)of real numbers. (In a more sophisticated treatment of geometry, the plane is more likely to be defined as the set of all ordered pairs of real numbers!)

The notion of ordered pair carries over to general sets. Given sets \(A\) and \(B\) , we define their \index{Cartesian product: countably infinite} cartesian product \(A \times  B\) to be the set of all ordered pairs(a, b)for which \(a\) is an element of \(A\) and \(b\) is an element of \(B\) . Formally,
...
it defines the ordered pair(a, b)as a collection of sets. If \(a \neq  b\) , this definition says that (a, b)is a collection containing two sets, one of which is a one-element set and the other a two-element set. The \index{first coordinate} of the ordered pair is defined to be the element belonging to both sets, and the second coordinate is the element belonging to only one of the sets. If \(a = b\) , then(a, b)is a collection containing only one set \(\{ a\}\) , since \(\{ a,b\}  =\)  \(\{ a,a\}  = \{ a\}\) in this case. Its \index{First coordinate}\index{First coordinate}\index{first coordinate@First coordinate}\index{first coordinate@First coordinate}first coordinate and second coordinate both equal the element in this single set.
...
\item If a set \(A\) has two elements, show that \(\mathcal{P}( A)\) has four elements. How many elements does \(\mathcal{P}( A)\) have if \(A\) has one element? Three elements? No elements? Why is \(\mathcal{P}( A)\) called the \index{Power set}\index{Power set}\index{power set@Power set}\index{power set@Power set}power set \index{Finiteness: of cartesian products!of unions}of \(A\) ?

\item Formulate and prove DeMorgan's laws \index{Least upper bound property!for \(\mathbb{R}\)}\index{Least upper bound property!for \(\mathbb{R}\)}for arbitrary unions and intersections.
...
\section*{§ 2 Functions}
The concept \index{Restriction: of a covering map!of a function}\index{Restriction: of a covering map!of a function}of \index{Function}\index{Function}\index{function@Function}\index{function@Function}function is one you have seen many times already, so it is hardly necessary to remind you how central it is to all mathematics. In this section, we give the precise mathematical definition, and we explore some of the associated concepts.

A function is usually thought of as a rule that assigns to each \index{Element of a set}\index{Element of a set}\index{element of a set@Element of a set}\index{element of a set@Element of a set}element of a set \(A\) , an element of a set \(B\) . In calculus, a function is often given by a simple formula such as \(f( x)  = 3{x}^{2} + 2\) or perhaps by a more complicated formula such as
...
\begin{definition} 
    ...
    Thus, a subset \(r\) of \(C \times  D\) is a \index{Rule of assignment}\index{Rule of assignment}\index{rule of assignment@Rule of assignment}\index{rule of assignment@Rule of assignment}rule of assignment if
    \[
    ...
    \end{aligned}
    \]
    Note that given a rule of assignment \(r\) , its \index{Domain}\index{Domain}\index{domain@Domain}\index{domain@Domain}domain and \index{Image}\index{Image}\index{image@Image}\index{image@Image}image are entirely determined.
\end{definition}

Now we can say what a function is.

\begin{definition}
    A function \(f\) is a rule of assignment \(r\) , together with a set \(B\) that \index{Contains}\index{Contains}\index{contains@Contains}\index{contains@Contains}contains the image set of \(r\) . The domain \(A\) of the rule \(r\) is also called the domain of the function \(f\) ; the image set of \(r\) is also called the image set of \(f\) ; and the set \(B\) is called the \index{Range of a function} \index{range of a function@Range of a function}\index{range of a function@Range of a function}range of \(f{.}^{\dagger}\) .
\end{definition}
...
If \(f : A \rightarrow  B\) and if \(a\) is an element of \(A\) , we denote by \(f( a)\) the unique element of \(B\) that the rule determining \(f\) assigns to \(a\) ; it is called the \index{Value of a function} value of \(f\) at \(a\) , or sometimes the image of \(a\) under \(f\) . Formally, if \(r\) is the rule of the function \(f\) , then \(f( a)\) denotes the unique element of \(B\) such that \(( {a,f( a) })  \in  r\) .
...
Restricting the domain of a function and changing its range are two ways of forming a new function from an old one. Another way is to form the \index{Composite: of functions} composite of two functions.
...
If \(f\) is bijective, there exists a function from \(B\) to \(A\) called the \index{Inverse function} of \(f\) . It is denoted by \({f}^{-1}\) and is defined by letting \({f}^{-1}( b)\) be that unique element \(a\) of \(A\) for which \(f( a)  = b\) . Given \(b \in  B\) , the fact that \(f\) is surjective implies that there exists such an element \(a \in  A\) ; the fact that \(f\) is injective implies that there is only one such element \(a\) . It is easy to see that if \(f\) is bijective, \({f}^{-1}\) is also bijective.
...
(1) (\index{Comparability: of cardinalities} Comparability) For every \(x\) and \(y\) in \(A\) for which \(x \neq  y\) , either \({xCy}\) or \({yCx}\) .
...
(2) (\index{Principle of induction}). If \(A\) is an \index{inductive set} of positive integers, then \(A =\)  \({Z}_{ + }\) .\\
...
\begin{theorem}[\index{Strong induction principle}]
...
For one thing, you need the least upper bound axiom to prove that the set \({\mathbb{Z}}_{ + }\) of positive integers has no upper bound in \(\mathbb{R}\) . This is the \index{Archimedean ordering} property of the real line. To prove it, we assume that \({\mathbb{Z}}_{ + }\) has an upper bound and derive a contradiction. If \({\mathbb{Z}}_{ + }\) has an upper bound, it has a least upper bound \(b\) . There exists \(n \in  {\mathbb{Z}}_{ + }\) such that \(n > b - 1\) ; for otherwise, \(b - 1\) would be an upper bound for \({\mathbb{Z}}_{ + }\) smaller than \(b\) . Then \(n + 1 > b\) , contrary to the fact that \(b\) is an upper bound for \({\mathbb{Z}}_{ + }\) .
...
\index{Finite Set} sets and \index{infinite set}, \index{countable set} and \index{uncountable set}, these are types of sets that you may have encountered before. Nevertheless, we shall discuss them in this section and the next, not only to make sure you understand them thoroughly, but also to elucidate some particular points of logic that will arise later on. First we consider finite sets.
...
\begin{definition}
    ...
    \]
    for some positive integer \(n\) . In the former case, we say that \(A\) has \index{Cardinality: comparability} cardinality 0; in the latter case, we say that \(A\) has cardinality \(n\) .
\end{definition}
...
What is needed is another principle, which we call the principle of \index{Recursive definition, principle} recursive definition. In the proof of the preceding theorem, we wished to assert the following:
...
\]
for example, is self-contradictory; although \(h( i)\) necessarily is an element of the set \(h( {\{ 1,\ldots ,i + 1\} })\) , this formula says that it does not belong to the set. Another example is the classic \index{Russell's paradox} paradox:

\begin{centeredblock}
    Let the \index{Barber of Seville paradox} barber of Seville shave every man of Seville who does not shave himself. Who shall shave the barber?
\end{centeredblock}
...
\item We say that two sets \(A\) and \(B\) \index{have the same cardinality} if there is a bijection of \(A\) with \(B\) .
  ...
  \item Theorem (\index{Schroeder-Bernstein theorem}). If there are injections \(f : A \rightarrow\)  \(C\) and \(g : C \rightarrow  A\) , then \(A\) and \(C\) have the same cardinality.
  \end{enumerate}
...
\item The \index{Fibonacci numbers} of number theory are defined recursively by the formula \[ \begin{aligned} {\lambda }_{1} &= {\lambda }_{2} = 1 \\ {\lambda }_{n} &= {\lambda }_{n - 1} + {\lambda }_{n - 2}\;\text{ for }n > 2. \end{aligned} \] Define them rigorously by use of Theorem 8.4.
...
\begin{example}
    ...
    \]
    If \({y}_{0} = {y}_{1}\) and \({x}_{0} < {x}_{1}\). This is a \index{Partial order}partial ordering of \({\mathbb{R}}^{2}\) under which two points are comparable only if they lie on the same horizontal line. The maximal simply ordered sets are the horizontal lines in \({\mathbb{R}}^{2}\).
\end{example}
\end{centeredblock}
...
\]
By the \index{Principle of recursive definition}principle of recursive definition, this formula determines a unique function \(h.{\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) It is easy to check that the set of those \({a}_{j}\) for which \(h( j)  = 0\) is a maximal simply ordered subset of \(A\) .

If \(A\) is not countable, a variant of this procedure will work, if we allow ourselves to use the \index{Well-ordering theorem}well-ordering theorem. Instead of indexing the elements of \(A\) with the set \({\mathbb{Z}}_{ + }\) , we index them (in a bijective fashion) with the elements \index{Section: of the positive integers!of a well-ordered set}of some \index{Well-ordered set}well-ordered set \(J\) , so that \(A = \{  {{a}_{\alpha } \mid  \alpha  \in  J}\}\) . For this we need the \index{Well-ordering theorem}well-ordering theorem, so that we know there is a bijection between \(A\) and some \index{Well-ordered set}well-ordered set \(J\) . Then we can proceed as in the previous paragraph, letting \(\alpha\) replace \(i\) in the argument. \index{Partial order!strict}Strictly speaking, you need to generalize the principle of recursive definition to well-ordered sets as well, but that is not particularly difficult. (See the Supplementary Exercises.)

Thus, the well-ordering theorem implies the \index{Maximum principle}maximum principle.

Although the maximum principle of Hausdorff was the first to be formulated and is probably the simplest to understand, there is another such principle that is nowadays the one most frequently quoted. It is popularly called ``\index{Zorn's Lemma},'' although Ku-ratowski (1922) and Bochner (1922) preceded Zorn (1935) in enunciating and proving versions of it. For a history and discussion of the tangled history of these ideas, see \cite{Campbell1978} or \cite{Moore1982}. To state this principle, we need some terminology.
...
It is also true that the \index{Maximum principle}maximum principle is an easy consequence of \index{Zorn's lemma}Zorn's lemma. See Exercises 5-7.

One final remark. We have defined what we mean by a \index{Strict partial order}strict partial order on a set, but we have not said what a partial order itself is. Let \(\prec\) be a strict partial order on a set \(A\). Suppose that we define \(a \preccurlyeq  b\) if either \(a \prec  b\) or \(a = b\) . Then the relation \(\preccurlyeq\) is called a partial order on \(A\). For example, the inclusion relation \(\subset\) on a collection of sets is a partial order, whereas \index{Proper inclusion}proper inclusion is a strict partial order.
...
\item Show that \index{Zorn's lemma}Zorn's lemma implies the following: \\ \textbf{Lemma (Kuratowski).} Let \(\mathcal{A}\) be a collection of sets. Suppose that for every subcollection \(\mathcal{B}\) of \(\mathcal{A}\) that is simply ordered by proper inclusion, the union of the elements of \(\mathcal{B}\) belongs to \(\mathcal{A}\) Then \(\mathcal{A}\) has an element that is properly contained in no other element of \(\mathcal{A}\) .
\item A collection \(\mathcal{A}\) of subsets of a set \(X\) is said to be of finite type provided that a subset \(B\) of \(X\) belongs to \(\mathcal{A}\) if and only if every finite subset of \(B\) belongs to \(\mathcal{A}\) . Show that the \index{Kuratowski lemma} implies the following. \\ \textbf{Lemma (Tukey, 1940).} Let \(\mathcal{A}\) be a collection of sets. If \(\mathcal{A}\) is of finite type, then \(\mathcal{A}\) has an element that is properly contained in no other element of \(\mathcal{A}\) .
\item Show that the \index{Tukey lemma} implies the \index{Hausdorff maximum principle}. [Hint: If \(\prec\) is a strict partial order on \(A\) , let \(\mathcal{A}\) be the collection of all subsets of \(A\) that are simply ordered by \(\prec\) . Show that \(\mathcal{A}\) is of finite type.]
\item A typical use of Zorn's lemma in algebra is the proof that every vector space has a basis. Recall that if \(A\) is a subset of the vector space \(V\) , we say a vector belongs to the span of \(A\) if it equals a finite linear combination of elements of \(A\) . The set \(A\) is independent if the only finite linear combination of elements of \(A\) that equals the zero vector is the trivial one having all coefficients zero. If \(A\) is independent and if every vector in \(V\) belongs to the span of \(A\) , then \(A\) is a basis for \(V\) .
  ...
  \item Show the collection of all independent sets in \(V\) has a \index{Maximal element}maximal element.
...
\item
  ...
  \item If \(E\) is a \index{well-ordered set@Well-ordered set}well-ordered set, show that no section of \(E\) has the \index{Order type}order type of \(E\) , nor do two different sections of \(E\) have the same order type. [Hint: Given \(J\) , there is at most one order-preserving map of \(J\) into \(E\) whose image is \(E\) or a section of \(E\) .]
  \end{enumerate}
...
\item Let \(X\) be a set; let \(\mathcal{A}\) be the collection of all pairs \(( {A, < })\) , where \(A\) is a subset of \(X\) and \(<\) is a well-ordering of \(A\) . Define \[ ( {A, < })  \prec  ( {{A}^{\prime },{ < }^{\prime }}) \] if \(( {A, < })\) equals a section of \(( {{A}^{\prime },{ < }^{\prime }})\) .
  ...
  \end{enumerate}
\item Use Exercises 1 and 5 to prove the following: \\ \textsl{Theorem. The \index{maximum principle@Maximum principle}maximum principle is equivalent to the \index{well-ordering theorem@Well-ordering theorem}well-ordering theorem.}
\item Use Exercises 1-5 to prove the following. \\ \textsl{Theorem. The choice axiom is equivalent to the \index{well-ordering theorem@Well-ordering theorem}well-ordering theorem.} \\ \textsl{Proof.} Let \(X\) be a set, let \(c\) be a fixed \index{choice function@Choice function}choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(( {T, < })\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) , \[ x = c( {X - {S}_{x}( T) }) , \] where \({S}_{x}( T)\) is the section of \(T\) by \(x\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Using Exercises 1-4, construct an uncountable well-ordered set, as follows. Let \(A\) be the collection of all pairs \(( {A, < })\) , where \(A\) is a subset \index{Countability!of \(\mathbb{Z}\)}of \({\mathbb{Z}}_{ + }\) and \(<\) is a well-ordering of \(A\) . (We allow \(A\) to be empty.) Define \(( {A, < })  \sim  ( {{A}^{\prime },{ < }^{\prime }})\) if \(( {A, < })\) and \(( {{A}^{\prime },{ < }^{\prime }})\) have the same \index{Order type}order type. It is trivial to show this is an \index{Equivalence relation}equivalence relation. Let \([  ( {A, < }) ]\) denote the \index{Equivalence class}equivalence class of \(( {A, < })\) ; let \(E\) denote the collection of these \index{Equivalence class}equivalence classes. Define \[ [  ( {A, < }) ] \ll  [  ( {{A}^{\prime },{ < }^{\prime }}) ] \] if \(( {A, < })\) has the \index{order type@Order type}order type of a section of \(( {{A}^{\prime },{ < }^{\prime }})\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Show that if \(\alpha  = [  ( {A, < }) ]\) is an element of \(E\) , then \(( {A, < })\) has the same \index{order type@Order type}order type as the section \({S}_{\alpha }( E)\) of \(E\) by \(\alpha\) . [Hint: Define a map \(f : A \rightarrow  E\) by setting \(f( x)  = [  ( {{S}_{x}( A) \text{ , restriction of } < }) ]\) for each \(x \in  A\) .]
  \item Conclude that \(E\) is well-ordered by \(\ll\) .
  \item Show that \(E\) is \index{Well-ordered set!uncountable}uncountable. [Hint: If \(h : E \rightarrow  {\mathbb{Z}}_{ + }\) is a bijection, then \(h\) gives rise to a well-ordering \index{Countability!of \(\mathbb{Z}\)}of \({\mathbb{Z}}_{ + }\) .]
  \end{enumerate}
