...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the intersection\idx{Section: of the positive integers} of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
\item Let \(\idxmath{R}{\(\mathbb{R}\) (reals)} \mathbb{R}\) denote the set of real numbers. For each of the following subsets of \(\mathbb{R} \times  \mathbb{R}\) , determine whether it is equal to the cartesian product of two subsets of \(\mathbb{R}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Let \(f : A \rightarrow  B\) and let \({A}_{i} \subset  A\) and \({B}_{i} \subset  B\) for \(i = 0\) and \(i = 1\) . Show that \({f}^{ - }\) preserves inclusions, unions, intersections, and differences of sets:
  ...
  \item \({f}^{-1}\left( {{B}_{0} - {B}_{1}}\right)  = {f}^{-1}\left( {B}_{0}\right)  - {f}^{-1}\left( {B}_{1}\right)\) . Show that \(f\) preserves inclusions and unions only:
...
\begin{centeredblock}
\begin{example}
    Consider the set \(\lbrack 0,1)\) of real numbers and the set \(\idxmath{Z}{\({\mathbb{Z}}_{ + }\)} {\idxmath{Z}{\(\mathbb{Z}\)} \mathbb{Z}}_{ + }\) of \idx{positive integers}, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
    \[
...
\item Let \(f : A \rightarrow  B\) be a surjective function. Let us define a relation on \(A\) by setting \({a}_{0} \sim  {a}_{1}\) if \[ f\left( {a}_{0}\right)  = f\left( {a}_{1}\right) . \]
  ...
  \end{enumerate}
\item Let \(S\) and \({S}^{\prime }\) be the following subsets of the plane. \[ S = \{ \left( {x,y}\right)  \mid  y = x + 1\text{ and }0 < x < 2\} , \] \[ {S}^{\prime } = \{ \left( {x,y}\right)  \mid  y - x\text{ is an integer }\} . \]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Describe the equivalence relation \(T\) on the real line that is the intersection of all equivalence relations on the real line that contain \(S\) . Describe the equivalence classes of \(T\) . Order Relations
  \end{enumerate}
...
\item (a) Show that the map \(f : \left( {-1,1}\right)  \rightarrow  \mathbb{R}\) of Example 9 is order preserving.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Let \({\mathbb{Z}}_{ + }\) denote the set of positive integers. Consider the following order relations on \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) :
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item The dictionary order. (ii) \(\left( {{x}_{0},{y}_{0}}\right)  < \left( {{x}_{1},{y}_{1}}\right)\) if either \({x}_{0} - {y}_{0} < {x}_{1} - {y}_{1}\) , or \({x}_{0} - {y}_{0} = {x}_{1} - {y}_{1}\) and \({y}_{0} < {y}_{1}\) . (iii) \(\left( {{x}_{0},{y}_{0}}\right)  < \left( {{x}_{1},{y}_{1}}\right)\) if either \({x}_{0} + {y}_{0} < {x}_{1} + {y}_{1}\) , or \({x}_{0} + {y}_{0} = {x}_{1} + {y}_{1}\) and \({y}_{0} < {y}_{1}\) . In these order relations, which elements have immediate predecessors? Does the set have a smallest element? Show that all three order types are different.
  \end{enumerate}
...
\item (a) Show that \(\mathbb{R}\) has the greatest lower bound property.
  ...
  \end{enumerate}
\item (a) Show that every nonempty subset of \(\mathbb{Z}\) that is bounded above has a largest element.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item (a) Show that if \(n > 1\) there is bijective correspondence of \[ {A}_{1} \times  \cdots  \times  {A}_{n}\;\text{ with }\;\left( {{A}_{1} \times  \cdots  \times  {A}_{n - 1}}\right)  \times  {A}_{n}. \]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Given the indexed family \(\left\{  {{A}_{1},{A}_{2},\ldots }\right\}\) , let \({B}_{t} = {A}_{{2i} - 1} \times  {A}_{2i}\) for each positive integer \(i\) . Show there is bijective correspondence of \({A}_{1} \times  {A}_{2} \times  \cdots\) with \({B}_{1} \times  {B}_{2} \times  \cdots\)
  \end{enumerate}
\item Let \(A = {A}_{1} \times  {A}_{2} \times  \cdots\) and \(B = {B}_{1} \times  {B}_{2} \times  \cdots\) .
  ...
  \item Show that if \(A\) is nonempty, each \({A}_{t}\) is nonempty. Does the converse hold? (We will return to this question in the exercises of §19.)
...
\item Which of the following subsets of \({\mathbb{R}}^{\omega }\) can be expressed as the cartesian product of subsets of \(\mathbb{R}\) ?
  ...
  \item \(\left\{  {x \mid  {x}_{2} = {x}_{3}}\right\}\) .
  \end{enumerate}
\end{enumerate}
...
\section*{Exercises}
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item (a) Make a list of all the injective maps \[ f : \{ 1,2,3\}  \rightarrow  \{ 1,2,3,4\} . \] Show that none is bijective. (This constitutes a direct proof that a set \(A\) of cardinality three does not have cardinality four.)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item (a) Let \(A = \{ 1,\ldots ,n\}\) . Show there is a bijection of \(\mathcal{P}\left( A\right)\) with the cartesian product \({X}^{n}\) , where \(X\) is the two-element set \(X = \{ 0,1\}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Determine, for each of the following sets, whether or not it is countable. Justify your answers.
  ...
  \item The set \(C = \mathop{\bigcup }\limits_{{n \in  {\mathbf{Z}}_{ + }}}{B}_{n}\) .
...
\item (a) The formula \[ \left( *\right) \qquad \begin{aligned} h\left( 1\right)  &= 1, \\ h\left( 2\right)  &= 2\text{ , } \\ h\left( n\right)  &= {\left\lbrack  h\left( n + 1\right) \right\rbrack  }^{2} - {\left\lbrack  h\left( n - 1\right) \right\rbrack  }^{2}\;\text{ for }n \geq  2 \end{aligned} \] is not one to which the principle of recursive definition applies. Show that nevertheless there does exist a function \(h : {\mathbb{Z}}_{ + } \rightarrow  \mathbb{R}\) satisfying this formula. [Hint: Reformulate \(\left( *\right)\) so that the principle will apply and require \(h\) to be positive.]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Obtain the definitions of \({a}^{n}\) and \(n\) ! for \(n \in  {\mathbb{Z}}_{ + }\) as special cases of Exercise 2. 4. The \idx{Fibonacci numbers} of number theory are defined recursively by the formula \[ \begin{aligned} {\lambda }_{1} &= {\lambda }_{2} = 1 \\ {\lambda }_{n} &= {\lambda }_{n - 1} + {\lambda }_{n - 2}\;\text{ for }n > 2. \end{aligned} \] Define them rigorously by use of Theorem 8.4.
...
\item (a) Show that there is no function \(h : {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{R}}_{ + }\) satisfying the formula \[ \begin{aligned} h\left( 1\right)  &= 3, \\ h\left( i\right)  &= {\left\lbrack  h\left( i - 1\right)  - 1\right\rbrack  }^{1/2}\;\text{ for }i > 1. \end{aligned} \] Explain why this example does not violate the principle of recursive definition.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\noindent \textbf{Axiom of choice.} \textsl{Given a collection \(\mathcal{A}\) of disjoint nonempty sets, there exists a set \(C\) consisting of exactly one element from each element of \(\mathcal{A}\) ; that is, a set \(\mathbf{C}\) such that \(\mathbf{C}\) is contained in the union of the elements of \(\mathcal{A}\) , and for each \(A \in  \mathcal{A}\) , the set \(C \cap  A\) contains a single element.}
...
\item Find if possible a choice function for each of the following collections, without using the choice axiom:
  ...
  \end{enumerate}
\item Suppose that \(A\) is a set and \({\left\{  {f}_{n}\right\}  }_{n \in  {\mathbf{Z}}_{ + }}\) is a given indexed family of injective functions \[ {f}_{n} : \{ 1,\ldots ,n\}  \rightarrow  A \] Show that \(A\) is infinite. Can you define an injective function \(f : {\mathbb{Z}}_{ + } \rightarrow  A\) without using the choice axiom?
...
\item (a) Use the choice axiom to show that if \(f : A \rightarrow  B\) is surjective, then \(f\) has a right inverse \(h \cdot  B \rightarrow  A\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\begin{definition}
    Let \(X\) be a well-ordered set. Given \(\alpha  \in  X\) , let \(\idxmath{S}{\({S}_{\alpha }\) (section of well-ordered set)} {S}_{\alpha }\) denote the set
    \[
...
\item (a) Let \({\mathbb{Z}}_{ - }\) denote the set of negative integers in the usual order. Show that a simply ordered set \(A\) fails to be well-ordered if and only if it contains a subset having the same order type as \({\mathbb{Z}}_{ - }\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item (a) Let \({A}_{1}\) and \({A}_{2}\) be \idx{disjoint sets}, well-ordered by \({ < }_{1}\) and \({ < }_{2}\) , respectively. Define an order relation on \({A}_{1} \cup  {A}_{2}\) by letting \(a < b\) either if \(a,b \in  {A}_{1}\) and \(a{ < }_{1}b\) , or if \(a,b \in  {A}_{2}\) and \(a{ < }_{2}b\) , or if \(a \in  {A}_{1}\) and \(b \in  {A}_{2}\) . Show that this is a well-ordering
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item (a) Let \(\prec\) be a strict partial order on the set \(A\) . Define a relation on \(A\) by letting \(a \preccurlyeq  b\) if either \(a \prec  b\) or \(a = b\) . Show that this relation has the following properties, which are called the partial order axioms:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \(a \preccurlyeq  a\) for all \(a \in  A\) . (ii) \(a \preccurlyeq  b\) and \(b \preccurlyeq  a \Rightarrow  a = b\) . (iii) \(a \preccurlyeq  b\) and \(b \preccurlyeq  c \Rightarrow  a \preccurlyeq  c\) .
...
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item Theorem (General principle of recursive definition). Let \(J\) be a well-ordered set; let \(C\) be a set. Let \(\mathcal{F}\) be the set of all functions mapping sections of \(J\) into \(C\) . Given a function \(\rho  : \mathcal{F} \rightarrow  C\) , there exists a unique function \(h : J \rightarrow  C\) such that \(h\left( \alpha \right)  = \rho \left( {h \mid  {S}_{\alpha }}\right)\) for each \(\alpha  \in  J\) . [Hint: Follow the pattern outlined in Exercise 10 of \(§{10}\) .]
\item (a) Let \(J\) and \(E\) be well-ordered sets, let \(h : J \rightarrow  E\) . Show the following two statements are equivalent:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \(h\) is order preserving and its image is \(E\) or a section of \(E\) . (ii) \(h\left( \alpha \right)  =\) smallest \(\left\lbrack  {E - h\left( {S}_{\alpha }\right) }\right\rbrack\) for all \(\alpha\) . [Hint: Show that each of these conditions implies that \(h\left( {S}_{\alpha }\right)\) is a section of \(E\) , conclude that it must be the section by \(h\left( \alpha \right)\) .]
  ...
  \end{enumerate}
\item Let \(J\) and \(E\) be well-ordered sets; suppose there is an order-preserving map \(k : J \rightarrow  E\) . Using Exercises 1 and 2, show that \(J\) has the order type of \(E\) or a section of \(E\) . [Hint: Choose \({e}_{0} \in  E\) . Define \(h : J \rightarrow  E\) by the recursion formula \[ h\left( \alpha \right)  = \text{ smallest }\left\lbrack  {E - h\left( {S}_{\alpha }\right) }\right\rbrack  \;\text{ if }\;h\left( {S}_{\alpha }\right)  \neq  E, \] and \(h\left( \alpha \right)  = {e}_{0}\) otherwise. Show that \(h\left( \alpha \right)  \leq  k\left( \alpha \right)\) for all \(\alpha\) ; conclude that \(\left. {h\left( {S}_{\alpha }\right)  \neq  E\text{ for all }\alpha .}\right\rbrack\)
...
\item Let \(X\) be a set; let \(\mathcal{A}\) be the collection of all pairs \(\left( {A, < }\right)\) , where \(A\) is a subset of \(X\) and \(<\) is a well-ordering of \(A\) . Define \[ \left( {A, < }\right)  \prec  \left( {{A}^{\prime },{ < }^{\prime }}\right) \] if \(\left( {A, < }\right)\) equals a section of \(\left( {{A}^{\prime },{ < }^{\prime }}\right)\) .
  ...
  \end{enumerate}
\item Use Exercises 1 and 5 to prove the following: \textbf{Theorem.} The maximum principle is equivalent to the well-ordering theorem.
\item Use Exercises 1-5 to prove the following. \textbf{Theorem.} The choice axiom is equivalent to the well-ordering theorem. \textit{Proof.} Let \(X\) be a set, let \(c\) be a fixed choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(\left( {T, < }\right)\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) , \[ x = c\left( {X - {S}_{x}\left( T\right) }\right) , \] where \({S}_{x}\left( T\right)\) is the section of \(T\) by \(x\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
