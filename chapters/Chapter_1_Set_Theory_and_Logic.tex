...
13. Prove the following:
    ...
    \textsl{theorem}. If an ordered set \(A\) has the least upper bound property, then it has the greatest lower bound property.
...
(a) Show that if \({B}_{i} \subset  {A}_{i}\) for all \(i\) , then \(B \subset  A\) . (Strictly speaking, if we are given a function mapping the index set \({\mathbb{Z}}_{ + }\) into the union of the sets \({B}_{i}\) , we must change its range before it can be considered as a function mapping \({\mathbb{Z}}_{ + }\) into the union of the sets \({A}_{i}\) . We shall ignore this technicality when dealing with cartesian products).
...
is not one to which the principle of recursive definition applies. Show that nevertheless there does exist a function \(h : {\mathbb{Z}}_{ + } \rightarrow  \mathbb{R}\) satisfying this formula. [Hint: Reformulate \(\left( *\right)\) so that the principle will apply and require \(h\) to be positive.]

(b) Show that the formula \(\left( *\right)\) of part (a) does not determine \(h\) uniquely. [Hint: If \(h\) is a positive function satisfying \(\left( *\right)\) , let \(f\left( i\right)  = h\left( i\right)\) for \(i \neq  3\) , and let \(f\left( 3\right)  =  - h\left( 3\right)\) .]
...
3. Obtain the definitions of \({a}^{n}\) and \(n\) ! for \(n \in  {\mathbb{Z}}_{ + }\) as special cases of Exercise 2. 4. The \idx{Fibonacci numbers} of number theory are defined recursively by the formula
...
\textbf{Theorem (The principle of \idx{transfinite induction}).} If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .
...
\[
\left( *\right) \qquad h\left( x\right)  = \text{ smallest }\left\lbrack  {C - h\left( {S}_{x}\right) }\right\rbrack
\]
...
(a) If \(h\) and \(k\) map sections of \(J\) , or all of \(J\) , into \(C\) and satisfy \(\left( *\right)\) for all \(x\) in their respective domains, show that \(h\left( x\right)  = k\left( x\right)\) for all \(x\) in both domains.
...
\textbf{Theorem.} The maximum principle is equivalent to the well-ordering theorem.

7. Use Exercises 1-5 to prove the following.

\textbf{Theorem.} The choice axiom is equivalent to the well-ordering theorem.

\textit{Proof.} Let \(X\) be a set, let \(c\) be a fixed choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(\left( {T, < }\right)\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) ,
    \[
