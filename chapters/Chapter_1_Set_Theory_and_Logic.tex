...
\begin{example}
    ...
    \(g : {\overline{\mathbb{R}}}_{ + } \rightarrow  \mathbb{R}\;\) defined by \(\;g\left( x\right)  = {x}^{2},\)
...
\begin{centeredblock}
\begin{example}
    Define two points in the plane to be equivalent if they lie at the same distance from the ongin. Reflexivity, symmetry, and transitivity hold trivially. The collection \(\varepsilon\) of equivalence classes consists of all circles centered at the origin, along with the set consisting of the origin alone.
\end{example}
\end{centeredblock}
...
\begin{theorem}[\idx{Strong induction principle}]
    Let \(A\) be a set of positive integers. Suppose that for each posiave integer \(n\) , the statement \({S}_{n} \subset  A\) implies the statement \(n \in  A\) . Then \(A = {\mathbb{Z}}_{ + }\) .
\end{theorem}
...
\begin{centeredblock}
\begin{example}
    How does the cartesian product \(A \times  B \times  C\) differ from the cartesian products \(A \times  \left( {B \times  C}\right)\) and \(\left( {A \times  B}\right)  \times  {C}^{7}\) Very little. There are obvious bijective correspondences between these sets, indicated as follows
    \[
...
\begin{centeredblock}
\begin{example}
    Consider the set \(\{ 1,2\}  \times  {\mathbb{Z}}_{ + }\) in the dictionary ordering. Schernatically, it can be represented as one infinite sequence followed by another infinite sequence:
    \[
