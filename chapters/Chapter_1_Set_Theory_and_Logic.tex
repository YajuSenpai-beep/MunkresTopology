...
The \index{Equality symbol} \(=\) is used throughout this book to mean \index{logical equivalence@Logical equivalence} logical identity. Thus, when we write \(a = b\) , we mean that `` \(a\) `` and `` \(b\) `` are symbols for the \index{Cardinality: comparability!same}same object. This is what one means in arithmetic, for example, when one writes \(\displaystyle\frac{2}{4} = \frac{1}{2}\) . Similarly, the equation \(A = B\) states that `` \(A\) `` and `` \(B\) `` are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
A \subset  B\text{ . }
\]
Nothing in this definition requires \(A\) to be different from \(B\) ; in fact, if \(A = B\) , it is true that both \(A \subset  B\) and \(B \subset  A\) . If \(A \subset  B\) and \(A\) is different from \(B\) , we say that \(A\) is a \index{Proper subset} \index{Restriction: of a covering map!of a relation}of \(B\) , and we write
\[
A \varsubsetneq  B\text{ . }
\]
The \index{Relation} \(\subset\) and \(\varsubsetneq\) are called \index{Inclusion} and \index{Proper inclusion}, respectively. If \(A \subset  B\) , we also write \(B \supset  A\) , which is read `` \(B\) \index{Contains} \(A\) .''
...
The usual way to specify a set, however, is to take some set \(A\) of objects and some property that elements of \(A\) may or may not possess, and to form the set consisting of all elements of \(A\) having that property. For instance, one might take the set of real numbers and form the \index{subset@Subset}subset \(B\) consisting of all even \index{Integers}. In symbols, this statement becomes
\[
...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the \index{intersection@Intersection}intersection\index{section: of the positive integers@Section: of the positive integers} of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
To take care of this eventuality, we make a special convention. We introduce a special set that we call the \index{Empty set}, denoted by \(\varnothing\) , which we think of as ``the set having no elements.''
...
Now some students are bothered by the notion of an ``\index{empty set@Empty set}empty set.'' ``How,'' they say, ``can you have a set with nothing in it?'' The problem is similar to that which arose many years ago when the number 0 was first introduced.
...
\]
It is a statement of the form, ``If \(P\) , then \(Q\) ,'' where \(P\) is the phrase `` \(x > 0\) '' (called the \index{Hypothesis} of the statement) and \(Q\) is the phrase `` \({x}^{3} \neq  0\) '' (called the \index{Conclusion} of the statement). This is a true statement, for in every case for which the \index{hypothesis@Hypothesis}hypothesis \(x > 0\) holds, the \index{conclusion@Conclusion}conclusion \({x}^{3} \neq  0\) holds as well.

Another true statement about real numbers is the following:
\[
\text{ If }{x}^{2} < 0\text{ , then }x = {23}\text{ ; }
\]
in every case for which the hypothesis holds, the conclusion holds as well. Of course, it happens in this example that there are no cases for which the hypothesis holds. A statement of this sort is sometimes said to be \index{Vacuously true}.

To return now to the empty set and inclusion, we see that the inclusion \(\varnothing  \subset  A\) does hold for every set \(A\) . Writing \(\varnothing  \subset  A\) is the same as saying, ``If \(x \in  \varnothing\) , then \(x \in  A\) ,'' and this statement is \index{vacuously true@Vacuously true}vacuously true.

\section*{Contrapositive and Converse}
Our discussion of the ``if ... then'' construction leads us to consider another point of elementary logic that sometimes causes difficulty. It concerns the relation between a statement, its \index{Contrapositive}, and its \index{Converse}.

Given a statement of the form ``If \(P\) , then \(Q\) ,'' its \index{contrapositive@Contrapositive}contrapositive is defined to be the statement ``If \(Q\) is not true, then \(P\) is not true.'' For example, the contrapositive of the statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ . }
\]
is the statement
...
Q \Rightarrow  P
\]
which is called the \index{converse@Converse}converse of \(P \Rightarrow  Q\) . One must be careful to distinguish between a statement's converse and its contrapositive. Whereas a statement and its contrapositive are logically equivalent, the truth of a statement says nothing at all about the truth or falsity of its converse. For example, the true statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ , }
\]
...
To illustrate, suppose that \(X\) is a set, \(A\) is a subset of \(X\) , and \(P\) is a statement about the \index{Principle of recursive definition!general}general element of \(X\) . Consider the following statement:
\[
( *) \hspace{1em} \text{ For every }x \in  A\text{ , statement }P\text{ holds. }
\]
How does one form the \index{negation@Negation}negation of this statement? Let us translate the problem into the language of sets. Suppose that we let \(B\) denote the set of all those elements \(x\) of \(X\) for which \(P\) holds. Then statement \(( *)\) is just the statement that \(A\) is a subset of \(B\) . What is its negation? Obviously, the statement that \(A\) is not a subset of \(B\) ; that is, the statement that there exists at least one element of \(A\) that does not belong to \(B\) . Translating back into ordinary language, this becomes
\[
...
We return now to our discussion of sets. There is one other \index{operation, binary@Operation, binary} operation on sets that is occasionally useful. It is the \index{Difference of two sets}, denoted by \(A - B\) , and defined as the set consisting of those elements of \(A\) that are not in \(B\) . Formally,
\[
A - B = \{ x \mid  x \in  A\text{ and }x \notin  B\} .
\]
It is sometimes called the \index{Complement} of \(B\) relative to \(A\) , or the \index{complement@Complement}complement of \(B\) in \(A\) . Our three set operations are represented schematically in Figure 1.1.
...
\]
and \index{demorgan's laws@DeMorgan's laws},
\[
\begin{aligned}
...
\end{aligned}
\]
We leave it to you to check these rules. One can state other rules of set theory, but these are the most important ones. \index{demorgan's laws@DeMorgan's laws}DeMorgan's laws are easier to remember if you verbalize them as follows:
...
When we have a set whose elements are sets, we shall often refer to it as a \index{Collection} of sets and denote it by a script letter such as \(\mathcal{A}\) or \(\mathcal{B}\) . This device will help us in keeping things straight in arguments where we have to consider objects, and sets of objects, and \index{collection@Collection}collections of sets of objects, all at the same time. For example, we might use \(\mathcal{A}\) to denote the collection of all decks of cards in the world, letting an ordinary capital letter \(A\) denote a deck of cards and a lowercase letter \(a\) denote a single playing card.

A certain amount of care with notation is needed at this point. We make a distinction between the object \(a\) , which is an \index{Element of a set} \(A\) , and the one-element set \(\{ a\}\) , which is a subset of \(A\) . To illustrate, if \(A\) is the set \(\{ a,b,c\}\) , then the statements
\[
...
\item If a set \(A\) has two elements, show that \(\mathcal{P}( A)\) has four elements. How many elements does \(\mathcal{P}( A)\) have if \(A\) has one element? Three elements? No elements? Why is \(\mathcal{P}( A)\) called the \index{power set@Power set}power set \index{Finiteness: of cartesian products!of unions}of \(A\) ?
...
\section*{§ 2 Functions}
The concept \index{Restriction: of a covering map!of a function}of \index{function@Function}function is one you have seen many times already, so it is hardly necessary to remind you how central it is to all mathematics. In this section, we give the precise mathematical definition, and we explore some of the associated concepts.

A function is usually thought of as a rule that assigns to each \index{element of a set@Element of a set}element of a set \(A\) , an element of a set \(B\) . In calculus, a function is often given by a simple formula such as \(f( x)  = 3{x}^{2} + 2\) or perhaps by a more complicated formula such as
\[
...
\begin{definition} 
    A \index{Rule of assignment} is a subset \(r\) of the cartesian product \(\mathbf{C} \times  \mathbf{D}\) of two sets, having the property that each element of \(C\) appears as the first coordinate of at most one ordered pair belonging to \(r\) .

    Thus, a subset \(r\) of \(C \times  D\) is a \index{rule of assignment@Rule of assignment}rule of assignment if
    \[
    ...
    \end{aligned}
    \]
    Note that given a rule of assignment \(r\) , its \index{domain@Domain}domain and \index{image@Image}image are entirely determined.
\end{definition}

Now we can say what a function is.

\begin{definition}
    A function \(f\) is a rule of assignment \(r\) , together with a set \(B\) that \index{contains@Contains}contains the image set of \(r\) . The domain \(A\) of the rule \(r\) is also called the domain of the function \(f\) ; the image set of \(r\) is also called the image set of \(f\) ; and the set \(B\) is called the \index{range of a function@Range of a function} \index{range of a function@Range of a function}range of \(f{.}^{\dagger}\) .
\end{definition}
...
\begin{definition}
    ...
    \]
    On the other hand, if \({B}_{0}\) is a subset of \(B\) , we denote by \({f}^{-1}( {B}_{0})\) the set of all elements of \(A\) whose images under \(f\) lie in \({B}_{0}\) ; it is called the \index{Preimage} of \({B}_{0}\) under \(f\) (or the ``\index{Counterimage},'' or the ``\index{Inverse image},'' of \({B}_{0}\) ). Formally,
    \[
...
\begin{definition}
    A \index{Partition of a set} \(A\) is a collection of disjoint nonempty subsets of \(A\) whose union is all of \(A\) .
\end{definition}
...
A relation \(C\) on a set \(A\) is called an order relation (or a \index{Simple order}, or a \index{Linear order}) if it has the following properties:
...
\begin{centeredblock}
\begin{example}
    Consider the set \(\lbrack 0,1)\) of real numbers and the set \({\mathbb{Z}}_{ + }\) of \index{Positive integers}, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
    \[
    f( {n \times  t})  = n + t - 1
    \]
...
We leave it to the exercises to show that \(A\) has the \index{Least upper bound property!vs. greatest lower bound property}least upper bound property if and only if it has the greatest lower bound property.
...
\item Assume that the real line has the least upper bound property.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that the sets \[ \begin{aligned} [  {0,1}] &= \{ x \mid  0 \leq  x \leq  1\} , \\ \lbrack 0,1) &= \{ x \mid  0 \leq  x < 1\rbrack \end{aligned} \] have the least upper bound property.
...
Properties (7) and (8), on the other hand, are familiar properties in topology. They involve only the order relation; any set with an order relation satisfying (7) and (8) is called by topologists a \index{Linear continuum}.
...
(2) (\index{principle of induction@Principle of induction}). If \(A\) is an \index{Inductive set} of positive integers, then \(A =\)  \({Z}_{ + }\) .\\
We define the set \index{Z@\(\mathbb{Z}\)}\(\mathbb{Z}\) of integers to be the set consisting of the positive integers \index{Z^+@\({\mathbb{Z}}_{ + }\)}\({\mathbb{Z}}_{ + }\) , the number 0, and the negatives of the elements of \({\mathbb{Z}}_{ + }\) . One proves that the sum, difference, and product of two integers are integers, but the quotient is not necessarily an integer. The set \(\mathbb{Q}\) of quotients of integers is called the set of \index{Rational number}.
...
\begin{theorem}[\index{strong induction principle@Strong induction principle}]
...
Everything we have done up to now has used only the axioms for an ordered field, \index{\(\mathbb{R}\) (reals)!algebraic properties}\index{\(\mathbb{R}\) (reals)!order properties}properties (1)-(6) of the real numbers. At what point do you need (7), the least upper bound axiom?
...
\item Prove the following properties of \(\mathbb{Z}\) and \({\mathbb{Z}}_{ + }\) :
  ...
  \end{enumerate}
\item Let \(a \in  \mathbb{R}\) . Define inductively \[ \begin{aligned} {a}^{1} &= a, \\ {a}^{n + 1} &= {a}^{n} \cdot  a \end{aligned} \] for \(n \in  {\mathbb{Z}}_{ + }\) . (See \(§7\) for a discussion of the process of \index{Inductive definition}.) Show that for \(n,m \in  {\mathbb{Z}}_{ + }\) and \(a,b \in  \mathbb{R}\) , \[ \begin{aligned} {a}^{n}{a}^{m} &= {a}^{n + m}, \\ {( {a}^{n}) }^{m} &= {a}^{nm}, \\ {a}^{m}{b}^{m} &= {( ab) }^{m}. \end{aligned} \] These are called the laws of exponents [Hint: For fixed \(n\) , prove the formulas by induction on \(m\) .]
...
\begin{definition}
    Let \(\mathcal{A}\) be a nonempty collection of sets. An \index{Indexing function} for \(\mathcal{A}\) is a surjective function \(f\) from some set \(J\) , called the \index{Index set}, to \(\mathcal{A}\) . The collection \(\mathcal{A}\) , together with the indexing function \(f\) , is called an indexed \index{Family of sets}. Given \(\alpha  \in  J\) , we shall denote the set \(f( \alpha )\) by the symbol \({A}_{\alpha }\) . And we shall denote the indexed family itself by the symbol
    \[
...
\begin{definition}
    Let \(m\) be a positive integer. Given a set \(X\) , we define an \index{Coordinate: of J-tuple!of \(m\) -tuple}\index{m-tuple@\(m\)-tuple}\index{J-tuple@\(J\)-tuple}\(m\) -tuple of elements of \(X\) to be a function
    \[
...
\begin{definition}
    Given a set \(X\) , we define an \index{Coordinate: of J-tuple!of \(\omega\) -tuple}\(\omega\) -tuple of elements of \(X\) to be a function
    \[
    \mathbf{x} : {\mathbb{Z}}_{ + } \rightarrow  X
    \]
    we also call such a function a sequence, or an \index{Infinite sequence}, of elements of \(X\) . If \(\mathbf{x}\) is an \(\omega\) -tuple, we often denote the value of \(\mathbf{x}\) at \(i\) by \({x}_{i}\) rather than \(\mathbf{x}( i)\) , and call it the i th coordinate of \(x\) . We denote \(x\) itself by the symbol
    \[
...
\index{finite set@Finite Set} sets and \index{Infinite set}, \index{Countable set} and \index{Uncountable set}, these are types of sets that you may have encountered before. Nevertheless, we shall discuss them in this section and the next, not only to make sure you understand them thoroughly, but also to elucidate some particular points of logic that will arise later on. First we consider finite sets.
...
There is a very useful \index{Countability!via injective and surjective maps}criterion for showing that a set is countable. It is the following:
...
\begin{corollary}
    A \index{Countability!of subsets}subset of a countable set is countable.
\end{corollary}
...
\begin{centeredblock}
\begin{example}
    The set \({\mathbb{Q}}_{ + }\) of positive rational numbers is \index{Countability!of rationals}countably infinite. For we can define a surjection \(g \colon {\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{Q}}_{ + }\) by the equation
    \[
    g( {n,m})  = m/n
    \]
...
\item Let \(X\) be the two-element set \(\{ 0,1\}\) . Show there is a bijective correspondence between the set \index{Uncountability:!of \(\mathcal{P}({\mathbb{Z}}_{ + })\)}\(\mathcal{P}( {\mathbb{Z}}_{ + })\) and the cartesian product \({X}^{\omega }\) .
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item A real number \(x\) is said to be algebraic (over the rationals) if it satisfies some polynomial equation of positive degree \[ {x}^{n} + {a}_{n - 1}{x}^{n - 1} + \cdots  + {a}_{1}x + {a}_{0} = 0 \] with rational coefficients \({a}_{i}\) . Assuming that each polynomial equation has only finitely many roots, show that the set of \index{Algebraic number} is \index{Countability!of algebraic numbers}countable.
  \item A real number is said to be transcendental if it is not algebraic. Assuming the reals are uncountable, show that the \index{Uncountability:!of transcendental numbers}\index{Transcendental number} are uncountable. (It is a somewhat surprising fact that only two transcendental numbers are familiar to us: \(e\) and \(\pi\) . Even proving these two numbers transcendental is highly nontrivial.)
  \end{enumerate}
\item Determine, for each of the following sets, whether or not it is countable. Justify your answers.
  ...
  \item The set \(F\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) that are ``\index{Eventually zero}.'' [We say that \(f\) is eventually zero if there is a positive integer \(N\) such that \(f( n)  = 0\) for all \(n \geq  N\) ]
...
\item We say that two sets \(A\) and \(B\) \index{Have the same cardinality} if there is a bijection of \(A\) with \(B\) .
  ...
  \item Theorem (\index{schroeder-bernstein theorem@Schroeder-Bernstein theorem}). If there are injections \(f : A \rightarrow\)  \(C\) and \(g : C \rightarrow  A\) , then \(A\) and \(C\) have the same cardinality.
  \end{enumerate}
...
We have already obtained several criteria for a set to be infinite. We know, for instance, that a set \(A\) is \index{Infinite set!via injective and bijective functions}infinite if it has a countably infinite subset, or if there is a bijection of \(A\) with a proper subset of itself. It turns out that either of these properties is sufficient to characterize infinite sets. This we shall now prove. The proof will lead us into a discussion of a point of logic we have not yet mentioned-the \index{Axiom of choice}.
...
The axiom of choice certainly seems an innocent-enough assertion. And, in fact, most mathematicians today accept it as part of the set theory on which they base their mathematics. But in years past a good deal of controversy raged around this particular assertion concerning set theory, for there are theorems one can prove with its aid that some mathematicians were reluctant to accept. One such is the \index{well-ordering theorem@Well-ordering theorem!and axiom of choice}\index{Axiom of choice!vs. well-ordering theorem}well-ordering theorem, which we shall discuss shortly. For the present we shall simply use the choice axiom to clear up the difficulty we mentioned in the preceding proof. First, we prove an easy consequence of the axiom of choice:

\begin{lemma}[Existence of a \index{Choice function}]
...
\item Find if possible a choice function for each of the following collections, without using the choice axiom:
  ...
  \end{enumerate}
\item Suppose that \(A\) is a set and \({\{  {f}_{n}\}  }_{n \in  {\mathbb{Z}}_{ + }}\) is a given indexed family of \index{Finiteness: of cartesian products!vs. injective and surjective maps}injective functions \[ {f}_{n} : \{ 1,\ldots ,n\}  \rightarrow  A \] Show that \(A\) is infinite. Can you define an injective function \(f : {\mathbb{Z}}_{ + } \rightarrow  A\) without using the choice axiom?
...
\begin{centeredblock}
\begin{example}
    The set of integers is not well-ordered in the usual order, the subset consisting of the negative integers has no smallest element. Nor is the set of real numbers in the interval \(0 \leq  x \leq  1\) well-ordered; the subset consisting of those \(x\) for which \(0 < x < 1\) has no smallest element (although it has a \index{Least upper bound property!for well-ordered sets}greatest lower bound, of course).
\end{example}
\end{centeredblock}

There are several ways of constructing well-ordered sets. Two of them are the following:

(1) If \(A\) is a well-ordered set, then any \index{Well-ordered set!subsets well-ordered}subset of \(A\) is well-ordered in the restricted order relation.

(2) If \(A\) and \(B\) are well-ordered sets, then \(A \times  B\) is well-ordered in the \index{Well-ordered set!dictionary order}dictionary order.\\
...
Now, given a set \(A\) without an order relation, it is natural to ask whether there exists an order relation for \(A\) that makes it into a \index{Well-ordered set!finite}well-ordered set. If \(A\) is finite, any bijection
\[
f : A \rightarrow  \{ 1,\ldots ,n\}
\]
...
\begin{ucorollary}
    There exists an \index{Uncountable well-ordered set (see also \({S}_{\Omega }\))}uncountable well-ordered set.
\end{ucorollary}
...
\begin{proof}
    ...
    Note that \({S}_{\Omega }\) is an \index{\({S}_{\Omega }\)!existence}uncountable well-ordered set every section of which is \index{\({S}_{\Omega }\)!countable subsets}countable. Its order type is in fact \index{\({S}_{\Omega }\)!uniqueness}uniquely determined by this condition. We shall call it a minimal uncountable well-ordered set. Furthermore, we shall denote the well-ordered set \(A = {S}_{\Omega } \cup  \{ \Omega \}\) by the symbol \({\widetilde{S}}_{\Omega }\) (for reasons to be seen later).
...
\item Let \({S}_{\Omega }\) be the minimal uncountable well-ordered set.
  ...
  \end{enumerate}
\item Let \(J\) be a well-ordered set. A subset \({J}_{0}\) of \(J\) is said to be inductive if for every \(\alpha  \in  J,\) \[ ( {{S}_{\alpha } \subset  {J}_{0}})  \Rightarrow  \alpha  \in  {J}_{0} \] \textbf{Theorem (The principle of \index{Transfinite induction}).} If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Let \({A}_{1}\) and \({A}_{2}\) be \index{Disjoint sets}, well-ordered by \({ < }_{1}\) and \({ < }_{2}\) , respectively. Define an order relation on \({A}_{1} \cup  {A}_{2}\) by letting \(a < b\) either if \(a,b \in  {A}_{1}\) and \(a{ < }_{1}b\) , or if \(a,b \in  {A}_{2}\) and \(a{ < }_{2}b\) , or if \(a \in  {A}_{1}\) and \(b \in  {A}_{2}\) . Show that this is a well-ordering
...
\textsl{Proof}
  ...
  \item Show by \index{Induction principle!transfinite}transfinite induction that for every \(\beta  \in  J\) , there exists a function \({h}_{\beta } : {S}_{\beta } \rightarrow  C\) satisfying \(( *)\) . [Hint: If \(\beta\) has an immediate predecessor \(\alpha\) , then \({S}_{\beta } = {S}_{\alpha } \cup  \{ \alpha \}\) . If not, \({S}_{\beta }\) is the union of all \({S}_{\alpha }\) with \(\alpha  < \beta\) .]
...
It is also true that the \index{maximum principle@Maximum principle}maximum principle is an easy consequence of \index{Maximum principle!vs. Zorn's lemma}\index{zorn's lemma@Zorn's lemma!vs. maximum principle}Zorn's lemma. See Exercises 5-7.
...
\item Show that \index{zorn's lemma@Zorn's lemma}Zorn's lemma implies the following: \\ \textbf{Lemma (Kuratowski).} Let \(\mathcal{A}\) be a collection of sets. Suppose that for every subcollection \(\mathcal{B}\) of \(\mathcal{A}\) that is simply ordered by proper inclusion, the union of the elements of \(\mathcal{B}\) belongs to \(\mathcal{A}\) Then \(\mathcal{A}\) has an element that is properly contained in no other element of \(\mathcal{A}\) .
\item A collection \(\mathcal{A}\) of subsets of a set \(X\) is said to be of finite type provided that a subset \(B\) of \(X\) belongs to \(\mathcal{A}\) if and only if every finite subset of \(B\) belongs to \(\mathcal{A}\) . Show that the \index{kuratowski lemma@Kuratowski lemma} implies the following. \\ \textbf{Lemma (Tukey, 1940).} Let \(\mathcal{A}\) be a collection of sets. If \(\mathcal{A}\) is of finite type, then \(\mathcal{A}\) has an element that is properly contained in no other element of \(\mathcal{A}\) .
\item Show that the \index{tukey lemma@Tukey lemma} implies the \index{hausdorff maximum principle@Hausdorff maximum principle}. [Hint: If \(\prec\) is a strict partial order on \(A\) , let \(\mathcal{A}\) be the collection of all subsets of \(A\) that are simply ordered by \(\prec\) . Show that \(\mathcal{A}\) is of finite type.]
...
\item Let \(X\) be a set; let \(\mathcal{A}\) be the collection of all pairs \(( {A, < })\) , where \(A\) is a subset of \(X\) and \(<\) is a well-ordering of \(A\) . Define \[ ( {A, < })  \prec  ( {{A}^{\prime },{ < }^{\prime }}) \] if \(( {A, < })\) equals a section of \(( {{A}^{\prime },{ < }^{\prime }})\) .
  ...
  \end{enumerate}
\item Use Exercises 1 and 5 to prove the following: \\ \textsl{Theorem. The maximum principle is equivalent to the \index{well-ordering theorem@Well-ordering theorem!and maximum principle}well-ordering theorem.}
