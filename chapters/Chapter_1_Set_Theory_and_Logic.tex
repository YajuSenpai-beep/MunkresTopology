...
The \index{Equality symbol} \(=\) is used throughout this book to mean \index{logical equivalence@Logical equivalence} logical identity. Thus, when we write \(a = b\) , we mean that `` \(a\) `` and `` \(b\) `` are symbols for the \index{Cardinality: comparability!same}\index{Cardinality: comparability!same}\index{Cardinality: comparability!same}same object. This is what one means in arithmetic, for example, when one writes \(\displaystyle\frac{2}{4} = \frac{1}{2}\) . Similarly, the equation \(A = B\) states that `` \(A\) `` and `` \(B\) `` are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
\]
It is sometimes called the \index{Complement} of \(B\) relative to \(A\) , or the \index{complement@Complement}\index{complement@Complement}\index{complement@Complement}complement of \(B\) in \(A\) . Our three set operations are represented schematically in Figure 1.1.
...
\begin{definition}
    Let \(m\) be a positive integer. Given a set \(X\) , we define an \index{m-tuple@\(m\)-tuple}\index{J-tuple@\(J\)-tuple}\(m\) -tuple of elements of \(X\) to be a function
    \[
...
It is also true that the \index{maximum principle@Maximum principle}maximum principle is an easy consequence of \index{zorn's lemma@Zorn's lemma!vs. maximum principle}Zorn's lemma. See Exercises 5-7.
