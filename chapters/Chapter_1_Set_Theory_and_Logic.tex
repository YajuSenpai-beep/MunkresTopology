...
The \index{Equality symbol}equality symbol \(=\) is used throughout this book to mean \index{logical equivalence@Logical equivalence}logical identity logical identity. Thus, when we write \(a = b\) , we mean that `` \(a\) `` and `` \(b\) `` are symbols for the same object. This is what one means in arithmetic, for example, when one writes \(\displaystyle\frac{2}{4} = \frac{1}{2}\) . Similarly, the equation \(A = B\) states that `` \(A\) `` and `` \(B\) `` are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
We say that \(A\) is a \index{Subset}subset of \(B\) if every element of \(A\) is also an element of \(B\) ; and we express this fact by writing
\[
A \subset  B\text{ . }
\]
Nothing in this definition requires \(A\) to be different from \(B\) ; in fact, if \(A = B\) , it is true that both \(A \subset  B\) and \(B \subset  A\) . If \(A \subset  B\) and \(A\) is different from \(B\) , we say that \(A\) is a \index{Proper subset}proper subset of \(B\) , and we write
\[
...
The usual way to specify a set, however, is to take some set \(A\) of objects and some property that elements of \(A\) may or may not possess, and to form the set consisting of all elements of \(A\) having that property. For instance, one might take the set of real numbers and form the subset \(B\) consisting of all even \index{Integers}integers. In symbols, this statement becomes
\[
...
Given two sets \(A\) and \(B\) , one can form a set from them that consists of all the elements of \(A\) together with all the elements of \(B\) . This set is called the \index{Union}union of \(A\) and \(B\) and is denoted by \(A \cup  B\) . Formally, we define
\[
...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the intersection\index{section: of the positive integers@Section: of the positive integers}section of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
Again, mathematics cannot tolerate ambiguity, so a \index{choice axiom (see axiom of choice)@Choice axiom (see Axiom of choice)}choice axiom choice of meanings must be made. Mathematicians have agreed always to use ``if ... then'' in the first sense, so that a statement of the form ``If \(P\) , then \(Q\) `` means that if \(P\) is true, \(Q\) is true also, but if \(P\) is false, \(Q\) may be either true or false.

As an example, consider the following statement about real numbers:
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ . }
\]
It is a statement of the form, ``If \(P\) , then \(Q\) ,'' where \(P\) is the phrase `` \(x > 0\) '' (called the \index{Hypothesis}hypothesis of the statement) and \(Q\) is the phrase `` \({x}^{3} \neq  0\) '' (called the \index{Conclusion}conclusion of the statement). This is a true statement, for in every case for which the hypothesis \(x > 0\) holds, the conclusion \({x}^{3} \neq  0\) holds as well.
...
\section*{Negation}
If one wishes to form the contrapositive of the statement \(P \Rightarrow  Q\) , one has to know how to form the statement ``not \(P\) ,'' which is called the \index{Negation}negation of \(P\) . In many cases, this causes no difficulty; but sometimes confusion occurs with statements involving the phrases ``for every'' and ``for at least one.'' These phrases are called logical \index{quantifiers, logical@Quantifiers, logical}logical quantifiers quantifiers.
...
\]
Therefore, to form the negation of statement \(\left( *\right)\), one replaces the quantifier ``for every'' by the quantifier ``for at least one,'' and one replaces statement \(P\) by its negation.
...
We return now to our discussion of sets. There is one other \index{operation, binary@Operation, binary}binary operation operation on sets that is occasionally useful. It is the \index{Difference of two sets}difference of two sets, denoted by \(A - B\) , and defined as the set consisting of those elements of \(A\) that are not in \(B\) . Formally,
\[
A - B = \{ x \mid  x \in  A\text{ and }x \notin  B\} .
\]
It is sometimes called the \index{Complement}complement of \(B\) relative to \(A\) , or the complement of \(B\) in \(A\) . Our three set operations are represented schematically in Figure 1.1.
...
\]
holds. The equation is illustrated in Figure 1.3; the shaded region represents the set in question, as you can check mentally. This equation can be thought of as a ``\index{distributive laws for  and@Distributive laws for  and}distributive laws distributive law'' for the operations \(\cap\) and \(\cup\) .
...
Other examples of set-theoretic rules include the \index{second coordinate of ordered pair@Second coordinate of ordered pair}Second coordinate of ordered pair second ``distributive law,''
\[
...
The notion of ordered pair carries over to general sets. Given sets \(A\) and \(B\) , we define their \index{cartesian product: countably infinite@Cartesian product: countably infinite}cartesian product cartesian product \(A \times  B\) to be the set of all ordered pairs(a, b)for which \(a\) is an element of \(A\) and \(b\) is an element of \(B\) . Formally,
...
Let us make a comment on notation. It is an unfortunate fact that the notation(a, b) is firmly established in mathematics with two entirely different meanings. One meaning, as an ordered pair of objects, we have just discussed. The other meaning is the one you are familiar with from analysis; if \(a\) and \(b\) are real numbers, the symbol(a, b) is used to denote the \index{Interval}interval consisting of all numbers \(x\) such that \(a < x < b\) . Most of the time, this conflict in notation will cause no difficulty because the meaning will be clear from the context. Whenever a situation occurs where confusion is possible, we shall adopt a different notation for the ordered pair(a, b), denoting it by the symbol
\[
a \times b
\]
instead.
...
\item Determine which of the following statements are true for all sets \(A,B,C\) , and \(D\) . If a double implication fails, determine whether one or the other of the possible implications holds. If an equality fails, determine whether the statement becomes true if the ``equals'' symbol is replaced by one or the other of the inclusion symbols \(\subset\) or \(\supset\) .
  ...
  \item \(A \subset  B\) and \(A \subset  C \Leftrightarrow  A \subset  \left( {B \cup  C}\right)\) .
  \item \(A \subset  B\) or \(A \subset  C \Leftrightarrow  A \subset  \left( {B \cup  C}\right)\) .
  \item \(A \subset  B\) and \(A \subset  C \Leftrightarrow  A \subset  \left( {B \cap  C}\right)\) .
  \item \(A \subset  B\) or \(A \subset  C \Leftrightarrow  A \subset  \left( {B \cap  C}\right)\) .
  \item \(A - \left( {A - B}\right)  = B\) .
  \item \(A - \left( {B - A}\right)  = A - B\) .
  \item \(A \cap  \left( {B - C}\right)  = \left( {A \cap  B}\right)  - \left( {A \cap  C}\right)\) .
  \item \(A \cup  \left( {B - C}\right)  = \left( {A \cup  B}\right)  - \left( {A \cup  C}\right)\) .
  \item \(\left( {A \cap  B}\right)  \cup  \left( {A - B}\right)  = A\) .
  \item \(A \subset  C\) and \(B \subset  D \Rightarrow  \left( {A \times  B}\right)  \subset  \left( {C \times  D}\right)\) .
  \item The converse of (j).
  \item The converse of(j), assuming that \(A\) and \(B\) are nonempty.
  \item \(\left( {A \times  B}\right)  \cup  \left( {C \times  D}\right)  = \left( {A \cup  C}\right)  \times  \left( {B \cup  D}\right)\) .
  \item \(\left( {A \times  B}\right)  \cap  \left( {C \times  D}\right)  = \left( {A \cap  C}\right)  \times  \left( {B \cap  D}\right)\) .
  \item \(A \times  \left( {B - C}\right)  = \left( {A \times  B}\right)  - \left( {A \times  C}\right)\) .
  \item \(\left( {A - B}\right)  \times  \left( {C - D}\right)  = \left( {A \times  C - B \times  C}\right)  - A \times  D\) .
  \item \(\left( {A \times  B}\right)  - \left( {C \times  D}\right)  = \left( {A - C}\right)  \times  \left( {B - D}\right)\) .
  \end{enumerate}
...
\item If a set \(A\) has two elements, show that \(\mathcal{P}\left( A\right)\) has four elements. How many elements does \(\mathcal{P}\left( A\right)\) have if \(A\) has one element? Three elements? No elements? Why is \(\mathcal{P}\left( A\right)\) called the power set of \(A\) ?
...
\item Let \(\mathbb{R}\) denote the set of real numbers. For each of the following subsets of \(\mathbb{R} \times  \mathbb{R}\) , determine whether it is equal to the cartesian product of two subsets of \(\mathbb{R}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \(\{ \left( {x,y}\right)  \mid  x\) is an integer \(\}\) .
  \item \(\{ \left( {x,y}\right)  \mid  0 < y \leq  1\}\) .
  \item \(\{ \left( {x,y}\right)  \mid  y > x\}\) .
  \item \(\{ \left( {x,y}\right)  \mid  x\) is not an integer and \(y\) is an integer \(\}\) .
  \item \(\left\{  {\left( {x,y}\right)  \mid  {x}^{2} + {y}^{2} < 1}\right\}\) .
  \end{enumerate}
\end{enumerate}
...
\begin{definition} 
    A \index{Rule of assignment}rule of assignment is a subset \(r\) of the cartesian product \(\mathbf{C} \times  \mathbf{D}\) of two sets, having the property that each element of \(C\) appears as the first coordinate of at most one ordered pair belonging to \(r\) .
    ...
    Given a rule of assignment \(r\) , the \index{Domain}domain of \(\mathrm{r}\) is defined to be the subset of \(C\) consisting of all first coordinates of elements of \(r\) , and the \index{Image}image set of \(r\) is defined as the subset of \(D\) consisting of all second coordinates of elements of \(r\) . Formally,
...
f : A \rightarrow  B
\]
which is read `` \(f\) is a function from \(A\) to \(B\) ,'' or `` \(f\) is a \index{Mapping}mapping from \(A\) into \(B\) ,'' or simply `` \(f\) maps \(A\) into \(B\) .'' One sometimes visualizes \(f\) as a geometric transformation physically carrying the points of \(A\) to points of \(B\) .

If \(f : A \rightarrow  B\) and if \(a\) is an element of \(A\) , we denote by \(f\left( a\right)\) the unique element of \(B\) that the rule determining \(f\) assigns to \(a\) ; it is called the \index{value of a function@Value of a function}Value of a function value of \(f\) at \(a\) , or sometimes the image of \(a\) under \(f\) . Formally, if \(r\) is the rule of the function \(f\) , then \(f\left( a\right)\) denotes the unique element of \(B\) such that \(\left( {a,f\left( a\right) }\right)  \in  r\) .
...
\begin{example}
    ...
    \[
    \begin{aligned}
    f &: \mathbb{R} \rightarrow  \mathbb{R}\; &&\text{defined by }\; f\left( x\right)  = {x}^{2}, \\
    g &: {\overline{\mathbb{R}}}_{ + } \rightarrow  \mathbb{R}\; &&\text{defined by }\; g\left( x\right)  = {x}^{2}, \\
    h &: \mathbb{R} \rightarrow  {\overline{\mathbb{R}}}_{ + }\; &&\text{defined by }\; h\left( x\right)  = {x}^{2}, \\
    k &: {\overline{\mathbb{R}}}_{ + } \rightarrow  {\overline{\mathbb{R}}}_{ + }\; &&\text{defined by }\; k\left( x\right)  = {x}^{2}.
    \end{aligned}
    \]
...
Restricting the domain of a function and changing its range are two ways of forming a new function from an old one. Another way is to form the \index{composite: of functions@Composite: of functions}composite composite of two functions.
...
Injectivity of \(f\) depends only on the rule of \(f\) ; surjectivity depends on the range of \(f\) as well. You can check that the composite of two \index{Injective function}injective functions is injective, and the composite of two \index{Surjective function}surjective functions is surjective; it follows that the composite of two \index{Bijective function}bijective functions is bijective.

If \(f\) is bijective, there exists a function from \(B\) to \(A\) called the \index{inverse function@Inverse function}inverse of \(f\) . It is denoted by \({f}^{-1}\) and is defined by letting \({f}^{-1}\left( b\right)\) be that unique element \(a\) of \(A\) for which \(f\left( a\right)  = b\) . Given \(b \in  B\) , the fact that \(f\) is surjective implies that there exists such an element \(a \in  A\) ; the fact that \(f\) is injective implies that there is only one such element \(a\) . It is easy to see that if \(f\) is bijective, \({f}^{-1}\) is also bijective.
...
\begin{lemma}
    Let \(f : A \rightarrow  B\) . If there are functions \(g : B \rightarrow  A\) and \(h : B \rightarrow  A\) such that \(g\left( {f\left( a\right) }\right)  = a\) for every \(a\) in \(A\) and \(f\left( {h\left( b\right) }\right)  = b\) for every \(b\) in \(B\) , then \(f\) is bijective and \(g = h = {f}^{-1}\) .
\end{lemma}

\begin{definition}
    Let \(f : A \rightarrow  B\) . If \({A}_{0}\) is a subset of \(A\) , we denote by \(f\left( {A}_{0}\right)\) the set of all images of points of \({A}_{0}\) under the function \(f\) ; this set is called the image of \({A}_{0}\) under \(f\) . Formally,
    \[
    f\left( {A}_{0}\right)  = \left\{  {b \mid  b = f\left( a\right) \text{ for at least one }a \in  {A}_{0}}\right\}  .
    \]
    On the other hand, if \({B}_{0}\) is a subset of \(B\) , we denote by \({f}^{-1}\left( {B}_{0}\right)\) the set of all elements of \(A\) whose images under \(f\) lie in \({B}_{0}\) ; it is called the \index{Preimage}preimage of \({B}_{0}\) under \(f\) (or the ``\index{Counterimage}counterimage,'' or the ``\index{Inverse image}inverse image,'' of \({B}_{0}\) ). Formally,
    \[
...
\begin{centeredblock}
\begin{example}
    Consider the function \(f : \mathbb{R} \rightarrow  \mathbb{R}\) given by \(f\left( x\right)  = 3{x}^{2} + 2\) (Figure 2.3). Let \(\left\lbrack  {a,b}\right\rbrack\) denote the closed interval \(a \leq  x \leq  b\) . Then
    \[
    \begin{aligned}
...
\item Let \(f : A \rightarrow  B\) . Let \({A}_{0} \subset  A\) and \({B}_{0} \subset  B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \({A}_{0} \subset  {f}^{-1}\left( {f\left( {A}_{0}\right) }\right)\) and that equality holds if \(f\) is injective.
  \item Show that \(f\left( {{f}^{-1}\left( {B}_{0}\right) }\right)  \subset  {B}_{0}\) and that equality holds if \(f\) is surjective.
  \end{enumerate}
\item Let \(f : A \rightarrow  B\) and let \({A}_{i} \subset  A\) and \({B}_{i} \subset  B\) for \(i = 0\) and \(i = 1\) . Show that \({f}^{ - }\) preserves inclusions, unions, intersections, and differences of sets:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \({B}_{0} \subset  {B}_{1} \Rightarrow  {f}^{-1}\left( {B}_{0}\right)  \subset  {f}^{-1}\left( {B}_{1}\right)\) .
  \item \({f}^{-1}\left( {{B}_{0} \cup  {B}_{1}}\right)  = {f}^{-1}\left( {B}_{0}\right)  \cup  {f}^{-1}\left( {B}_{1}\right)\) .
  \item \({f}^{-1}\left( {{B}_{0} \cap  {B}_{1}}\right)  = {f}^{-1}\left( {B}_{0}\right)  \cap  {f}^{-1}\left( {B}_{1}\right)\) .
  \item \({f}^{-1}\left( {{B}_{0} - {B}_{1}}\right)  = {f}^{-1}\left( {B}_{0}\right)  - {f}^{-1}\left( {B}_{1}\right)\) . 
\end{enumerate}
	  Show that \(f\) preserves inclusions and unions only:
	  \begin{enumerate}[start=5, itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \({A}_{0} \subset  {A}_{1} \Rightarrow  f\left( {A}_{0}\right)  \subset  f\left( {A}_{1}\right)\) .
  \item \(f\left( {{A}_{0} \cup  {A}_{1}}\right)  = f\left( {A}_{0}\right)  \cup  f\left( {A}_{1}\right)\) .
  \item \(f\left( {{A}_{0} \cap  {A}_{1}}\right)  \subset  f\left( {A}_{0}\right)  \cap  f\left( {A}_{1}\right)\) ; show that equality holds if \(f\) is injective.
  \item \(f\left( {{A}_{0} - {A}_{1}}\right)  \supset  f\left( {A}_{0}\right)  - f\left( {A}_{1}\right)\) ; show that equality holds if \(f\) is injective.
  \end{enumerate}
...
\item Let \(f : A \rightarrow  B\) and \(g : B \rightarrow  C\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \({C}_{0} \subset  C\) , show that \({\left( g \circ  f\right) }^{-1}\left( {C}_{0}\right)  = {f}^{-1}\left( {{g}^{-1}\left( {C}_{0}\right) }\right)\) .
  ...
  \end{enumerate}
\item In general, let us denote the \index{Identity function}identity function for a set \(C\) by \({i}_{C}\) . That is, define \({i}_{C} : C \rightarrow  C\) to be the function given by the rule \({i}_{C}\left( x\right)  = x\) for all \(x \in  C\) . Given \(f : A \rightarrow  B\) , we say that a function \(g : B \rightarrow  A\) is a \index{Left inverse}left inverse for \(f\) if \(g \circ  f = {i}_{A}\) ; and we say that \(h : B \rightarrow  A\) is a \index{Right inverse}right inverse for \(f\) if \(f \circ  h = {i}_{B}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Let \(f : \mathbb{R} \rightarrow  \mathbb{R}\) be the function \(f\left( x\right)  = {x}^{3} - x\) . By restricting the domain and range of \(f\) appropriately, obtain from \(f\) a bijective function \(g\) . Draw the graphs of \(g\) and \({g}^{-1}\) . (There are several possible choices for \(g\) .)
\end{enumerate}
\section*{§ 3 Relations}
...
Given an equivalence relation \(\sim\) on a set \(A\) and an element \(x\) of \(A\) , we define a certain subset \(E\) of \(A\) , called the \index{Equivalence class}equivalence class determined by \(x\) , by the equation
\[
E = \{ y \mid  y \sim  x\} .
\]
...
(1) (\index{comparability: of cardinalities@Comparability: of cardinalities}comparability Comparability) For every \(x\) and \(y\) in \(A\) for which \(x \neq  y\) , either \({xCy}\) or \({yCx}\) .
...
\begin{definition}
    ...
    \{ x \mid  a < x < b\} \text{ ; }
    \]
    it is called an \index{Open interval}open interval in \(X\) . If this set is empty, we call a the \index{Immediate predecessor}immediate predecessor of \(b\) , and we call \(b\) the \index{Immediate successor}immediate successor of \(a\) .
\end{definition}


\begin{definition}
    Suppose that \(A\) and \(B\) are two sets with order relations \({ < }_{A}\) and \({ < }_{B}\) respectively. We say that \(A\) and \(B\) have the same \index{Order type}order type if there is a bijective correspondence between them that preserves order; that is, if there exists a bijective function \(f : A \rightarrow  B\) such that
    \[
...
\begin{centeredblock}
\begin{example}
    The interval(-1,1)of real numbers has the same order type as the set \(\mathbb{R}\) of real numbers itself, for the function \(f : \left( {-1,1}\right)  \rightarrow  \mathbb{R}\) given by
    \[
...
\begin{centeredblock}
\begin{example}
    The subset \(A = \{ 0\}  \cup  \left( {1,2}\right)\) of \(\mathbb{R}\) has the same order type as the subset
    \[
...
\begin{definition}
    ...
    \]
    if \({a}_{1}{ < }_{A}{a}_{2}\) , or if \({a}_{1} = {a}_{2}\) and \({b}_{1}{ < }_{B}{b}_{2}\) . It is called the \index{Dictionary order}dictionary order relation on \(A \times  B\) .
\end{definition}
...
One of the properties of the real numbers that you may have seen before is the “least \index{Upper bound}upper bound property.” One can define this property for an arbitrary ordered set. First, we need some preliminary definitions.

Suppose that \(A\) is a set ordered by the relation \(<\) . Let \({A}_{0}\) be a subset of \(A\) . We say that the element \(b\) is the \index{Largest element}largest element of \({A}_{0}\) if \(b \in  {A}_{0}\) and if \(x \leq  b\) for every \(x \in  {A}_{0}\) . Similarly, we say that \(a\) is the \index{smallest element of ordered set@Smallest element of ordered set}Smallest element of ordered set smallest element of \({A}_{0}\) if \(a \in  {A}_{0}\) and if \(a \leq  x\) for every \(x \in  {A}_{0}\) . It is easy to see that a set has at most one largest element and at most one smallest element.

We say that the subset \({A}_{0}\) of \(A\) is \index{Bounded above}bounded above if there is an element \(b\) of \(A\) such that \(x \leq  b\) for every \(x \in  {A}_{0}\) ; the element \(b\) is called an upper bound for \({A}_{0}\) . If the set of all upper bounds for \({A}_{0}\) has a smallest element, that element is called the least upper bound, or the \index{Supremum}supremum, of \({A}_{0}\) . It is denoted by sup \({A}_{0}\) ; it may or may not belong to \({A}_{0}\) . If it does, it is the largest element of \({A}_{0}\) .

Similarly, \({A}_{0}\) is \index{Bounded below}bounded below if there is an element \(a\) of \(A\) such that \(a \leq  x\) for every \(x \in  {A}_{0}\) ; the element \(a\) is called a \index{Lower bound}lower bound for \({A}_{0}\) . If the set of all lower bounds for \({A}_{0}\) has a largest element, that element is called the \index{Greatest lower bound}greatest lower bound, or the \index{Infimum}infimum, of \({A}_{0}\) . It is denoted by inf \({A}_{0}\) , it may or may not belong to \({A}_{0}\) . If it does, it is the smallest element of \({A}_{0}\) .
...
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item Define two points \(\left( {{x}_{0},{y}_{0}}\right)\) and \(\left( {{x}_{1},{y}_{1}}\right)\) of the plane to be equivalent if \({y}_{0} - {x}_{0}^{2} =\)  \({y}_{1} - {x}_{1}^{2}\) . Check that this is an equivalence relation and describe the equivalence classes.
\item Let \(C\) be a relation on a set \(A\) . If \({A}_{0} \subset  A\) , define the restriction of \(C\) to \({A}_{0}\) to be the relation \(C \cap  \left( {{A}_{0} \times  {A}_{0}}\right)\) . Show that the restriction of an equivalence relation is an equivalence relation.
...
\item Let \(f : A \rightarrow  B\) be a surjective function. Let us define a relation on \(A\) by setting \({a}_{0} \sim  {a}_{1}\) if \[ f\left( {a}_{0}\right)  = f\left( {a}_{1}\right) . \]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Let \(S\) and \({S}^{\prime }\) be the following subsets of the plane. \[ \begin{aligned} S &= \{ \left( {x,y}\right)  \mid  y = x + 1\text{ and }0 < x < 2\} , \\ {S}^{\prime } &= \{ \left( {x,y}\right)  \mid  y - x\text{ is an integer }\} . \end{aligned} \]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Define a relation on the plane by setting \[ \left( {{x}_{0},{y}_{0}}\right)  < \left( {{x}_{1},{y}_{1}}\right) \] if either \({y}_{0} - {x}_{0}^{2} < {y}_{1} - {x}_{1}^{2}\) , or \({y}_{0} - {x}_{0}^{2} = {y}_{1} - {x}_{1}^{2}\) and \({x}_{0} < {x}_{1}\) . Show that this is an order relation on the plane, and describe it geometrically.
...
\item
  ...
  \item Show that the equation \(g\left( y\right)  = {2y}/\left\lbrack  {1 + {\left( 1 + 4{y}^{2}\right) }^{1/2}}\right\rbrack\) defines a function \(g : \mathbb{R} \rightarrow  \left( {-1,1}\right)\) that is both a left and a right inverse for \(f\) .
  \end{enumerate}
...
\item Let \({\mathbb{Z}}_{ + }\) denote the set of positive integers. Consider the following order relations on \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) :
  ...
  \item \(\left( {{x}_{0},{y}_{0}}\right)  < \left( {{x}_{1},{y}_{1}}\right)\) if either \({x}_{0} - {y}_{0} < {x}_{1} - {y}_{1}\) , or \({x}_{0} - {y}_{0} = {x}_{1} - {y}_{1}\) and \({y}_{0} < {y}_{1}\) .
  \item \(\left( {{x}_{0},{y}_{0}}\right)  < \left( {{x}_{1},{y}_{1}}\right)\) if either \({x}_{0} + {y}_{0} < {x}_{1} + {y}_{1}\) , or \({x}_{0} + {y}_{0} = {x}_{1} + {y}_{1}\) and \({y}_{0} < {y}_{1}\) .
  \end{enumerate}
...
\item If \(C\) is a relation on a set \(A\) , define a new relation \(D\) on \(A\) by letting \(\left( {b,a}\right)  \in  D\) if \(\left( {a,b}\right)  \in  C\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Assume that the real line has the least upper bound property.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that the sets \[ \begin{aligned} \left\lbrack  {0,1}\right\rbrack &= \{ x \mid  0 \leq  x \leq  1\} , \\ \lbrack 0,1) &= \{ x \mid  0 \leq  x < 1\rbrack \end{aligned} \] have the least upper bound property.
  \item Does \(\left\lbrack  {0,1}\right\rbrack \times  \left\lbrack  {0,1}\right\rbrack\) in the dictionary order have the least upper bound property? What about \(\left\lbrack  {0,1}\right\rbrack \times  \lbrack 0,1)\) ? What about \(\lbrack 0,1) \times  \left\lbrack  {0,1}\right\rbrack\) ?
  \end{enumerate}
\end{enumerate}
...
(2) (\index{principle of induction@Principle of induction}Principle of induction). If \(A\) is an \index{Inductive set}inductive set of positive integers, then \(A =\)  \({Z}_{ + }\) .\\
...
Now we prove two properties of the positive integers that may not be quite so familiar, but are quite useful. They may be thought of as alternative versions of the \index{Induction principle}induction principle.
...
\item Prove the following ``laws of algebra'' for \(\mathbb{R}\) , using only axioms (1)-(5)
  ...
  \item \(0 \cdot  x = 0\) . [Hint: Compute \(\left( {x + 0}\right)  \cdot  x\) .]
  \item \(- 0 = 0\) .
  \item \(- \left( {-x}\right)  = x\) .
  \item \(x\left( {-y}\right)  =  - \left( {xy}\right)  = \left( {-x}\right) y\) .
  \item \(\left( {-1}\right) x =  - x\) .
  \item \(x\left( {y - z}\right)  = {xy} - {xz}\) .
  \item \(- \left( {x + y}\right)  =  - x - y; - \left( {x - y}\right)  =  - x + y\) .
  \item If \(x \neq  0\) and \(x \cdot  y = x\) , then \(y = 1\) .
  \item \(x/x = 1\) if \(x \neq  0\) .
  \item \(x/1 = x\) .
  \item \(x \neq  0\) and \(y \neq  0 \Rightarrow  {xy} \neq  0\) .
  \item \(\left( {1/y}\right) \left( {1/z}\right)  = 1/\left( {yz}\right)\) if \(y,z \neq  0\) .
  \item \(\left( {x/y}\right) \left( {w/z}\right)  = \left( {xw}\right) /\left( {yz}\right)\) if \(y,z \neq  0\) .
  \item \(\left( {x/y}\right)  + \left( {w/z}\right)  = \left( {{xz} + {wy}}\right) /\left( {yz}\right)\) if \(y,z \neq  0\) .
  \item \(x \neq  0 \Rightarrow  1/x \neq  0\) .
  \item \(1/\left( {w/z}\right)  = z/w\) if \(w,z \neq  0\) .
  \item \(\left( {x/y}\right) /\left( {w/z}\right)  = \left( {xz}\right) /\left( {yw}\right)\) if \(y,w,z \neq  0\) .
  \item \(\left( {ax}\right) /y = a\left( {x/y}\right)\) if \(y \neq  0\) .
  \item \(\left( {-x}\right) /y = x/\left( {-y}\right)  =  - \left( {x/y}\right)\) if \(y \neq  0\) .
  \end{enumerate}
\item Prove the following ``laws of inequalities'' for \(\mathbb{R}\) , using axioms (1)-(6) along with the results of Exercise 1:
  ...
  \item \(x < y \Rightarrow  x < \left( {x + y}\right) /2 < y\) .
  \end{enumerate}
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Prove the following properties of \(\mathbb{Z}\) and \({\mathbb{Z}}_{ + }\) :
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \(a,b \in  {\mathbb{Z}}_{ + } \Rightarrow  a + b \in  {\mathbb{Z}}_{ + }\) . [Hint: Show that given \(a \in  {\mathbb{Z}}_{ + }\) , the set \(X = \left\{  {x \mid  x \in  \mathbb{R}\text{ and }a + x \in  {\mathbb{Z}}_{ + }}\right\}\) is inductive.
  ...
  \end{enumerate}
\item Let \(a \in  \mathbb{R}\) . Define inductively \[ \begin{aligned} {a}^{1} &= a, \\ {a}^{n + 1} &= {a}^{n} \cdot  a \end{aligned} \] for \(n \in  {\mathbb{Z}}_{ + }\) . (See \(§7\) for a discussion of the process of \index{Inductive definition}inductive definition.) Show that for \(n,m \in  {\mathbb{Z}}_{ + }\) and \(a,b \in  \mathbb{R}\) , \[ \begin{aligned} {a}^{n}{a}^{m} &= {a}^{n + m}, \\ {\left( {a}^{n}\right) }^{m} &= {a}^{nm}, \\ {a}^{m}{b}^{m} &= {\left( ab\right) }^{m}. \end{aligned} \] These are called the laws of exponents [Hint: For fixed \(n\) , prove the formulas by induction on \(m\) .]
...
\item
  ...
  \item Show that given \(a\) with \(0 < a < 1\) , inf \(\left\{  {{a}^{n} \mid  n \in  {\mathbb{Z}}_{ + }}\right\} = 0\) . [Hint: Let \(h = \left( {1 - a}\right) /a\) , and show that \({\left( 1 + h\right) }^{n} \geq  1 + {nh}\) .]
  \end{enumerate}
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Show that every positive number \(a\) has exactly one positive square root, as follows:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that if \(x > 0\) and \(0 \leq  h < 1\) , then \[ \begin{aligned} {\left( x + h\right) }^{2} &\leq  {x}^{2} + h\left( {{2x} + 1}\right) , \\ {\left( x - h\right) }^{2} &\geq  {x}^{2} - h\left( {2x}\right) \end{aligned} \]
  \item Let \(x > 0\) . Show that if \({x}^{2} < a\) , then \({\left( x + h\right) }^{2} < a\) for some \(h > 0\) ; and if \({x}^{2} > a\) , then \({\left( x - h\right) }^{2} > a\) for some \(h > 0\) .
...
\item Given \(m \in  \mathbb{Z}\) , we say that \(m\) is even if \(m/2 \in  \mathbb{Z}\) , and \(m\) is odd otherwise.
  ...
  \item Show that if \(a > 0\) is rational, then \(a = m/n\) for some \(m,n \in  {\mathbb{Z}}_{ + }\) where not both \(m\) and \(n\) are even. [Hint: Let \(n\) be the smallest element of the set \(\left\{  {x \mid  x \in  {\mathbb{Z}}_{ + }\text{ and }x \cdot  a \in  {\mathbb{Z}}_{ + }}\right\}\) .
...
\begin{definition}
    Let \(\mathcal{A}\) be a nonempty collection of sets. An \index{Indexing function}indexing function for \(\mathcal{A}\) is a surjective function \(f\) from some set \(J\) , called the \index{Index set}index set, to \(\mathcal{A}\) . The collection \(\mathcal{A}\) , together with the indexing function \(f\) , is called an indexed \index{Family of sets}family of sets. Given \(\alpha  \in  J\) , we shall denote the set \(f\left( \alpha \right)\) by the symbol \({A}_{\alpha }\) . And we shall denote the indexed family itself by the symbol
    \[
...
\begin{centeredblock}
\begin{example}
    How does the cartesian product \(A \times  B \times  C\) differ from the cartesian products \(A \times  \left( {B \times  C}\right)\) and \(\left( {A \times  B}\right)  \times  C\)? Very little. There are obvious bijective correspondences between these sets, indicated as follows
    \[
...
\begin{definition}
    ...
    \]
    we also call such a function a sequence, or an \index{Infinite sequence}infinite sequence, of elements of \(X\) . If \(\mathbf{x}\) is an \(\omega\) -tuple, we often denote the value of \(\mathbf{x}\) at \(i\) by \({x}_{i}\) rather than \(\mathbf{x}\left( i\right)\) , and call it the i th coordinate of \(x\) . We denote \(x\) itself by the symbol
    \[
...
\item
  ...
  \item Given the indexed family \(\left\{  {{A}_{1},{A}_{2},\ldots }\right\}\) , let \({B}_{i} = {A}_{2i - 1} \times  {A}_{2i}\) for each positive integer \(i\) . Show there is bijective correspondence of \({A}_{1} \times  {A}_{2} \times  \cdots\) with \({B}_{1} \times  {B}_{2} \times  \cdots\)
  \end{enumerate}
...
\item Let \(m,n \in  {\mathbb{Z}}_{ + }\) . Let \(X \neq  \varnothing\) .
  ...
  \item If \(A \subset  B\) , find an injective map \(m : {\left( {A}^{\omega }\right) }^{n} \rightarrow  {B}^{\omega }\) .
  \end{enumerate}
\item Which of the following subsets of \({\mathbb{R}}^{\omega }\) can be expressed as the cartesian product of subsets of \(\mathbb{R}\) ?
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item \(\left\{  {\mathbf{x} \mid  {x}_{i}\text{ is an integer for all }i}\right\}\) .
  \item \(\left\{  {\mathbf{x} \mid  {x}_{i} \geq  i\text{ for all }i}\right\}\) .
  \item \(\left\{  {\mathbf{x} \mid  {x}_{i}\text{ is an integer for all }i \geq  {100}}\right\}\) .
  \item \(\left\{  {\mathbf{x} \mid  {x}_{2} = {x}_{3}}\right\}\) .
  \end{enumerate}
\end{enumerate}

\section*{§ 6 Finite Sets}
\index{finite set@Finite Set}Finite Set sets and \index{Infinite set}infinite sets, \index{Countable set}countable sets and \index{Uncountable set}uncountable sets, these are types of sets that you may have encountered before. Nevertheless, we shall discuss them in this section and the next, not only to make sure you understand them thoroughly, but also to elucidate some particular points of logic that will arise later on. First we consider finite sets.
...
\begin{definition}
    ...
    \]
    for some positive integer \(n\) . In the former case, we say that \(A\) has \index{cardinality: comparability@Cardinality: comparability}cardinality cardinality 0; in the latter case, we say that \(A\) has cardinality \(n\) .
\end{definition}
...
\begin{lemma}
    Let \(n\) be a positive integer. Let \(A\) be a set; let \({a}_{0}\) be an element of \(A\) . Then there exists a bijective correspondence \(f\) of the set \(A\) with the set \(\{ 1,\ldots ,n + 1\}\) if and only if there exists a bijective correspondence \(g\) of the set \(A - \left\{  {a}_{0}\right\}\) with the set \(\{ 1,\ldots ,n\}\) .
\end{lemma}
...
From this lemma a number of useful con\index{Sequences}sequences follow:
...
\begin{proof}
    The function \(f : {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{Z}}_{ + } - \{ 1\}\) defined by \(f\left( n\right)  = n + 1\) is a bijection of \({\mathbb{Z}}_{ + }\) with a proper subset of itself.
\end{proof}
...
\begin{proof}
    ...
    then the sets \({f}^{-1}\left( {\{ b\} }\right)\) and \({f}^{-1}\left( \left\{  {b}^{\prime }\right\}  \right)\) are disjoint, so their smallest elements must be different.
...
\begin{proof}
    We first show that if \(A\) and \(B\) are finite, so is \(A \cup  B\) . The result is trivial if \(A\) or \(B\) is empty. Otherwise, there are bijections \(f : \{ 1,\ldots ,m\}  \rightarrow  A\) and \(g\) : \(\{ 1,\ldots ,n\}  \rightarrow  B\) for some choice of \(m\) and \(n\) . Define a function \(h.\{ 1,\ldots ,m +\)  \(n\}  \rightarrow  A \cup  B\) by setting \(h\left( i\right)  = f\left( i\right)\) for \(i = 1,2,\ldots ,m\) and \(h\left( i\right)  = g\left( {i - m}\right)\) for \(i = m + 1,\ldots ,m + n\) . It is easy to check that \(h\) is surjective, from which it follows that \(A \cup  B\) is finite.

    Now we show by induction that \index{finiteness: of cartesian products@Finiteness: of cartesian products}finiteness finiteness of the sets \({A}_{1},\ldots ,{A}_{n}\) implies finiteness of their union. This result is trivial for \(n = 1\). Assuming it true for \(n - 1\) , we note that \({A}_{1} \cup  \cdots  \cup  {A}_{n}\) is the union of the two finite sets \({A}_{1} \cup  \cdots  \cup  {A}_{n - 1}\) and \({A}_{n}\) , so the result of the preceding paragraph applies.
...
\begin{proof}
    We define a bijection \(h : {\mathbb{Z}}_{ + } \rightarrow  C\) . We proceed by induction. Define \(h\left( 1\right)\) to be the smallest element of \(C\) ; it exists because every nonempty subset \(C\) of \({\mathbb{Z}}_{ + }\) has a smallest element. Then assuming that \(h\left( 1\right) ,\ldots ,h\left( {n - 1}\right)\) are defined, define
    \[
...
What is needed is another principle, which we call the principle of \index{recursive definition, principle@Recursive definition, principle}principle of recursive definition recursive definition. In the proof of the preceding theorem, we wished to assert the following:
...
\]
for example, is self-contradictory; although \(h\left( i\right)\) necessarily is an element of the set \(h\left( {\{ 1,\ldots ,i + 1\} }\right)\) , this formula says that it does not belong to the set. Another example is the classic \index{russell's paradox@Russell's paradox}Russell's paradox paradox:

\begin{centeredblock}
    Let the \index{barber of seville paradox@Barber of Seville paradox}barber of Seville barber of Seville shave every man of Seville who does not shave himself. Who shall shave the barber?
\end{centeredblock}
...
\begin{centeredblock}
\begin{example}
    The set \({\mathbb{Q}}_{ + }\) of positive rational numbers is countably infinite. For we can define a surjection \(g \cdot  {\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{Q}}_{ + }\) by the equation
    \[
...
\begin{proof}
    Let \({\left\{  {A}_{n}\right\}  }_{n \in  J}\) be an indexed family of countable sets, where the index set \(J\) is either \(\{ 1,\ldots ,N\}\) or \({\mathbb{Z}}_{ + }\) . Assume that each set \({A}_{n}\) is nonempty, for convenience; this assumption does not change anything.
    ...
    \]
    It is easy to check that \(h\) is surjective. Since \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) is in bijective correspondence with \({\mathbb{Z}}_{ + }\) , the \index{Countability}countability of the union follows from Theorem 7.1.
\end{proof}
...
\begin{theorem}
    Let \(A\) be a set. There is no injective map \(f : \mathcal{P}\left( A\right)  \rightarrow  A\) , and there is no surjective map \(g : A \rightarrow  \mathcal{P}\left( A\right)\) .
\end{theorem}

\begin{proof}
    In general, if \(B\) is a nonempty set, the existence of an injective map \(f : B \rightarrow\)  \(C\) implies the existence of a surjective map \(g : C \rightarrow  B\) ; one defines \(g\left( c\right)  = {f}^{-1}\left( c\right)\) for each \(c\) in the image set of \(f\) , and defines \(g\) arbitrarily on the rest of \(C\) .
...
\item Let \(X\) be the two-element set \(\{ 0,1\}\) . Show there is a bijective correspondence between the set \(\mathcal{P}\left( {\mathbb{Z}}_{ + }\right)\) and the cartesian product \({X}^{\omega }\) .
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item A real number \(x\) is said to be algebraic (over the rationals) if it satisfies some polynomial equation of positive degree \[ {x}^{n} + {a}_{n - 1}{x}^{n - 1} + \cdots  + {a}_{1}x + {a}_{0} = 0 \] with rational coefficients \({a}_{i}\) . Assuming that each polynomial equation has only finitely many roots, show that the set of \index{Algebraic number}algebraic numbers is countable.
  \item A real number is said to be transcendental if it is not algebraic. Assuming the reals are uncountable, show that the \index{Transcendental number}transcendental numbers are uncountable. (It is a somewhat surprising fact that only two transcendental numbers are familiar to us: \(e\) and \(\pi\) . Even proving these two numbers transcendental is highly nontrivial.)
  \end{enumerate}
\item Determine, for each of the following sets, whether or not it is countable. Justify your answers.
  ...
  \item The set \(F\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) that are ``\index{Eventually zero}eventually zero.'' [We say that \(f\) is eventually zero if there is a positive integer \(N\) such that \(f\left( n\right)  = 0\) for all \(n \geq  N\) ]
  ...
  \end{enumerate}
\item We say that two sets \(A\) and \(B\) \index{Have the same cardinality}have the same cardinality if there is a bijection of \(A\) with \(B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that if \(B \subset  A\) and if there is an injection \[ f : A \rightarrow  B, \] then \(A\) and \(B\) have the same cardinality. [Hint: Define \({A}_{1} = A,{B}_{1} = B\) . and for \(n > 1,{A}_{n} = f\left( {A}_{n - 1}\right)\) and \({B}_{n} = f\left( {B}_{n - 1}\right)\) . (Recursive definition again!) Note that \({A}_{1} \supset  {B}_{1} \supset  {A}_{2} \supset  {B}_{2} \supset  {A}_{3} \supset  \cdots\) . Define a bijection \(h : A \rightarrow  B\) by the rule \[ h\left( x\right)  = \left\{  \begin{array}{ll} f\left( x\right) & \text{ if }x \in  {A}_{n} - {B}_{n}\text{ for some }n, \\  x & \text{ otherwise.} \end{array}\right. \]
  \item Theorem (\index{schroeder-bernstein theorem@Schroeder-Bernstein theorem}Schroeder-Bernstein theorem). If there are injections \(f : A \rightarrow\)  \(C\) and \(g : C \rightarrow  A\) , then \(A\) and \(C\) have the same cardinality.
  \end{enumerate}
...
\item
  ...
  \item Show that the formula \(\left( *\right)\) of part (a) does not determine \(h\) uniquely. [Hint: If \(h\) is a positive function satisfying \(\left( *\right)\) , let \(f\left( i\right)  = h\left( i\right)\) for \(i \neq  3\) , and let \(f\left( 3\right)  =  - h\left( 3\right)\) .]
  \item Show that there is no function \(h : {\mathbb{Z}}_{ + } \rightarrow  \mathbb{R}\) satisfying the formula \[ \begin{aligned} h\left( 1\right)  &= 1, \\ h\left( 2\right)  &= 2, \\ h\left( n\right)  &= {\left\lbrack  h\left( n + 1\right) \right\rbrack  }^{2} + {\left\lbrack  h\left( n - 1\right) \right\rbrack  }^{2}\;\text{ for }n \geq  2. \end{aligned} \]
  \end{enumerate}
\end{enumerate}
...
\begin{lemma}
    Suppose that \(f.\{ 1,\ldots ,n\}  \rightarrow  C\) and \(g : \{ 1,\ldots ,m\}  \rightarrow  C\) both satisfy \(\left( *\right)\) for all \(i\) in their respective domains. Then \(f\left( i\right)  = g\left( i\right)\) for all \(i\) in both domains.
\end{lemma}

\begin{proof}
    Suppose not. Let \(i\) be the smallest integer for which \(f\left( i\right)  \neq  g\left( i\right)\) . The integer \(i\) is not 1, because
    \[
...
\begin{proof}
    By Lemma 8.1, there exists for each \(n\) a function that maps \(\{ 1,\ldots ,n\}\) into \(C\) and satisfies \(\left( *\right)\) for all \(i\) in its domain. Given \(n\) , Lemma 8.2 shows that this function is unique; two such functions having the same domain must be equal. Let \({f}_{n}\) : \(\{ 1,\ldots ,n\}  \rightarrow  C\) denote this unique function.
...
\begin{example}
    ...
    We wish to apply Theorem 8.4 to define a function \(h : {\mathbb{Z}}_{ + } \rightarrow  \mathbb{R}\) rigorously such that \(h\left( n\right)  = {a}^{n}\) . To apply this theorem, let \({a}_{0}\) denote the element \(a\) of \(\mathbb{R}\) , and define \(\rho\) by the equation \(\rho \left( f\right)  = f\left( m\right)  \cdot  a\) , where \(f.\{ 1,\ldots ,m\}  \rightarrow  \mathbb{R}\) Then there exists a unique function \(h : {\mathbb{Z}}_{ + } \rightarrow  \mathbb{R}\) such that
...
\section*{Exercises}
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item Let \(\left( {{b}_{1},{b}_{2},\ldots }\right)\) be an infinite sequence of real numbers. The sum \(\mathop{\sum }\limits_{{k = 1}}^{n}{b}_{k}\) is defined by induction as follows : \[ \begin{aligned} \mathop{\sum }\limits_{{k = 1}}^{n}{b}_{k} &= {b}_{1}\;\text{ for }n = 1, \\ \mathop{\sum }\limits_{{k = 1}}^{n}{b}_{k} &= \left( {\mathop{\sum }\limits_{{k = 1}}^{{n - 1}}{b}_{k}}\right)  + {b}_{n}\;\text{ for }n > 1. \end{aligned} \] Let \(A\) be the set of real numbers; choose \(\rho\) so that Theorem 8.4 applies to define this sum rigorously. We sometimes denote the sum \(\mathop{\sum }\limits_{{k = 1}}^{n}{b}_{k}\) by the symbol \({b}_{1} + {b}_{2} + \cdots  + {b}_{n}\) .
\item Let \(\left( {{b}_{1},{b}_{2},\ldots }\right)\) be an infinite sequence of real numbers. We define the product \(\mathop{\prod }\limits_{{k = 1}}^{n}{b}_{k}\) by the equations \[ \begin{aligned} \mathop{\prod }\limits_{{k = 1}}^{1}{b}_{k} &= {b}_{1} \\ \mathop{\prod }\limits_{{k = 1}}^{n}{b}_{k} &= \left( {\mathop{\prod }\limits_{{k = 1}}^{{n - 1}}{b}_{k}}\right)  \cdot  {b}_{n}\;\text{ for }n > 1. \end{aligned} \] Use Theorem 8.4 to define this product rigorously. We sometimes denote the product \(\mathop{\prod }\limits_{{k = 1}}^{n}{b}_{k}\) by the symbol \({b}_{1}{b}_{2}\cdots {b}_{n}\) .
\item Obtain the definitions of \({a}^{n}\) and \(n\) ! for \(n \in  {\mathbb{Z}}_{ + }\) as special cases of Exercise 2.

\item The \index{fibonacci numbers@Fibonacci numbers}Fibonacci numbers of number theory are defined recursively by the formula \[ \begin{aligned} {\lambda }_{1} &= {\lambda }_{2} = 1 \\ {\lambda }_{n} &= {\lambda }_{n - 1} + {\lambda }_{n - 2}\;\text{ for }n > 2. \end{aligned} \] Define them rigorously by use of Theorem 8.4.
\item Show that there is a unique function \(h : {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{R}}_{ + }\) satisfying the formula \[ \begin{aligned} h\left( 1\right)  &= 3, \\ h\left( i\right)  &= {\left\lbrack  h\left( i - 1\right)  + 1\right\rbrack  }^{1/2}\;\text{ for }i > 1. \end{aligned} \]
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Consider the recursion formula \[ \begin{aligned} h\left( 1\right)  &= 3, \\ h\left( i\right)  &= \left\{  \begin{array}{ll} {\left\lbrack  h\left( i - 1\right)  - 1\right\rbrack  }^{1/2} & \text{ if }h\left( {i - 1}\right)  > 1 \\  5 & \text{ if }h\left( {i - 1}\right)  \leq  1 \end{array}\right\}  \;\text{ for }i > 1. \end{aligned} \] Show that there exists a unique function \(h : {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{R}}_{ + }\) satisfying this formula.
  \end{enumerate}
...
\begin{proof}[Proof of the lemma.]
    ...
    \[
    \mathcal{C} = \left\{  {{B}^{\prime } \mid  B \in  \mathcal{B}}\right\}  \text{ ; }
    \]
...
\item Find if possible a choice function for each of the following collections, without using the choice axiom:
  ...
  \end{enumerate}
\item Suppose that \(A\) is a set and \({\left\{  {f}_{n}\right\}  }_{n \in  {\mathbb{Z}}_{ + }}\) is a given indexed family of injective functions \[ {f}_{n} : \{ 1,\ldots ,n\}  \rightarrow  A \] Show that \(A\) is infinite. Can you define an injective function \(f : {\mathbb{Z}}_{ + } \rightarrow  A\) without using the choice axiom?
...
\item Most of the famous paradoxes of naive set theory are associated in some way or other with the concept of the ``set of all sets.'' None of the rules we have given for forming sets allows us to consider such a set. And for good reason-the concept itself is self-contradictory. For suppose that \(\mathcal{A}\) denotes the ``set of all sets.''
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \(\mathcal{P}\left( \mathcal{A}\right)  \subset  \mathcal{A}\) ; derive a contradiction.
  ...
  \end{enumerate}
\item Let \(A\) and \(B\) be two nonempty sets. If there is an injection of \(B\) into \(A\) , but no injection of \(A\) into \(B\) , we say that \(A\) has \index{Greater cardinality}greater cardinality than \(B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item[*8.] Show that \(\mathcal{P}\left( {\mathbb{Z}}_{ + }\right)\) and \(\mathbb{R}\) have the same cardinality. [Hint: You may use the fact that every real number has a decimal expansion, which is unique if expansions that end in an infinite string of 9 ’s are forbidden.] \\A famous conjecture of set theory, called the \index{Continuum hypothesis}continuum hypothesis, asserts that there exists no set having greater cardinality than \({\mathbb{Z}}_{ + }\) and lesser cardinality than \(\mathbb{R}\) . The \index{Generalized continuum hypothesis}generalized continuum hypothesis asserts that, given the infinite set \(A\) , there is no set having greater cardinality than \(A\) and lesser cardinality than \(\mathcal{P}\left( A\right)\) . Surprisingly enough, both of these assertions have been shown to be independent of the usual axioms for set theory. For a readable expository account, see \cite{Smullyan1969}.
\end{enumerate}
\section*{§ 10 Well-Ordered Sets}

One of the useful properties of the set \({\mathbb{Z}}_{ + }\) of positive integers is the fact that each of its nonempty subsets has a smallest element. Generalizing this property leads to the concept of a \index{Well-ordered set}well-ordered set.
...
\item Let \({S}_{\Omega }\) be the minimal uncountable well-ordered set.
  ...
  \end{enumerate}
\item Let \(J\) be a well-ordered set. A subset \({J}_{0}\) of \(J\) is said to be inductive if for every \(\alpha  \in  J,\) \[ \left( {{S}_{\alpha } \subset  {J}_{0}}\right)  \Rightarrow  \alpha  \in  {J}_{0} \] \textbf{Theorem (The principle of \index{Transfinite induction}transfinite induction).} If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Consider the subset \(A\) of \({\left( {\mathbb{Z}}_{ + }\right) }^{\omega }\) consisting of all infinite sequences of positive integers \(\mathbf{x} = \left( {{x}_{1},{x}_{2},\ldots }\right)\) that end in an infinite string of 1 ’s. Give \(A\) the following order: \(\mathbf{x} < \mathbf{y}\) if \({x}_{n} < {y}_{n}\) and \({x}_{i} = {y}_{i}\) for \(i > n\) . We call this the ``antidictionary order'' on \(A\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that for every \(n\) , there is a section of \(A\) that has the same order type as \({\left( {\mathbb{Z}}_{ + }\right) }^{n}\) in the dictionary order.
  \item Show \(A\) is well-ordered.
  \end{enumerate}
\item \textsl{Theorem.} Let \(J\) and \(C\) be well-ordered sets; assume that there is no surjective function mapping a section of \(J\) onto \(C\) . Then there exists a unique function \(h : J \rightarrow  C\) satisfying the equation \[ \left( *\right) \qquad h\left( x\right)  = \text{ smallest }\left\lbrack  {C - h\left( {S}_{x}\right) }\right\rbrack \] for each \(x \in  J\) , where \({S}_{x}\) is the section of \(J\) by \(x\) .

\textsl{Proof}
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(h\) and \(k\) map sections of \(J\) , or all of \(J\) , into \(C\) and satisfy \(\left( *\right)\) for all \(x\) in their respective domains, show that \(h\left( x\right)  = k\left( x\right)\) for all \(x\) in both domains.
  \item If there exists a function \(h : {S}_{\alpha } \rightarrow  C\) satisfying \(\left( *\right)\) , show that there exists a function \(k \cdot  {S}_{\alpha } \cup  \{ \alpha \}  \rightarrow  C\) satisfying \(\left( *\right)\) .
  \item If \(K \subset  J\) and for all \(\alpha  \in  K\) there exists a function \({h}_{\alpha } \cdot  {S}_{\alpha } \rightarrow  C\) satisfying \(\left( *\right)\) , show that there exists a function \[ k : \mathop{\bigcup }\limits_{{\alpha  \in  K}}{S}_{\alpha } \rightarrow  C \] satisfying \(\left( *\right)\) .
  \item Show by transfinite induction that for every \(\beta  \in  J\) , there exists a function \({h}_{\beta } : {S}_{\beta } \rightarrow  C\) satisfying \(\left( *\right)\) . [Hint: If \(\beta\) has an immediate predecessor \(\alpha\) , then \({S}_{\beta } = {S}_{\alpha } \cup  \{ \alpha \}\) . If not, \({S}_{\beta }\) is the union of all \({S}_{\alpha }\) with \(\alpha  < \beta\) .]
...
\begin{centeredblock}
\begin{example}
    If \(\left( {{x}_{0},{y}_{0}}\right)\) and \(\left( {{x}_{1},{y}_{1}}\right)\) are two points of the plane \({\mathbb{R}}^{2}\) , define
    \[
...
\item Given two points \(\left( {{x}_{0},{y}_{0}}\right)\) and \(\left( {{x}_{1},{y}_{1}}\right)\) of \({\mathbb{R}}^{2}\) , define \[ \left( {{x}_{0},{y}_{0}}\right)  \prec  \left( {{x}_{1},{y}_{1}}\right) \] if \({x}_{0} < {x}_{1}\) and \({y}_{0} \leq  {y}_{1}\) . Show that the curves \(y = {x}^{3}\) and \(y = 2\) are maximal simply ordered subsets of \({\mathbb{R}}^{2}\) , and the curve \(y = {x}^{2}\) is not. Find all maximal simply ordered subsets.
...
\item
  ...
  \item Let \(J\) and \(E\) be well-ordered sets, let \(h : J \rightarrow  E\) . Show the following two statements are equivalent:
    ...
    \item \(h\left( \alpha \right)  =\) smallest \(\left\lbrack  {E - h\left( {S}_{\alpha }\right) }\right\rbrack\) for all \(\alpha\) .
    \end{enumerate}
...
\item Let \(J\) and \(E\) be well-ordered sets; suppose there is an order-preserving map \(k : J \rightarrow  E\) . Using Exercises 1 and 2, show that \(J\) has the order type of \(E\) or a section of \(E\) . [Hint: Choose \({e}_{0} \in  E\) . Define \(h : J \rightarrow  E\) by the recursion formula
      ...
      and \(h\left( \alpha \right)  = {e}_{0}\) otherwise. Show that \(h\left( \alpha \right)  \leq  k\left( \alpha \right)\) for all \(\alpha\) ; conclude that \([\,h\left( {S}_{\alpha }\right)  \neq  E\text{ for all }\alpha .\,]\)
\item Use Exercises 1-3 to prove the following
  ...
  \end{enumerate}
\item Let \(X\) be a set; let \(\mathcal{A}\) be the collection of all pairs \(\left( {A, < }\right)\) , where \(A\) is a subset of \(X\) and \(<\) is a well-ordering of \(A\) . Define \[ \left( {A, < }\right)  \prec  \left( {{A}^{\prime },{ < }^{\prime }}\right) \] if \(\left( {A, < }\right)\) equals a section of \(\left( {{A}^{\prime },{ < }^{\prime }}\right)\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \(\prec\) is a strict partial order on \(\mathcal{A}\) .
  \item Let \(\mathcal{B}\) be a subcollection of \(\mathcal{A}\) that is simply ordered by \(\prec\) . Define \({B}^{\prime }\) to be the union of the sets \(B\) , for all \(\left( {B, < }\right)  \in  \mathcal{B}\) ; and define \({ < }^{\prime }\) to be the union of the relations \(<\) , for all \(\left( {B, < }\right)  \in  \mathcal{B}\) . Show that \(\left( {{B}^{\prime },{ < }^{\prime }}\right)\) is a well-ordered set.
  \end{enumerate}
...
\item Use Exercises 1-5 to prove the following. \\ \textsl{Theorem. The choice axiom is equivalent to the well-ordering theorem.} \\ \textsl{Proof.} Let \(X\) be a set, let \(c\) be a fixed choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(\left( {T, < }\right)\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) , \[ x = c\left( {X - {S}_{x}\left( T\right) }\right) , \] where \({S}_{x}\left( T\right)\) is the section of \(T\) by \(x\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Let \(\left( {{T}_{1},{ < }_{1}}\right)\) and \(\left( {{T}_{2},{ < }_{2}}\right)\) be two towers in \(X\) . Show that either these two ordered sets are the same, or one equals a section of the other. [Hint: Switching indices if necessary, we can assume that \(h : {T}_{1} \rightarrow  {T}_{2}\) is order preserving and \(h\left( {T}_{1}\right)\) equals either \({T}_{2}\) or a section of \({T}_{2}\) . Use Exercise 2 to show that \(h\left( x\right)  = x\) for all \(x\) .]
  \item If \(\left( {T, < }\right)\) is a tower in \(X\) and \(T \neq  X\) , show there is a tower in \(X\) of which \(\left( {T, < }\right)\) is a section.
  \item Let \(\left\{  {\left( {{T}_{k},{ < }_{k}}\right)  \mid  k \in  K}\right\}\) be the collection of all towers in \(X\) . Let \[ T = \mathop{\bigcup }\limits_{{k \in  K}}{T}_{k}\;\text{ and }\; <  = \mathop{\bigcup }\limits_{{k \in  K}}\left( { < }_{k}\right) . \] Show that \(\left( {T, < }\right)\) is a tower in \(X\) . Conclude that \(T = X\) .
  \end{enumerate}
\item Using Exercises 1-4, construct an uncountable well-ordered set, as follows. Let \(A\) be the collection of all pairs \(\left( {A, < }\right)\) , where \(A\) is a subset of \({\mathbb{Z}}_{ + }\) and \(<\) is a well-ordering of \(A\) . (We allow \(A\) to be empty.) Define \(\left( {A, < }\right)  \sim  \left( {{A}^{\prime },{ < }^{\prime }}\right)\) if \(\left( {A, < }\right)\) and \(\left( {{A}^{\prime },{ < }^{\prime }}\right)\) have the same order type. It is trivial to show this is an equivalence relation. Let \(\left\lbrack  \left( {A, < }\right) \right\rbrack\) denote the equivalence class of \(\left( {A, < }\right)\) ; let \(E\) denote the collection of these equivalence classes. Define \[ \left\lbrack  \left( {A, < }\right) \right\rbrack \ll  \left\lbrack  \left( {{A}^{\prime },{ < }^{\prime }}\right) \right\rbrack \] if \(\left( {A, < }\right)\) has the order type of a section of \(\left( {{A}^{\prime },{ < }^{\prime }}\right)\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that the relation \(\ll\) is well defined and is a simple order on \(E\) . Note that the equivalence class \(\left\lbrack  \left( {\varnothing ,\varnothing }\right) \right\rbrack\) is the smallest element of \(E\) .
  \item Show that if \(\alpha  = \left\lbrack  \left( {A, < }\right) \right\rbrack\) is an element of \(E\) , then \(\left( {A, < }\right)\) has the same order type as the section \({S}_{\alpha }\left( E\right)\) of \(E\) by \(\alpha\) . [Hint: Define a map \(f : A \rightarrow  E\) by setting \(f\left( x\right)  = \left\lbrack  \left( {{S}_{x}\left( A\right) \text{ , restriction of } < }\right) \right\rbrack\) for each \(x \in  A\) .]
