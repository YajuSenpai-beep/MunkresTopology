...
\]
The \idx[relations]{relation} \(\subset\) and \(\varsubsetneq\) are called \idx{inclusion} and \idx{proper inclusion}, respectively. If \(A \subset  B\) , we also write \(B \supset  A\) , which is read `` \(B\) \idx{contains} \(A\) .''
...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the intersection\idx[section]{Section: of the positive integers} of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
Again, mathematics cannot tolerate ambiguity, so a \idx[choice axiom]{Choice axiom (see Axiom of choice)} choice of meanings must be made. Mathematicians have agreed always to use ``if ... then'' in the first sense, so that a statement of the form ``If \(P\) , then \(Q\) `` means that if \(P\) is true, \(Q\) is true also, but if \(P\) is false, \(Q\) may be either true or false.
...
\section*{Negation}
If one wishes to form the contrapositive of the statement \(P \Rightarrow  Q\) , one has to know how to form the statement ``not \(P\) ,'' which is called the \idx{negation} of \(P\) . In many cases, this causes no difficulty; but sometimes confusion occurs with statements involving the phrases ``for every'' and ``for at least one.'' These phrases are called logical \idx[logical quantifiers]{Quantifiers, logical} quantifiers.
...
We return now to our discussion of sets. There is one other \idx[binary operation]{Operation, binary} operation on sets that is occasionally useful. It is the \idx{difference of two sets}, denoted by \(A - B\) , and defined as the set consisting of those elements of \(A\) that are not in \(B\) . Formally,
\[
...
\]
holds. The equation is illustrated in Figure 1.3; the shaded region represents the set in question, as you can check mentally. This equation can be thought of as a ``\idx[distributive laws]{Distributive laws for  and} distributive law'' for the operations \(\cap\) and \(\cup\) .
...
We now have another way to form new sets from old ones. Given a set \(A\) , we can consider sets whose elements are subsets of \(A\) . In particular, we can consider the set of all subsets of \(A\) . This set is sometimes denoted by the symbol \(\idxmath{P}{\(\mathcal{P}\left( A\right)\)}\) and is called the \idx{power set} of \(A\) (for reasons to be explained later).
...
The notion of ordered pair carries over to general sets. Given sets \(A\) and \(B\) , we define their \idx[cartesian product]{Cartesian product: countably infinite} cartesian product \(A \times  B\) to be the set of all ordered pairs(a, b)for which \(a\) is an element of \(A\) and \(b\) is an element of \(B\) . Formally,
...
\begin{definition} 
    A \idx{rule of assignment} is a subset \(r\) of the cartesian product \(\mathbf{C} \times  \mathbf{D}\) of two sets, having the property that each element of \(C\) appears as the first coordinate of at most one ordered pair belonging to \(r\) .
...
\begin{definition}
    A function \(f\) is a rule of assignment \(r\) , together with a set \(B\) that contains the image set of \(r\) . The domain \(A\) of the rule \(r\) is also called the domain of the function \(f\) ; the image set of \(r\) is also called the image set of \(f\) ; and the set \(B\) is called the \idx{Range of a function} range of \(f{.}^{\dagger}\) .
\end{definition}
...
Restricting the domain of a function and changing its range are two ways of forming a new function from an old one. Another way is to form the \idx[composite]{Composite: of functions} composite of two functions.
...
Injectivity of \(f\) depends only on the rule of \(f\) ; surjectivity depends on the range of \(f\) as well. You can check that the composite of two \idx[injective functions]{injective function} is injective, and the composite of two \idx[surjective functions]{surjective function} is surjective; it follows that the composite of two \idx[bijective functions]{bijective function} is bijective.

If \(f\) is bijective, there exists a function from \(B\) to \(A\) called the \idx[inverse]{Inverse function} of \(f\) . It is denoted by \({f}^{-1}\) and is defined by letting \({f}^{-1}\left( b\right)\) be that unique element \(a\) of \(A\) for which \(f\left( a\right)  = b\) . Given \(b \in  B\) , the fact that \(f\) is surjective implies that there exists such an element \(a \in  A\) ; the fact that \(f\) is injective implies that there is only one such element \(a\) . It is easy to see that if \(f\) is bijective, \({f}^{-1}\) is also bijective.
...
\end{enumerate}
\section*{§ 3 Relations}
A concept that is, in some ways, more general than that of function is the concept of a relation. In this section, we define what mathematicians mean by a relation, and we consider two types of relations that occur with great frequency in mathematics: \idx[equivalence relations]{equivalence relation} and \idx[order relations]{order relation}. Order relations will be used throughout the book; equivalence relations will not be used until \(\$ {22}\) .
...
\begin{definition}
    A \idx{partition of a set} \(A\) is a collection of disjoint nonempty subsets of \(A\) whose union is all of \(A\) .
\end{definition}
...
(1) (\idx[comparability]{Comparability: of cardinalities} Comparability) For every \(x\) and \(y\) in \(A\) for which \(x \neq  y\) , either \({xCy}\) or \({yCx}\) .
...
\begin{definition}
    Suppose that \(A\) and \(B\) are two sets with order relations \({ < }_{A}\) and \({ < }_{B}\) respectively. We say that \(A\) and \(B\) have the same \idx{order type} if there is a bijective correspondence between them that preserves order; that is, if there exists a bijective function \(f : A \rightarrow  B\) such that
    \[
...
\begin{centeredblock}
\begin{example}
    Consider the set \(\lbrack 0,1)\) of real numbers and the set \({\mathbb{Z}}_{ + }\) of \idx{positive integers}, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
    \[
...
\begin{theorem}[\idx{Strong induction principle}]
...
\item Prove the following properties of \(\mathbb{Z}\) and \({\mathbb{Z}}_{ + }\) :
  ...
  \end{enumerate}
\item Let \(a \in  \mathbb{R}\) . Define inductively \[ \begin{aligned} {a}^{1} &= a, \\ {a}^{n + 1} &= {a}^{n} \cdot  a \end{aligned} \] for \(n \in  {\mathbb{Z}}_{ + }\) . (See \(§7\) for a discussion of the process of \idx{inductive definition}.) Show that for \(n,m \in  {\mathbb{Z}}_{ + }\) and \(a,b \in  \mathbb{R}\) , \[ \begin{aligned} {a}^{n}{a}^{m} &= {a}^{n + m}, \\ {\left( {a}^{n}\right) }^{m} &= {a}^{nm}, \\ {a}^{m}{b}^{m} &= {\left( ab\right) }^{m}. \end{aligned} \] These are called the laws of exponents [Hint: For fixed \(n\) , prove the formulas by induction on \(m\) .]
...
\begin{definition}
    Let \(\mathcal{A}\) be a nonempty collection of sets. An \idx{indexing function} for \(\mathcal{A}\) is a surjective function \(f\) from some set \(J\) , called the \idx{index set}, to \(\mathcal{A}\) . The collection \(\mathcal{A}\) , together with the indexing function \(f\) , is called an indexed \idx{family of sets}. Given \(\alpha  \in  J\) , we shall denote the set \(f\left( \alpha \right)\) by the symbol \({A}_{\alpha }\) . And we shall denote the indexed family itself by the symbol
    \[
...
Nothing in these definitions requires the sets \({A}_{l}\) to be different from one another. Indeed, they may all equal the same set \(X\). In that case, the cartesian product \({A}_{1} \times\)  \(\cdots  \times  {A}_{m}\) is just the set of all \(m\) -tuples of elements of \(X\) , which we denote by \(\idxmath{X}{\({X}^{m}\)}\) . Similarly, the product \({A}_{1} \times  {A}_{2} \times  \cdots\) is just the set of all \(\omega\) -tuples of elements of \(X\) , which we denote by \(\idxmath{X}{\({X}^{\omega }\)}\) .
...
\idx{Finite Set} sets and \idx[infinite sets]{infinite set}, \idx[countable sets]{countable set} and \idx[uncountable sets]{uncountable set}, these are types of sets that you may have encountered before. Nevertheless, we shall discuss them in this section and the next, not only to make sure you understand them thoroughly, but also to elucidate some particular points of logic that will arise later on. First we consider finite sets.
...
\begin{definition}
    ...
    \]
    for some positive integer \(n\) . In the former case, we say that \(A\) has \idx[cardinality]{Cardinality: comparability} cardinality 0; in the latter case, we say that \(A\) has cardinality \(n\) .
\end{definition}
...
What is needed is another principle, which we call the principle of \idx[principle of recursive definition]{Recursive definition, principle} recursive definition. In the proof of the preceding theorem, we wished to assert the following:
...
\begin{centeredblock}
    Let the \idx[barber of Seville]{Barber of Seville paradox} barber of Seville shave every man of Seville who does not shave himself. Who shall shave the barber?
\end{centeredblock}
...
\item Determine, for each of the following sets, whether or not it is countable. Justify your answers.
  ...
  \item The set \(F\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) that are ``\idx{eventually zero}.'' [We say that \(f\) is eventually zero if there is a positive integer \(N\) such that \(f\left( n\right)  = 0\) for all \(n \geq  N\) ]
  ...
  \end{enumerate}
\item We say that two sets \(A\) and \(B\) \idx{have the same cardinality} if there is a bijection of \(A\) with \(B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Theorem (\idx{Schroeder-Bernstein theorem}). If there are injections \(f : A \rightarrow\)  \(C\) and \(g : C \rightarrow  A\) , then \(A\) and \(C\) have the same cardinality.
  \end{enumerate}
...
\begin{example}
    ...
    Because \(C\) is infinite and \(f\) is a function mapping a finite set into \(C\) , the image set of \(f\) is not all of \(C\) , therefore, \(\rho\) is well defined. By Theorem 8.4 there exists a function \(h \colon {\mathbb{Z}}_{ + } \rightarrow\)  \(C\) such that \(h\left( 1\right)  = {a}_{0}\) , and for \(i > 1\) ,
...
\item The \idx{Fibonacci numbers} of number theory are defined recursively by the formula \[ \begin{aligned} {\lambda }_{1} &= {\lambda }_{2} = 1 \\ {\lambda }_{n} &= {\lambda }_{n - 1} + {\lambda }_{n - 2}\;\text{ for }n > 2. \end{aligned} \] Define them rigorously by use of Theorem 8.4.
...
\begin{lemma}[Existence of a \idx{choice function}]
...
\item Most of the famous paradoxes of naive set theory are associated in some way or other with the concept of the ``set of all sets.'' None of the rules we have given for forming sets allows us to consider such a set. And for good reason-the concept itself is self-contradictory. For suppose that \(\mathcal{A}\) denotes the ``set of all sets.''
  ...
  \end{enumerate}
\item Let \(A\) and \(B\) be two nonempty sets. If there is an injection of \(B\) into \(A\) , but no injection of \(A\) into \(B\) , we say that \(A\) has \idx{greater cardinality} than \(B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\begin{definition}
    Let \(X\) be a well-ordered set. Given \(\alpha  \in  X\) , let \(\idxmath{S}{\({S}_{\alpha }\)} {S}_{\alpha }\) denote the set
    \[
...
\begin{lemma}
    There exists a well-ordered set \(A\) having a largest element \(\Omega\) , such that the section \(\idxmath{S}{\({S}_{\Omega }\)} {S}_{\Omega }\) of \(A\) by \(\Omega\) is uncountable but every other section of \(A\) is countable.
\end{lemma}
...
\item Let \({S}_{\Omega }\) be the minimal uncountable well-ordered set.
  ...
  \end{enumerate}
\item Let \(J\) be a well-ordered set. A subset \({J}_{0}\) of \(J\) is said to be inductive if for every \(\alpha  \in  J,\) \[ \left( {{S}_{\alpha } \subset  {J}_{0}}\right)  \Rightarrow  \alpha  \in  {J}_{0} \] \textbf{Theorem (The principle of \idx{transfinite induction}).} If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\textsl{Proof}
  ...
  \item If there exists a function \(h : {S}_{\alpha } \rightarrow  C\) satisfying \(\left( *\right)\) , show that there exists a function \(k \colon {S}_{\alpha } \cup  \{ \alpha \}  \rightarrow  C\) satisfying \(\left( *\right)\) .
  \item If \(K \subset  J\) and for all \(\alpha  \in  K\) there exists a function \({h}_{\alpha } \colon {S}_{\alpha } \rightarrow  C\) satisfying \(\left( *\right)\) , show that there exists a function \[ k : \mathop{\bigcup }\limits_{{\alpha  \in  K}}{S}_{\alpha } \rightarrow  C \] satisfying \(\left( *\right)\) .
...
\begin{definition}
    Let \(A\) be a set and let \(\prec\) be a strict partial order on \(A\) . If \(B\) is a subset of \(A\) , an upper bound on \(B\) is an element \(c\) of \(A\) such that for every \(b\) in \(B\) , either \(b = c\) or \(b \prec  c\) . A \idx{maximal element} of \(A\) is an element \(m\) of \(A\) such that for no element \(a\) of \(A\) does the relation \(m \prec  a\) hold.
\end{definition}
...
\item A collection \(\mathcal{A}\) of subsets of a set \(X\) is said to be of finite type provided that a subset \(B\) of \(X\) belongs to \(\mathcal{A}\) if and only if every finite subset of \(B\) belongs to \(\mathcal{A}\) . Show that the \idx{Kuratowski lemma} implies the following. \\ \textbf{Lemma (Tukey, 1940).} Let \(\mathcal{A}\) be a collection of sets. If \(\mathcal{A}\) is of finite type, then \(\mathcal{A}\) has an element that is properly contained in no other element of \(\mathcal{A}\) .
\item Show that the \idx{Tukey lemma} implies the \idx{Hausdorff maximum principle}. [Hint: If \(\prec\) is a strict partial order on \(A\) , let \(\mathcal{A}\) be the collection of all subsets of \(A\) that are simply ordered by \(\prec\) . Show that \(\mathcal{A}\) is of finite type.]
