...
The \index{equality symbol} \(=\) is used throughout this book to mean \index{Logical equivalence} logical identity. Thus, when we write \(a = b\) , we mean that `` \(a\) `` and `` \(b\) `` are symbols for the \index{Cardinality: comparability!same}\index{Cardinality: comparability!same}same object. This is what one means in arithmetic, for example, when one writes \(\displaystyle\frac{2}{4} = \frac{1}{2}\) . Similarly, the equation \(A = B\) states that `` \(A\) `` and `` \(B\) `` are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
Given sets \(A\) and \(B\) , another way one can form a set is to take the common part of \(A\) and \(B\) . This set is called the \index{Intersection}\index{Intersection}intersection\index{Section: of the positive integers} of \(A\) and \(B\) and is denoted by \(A \cap  B\) . Formally, we define
\[
...
Now some students are bothered by the notion of an ``\index{Empty set}\index{Empty set}empty set.'' ``How,'' they say, ``can you have a set with nothing in it?'' The problem is similar to that which arose many years ago when the number 0 was first introduced.
...
\]
It is a statement of the form, ``If \(P\) , then \(Q\) ,'' where \(P\) is the phrase `` \(x > 0\) '' (called the \index{hypothesis} of the statement) and \(Q\) is the phrase `` \({x}^{3} \neq  0\) '' (called the \index{conclusion} of the statement). This is a true statement, for in every case for which the \index{Hypothesis}\index{Hypothesis}hypothesis \(x > 0\) holds, the \index{Conclusion}\index{Conclusion}conclusion \({x}^{3} \neq  0\) holds as well.
...
To return now to the empty set and inclusion, we see that the inclusion \(\varnothing  \subset  A\) does hold for every set \(A\) . Writing \(\varnothing  \subset  A\) is the same as saying, ``If \(x \in  \varnothing\) , then \(x \in  A\) ,'' and this statement is \index{Vacuously true}\index{Vacuously true}vacuously true.
...
Given a statement of the form ``If \(P\) , then \(Q\) ,'' its \index{Contrapositive}\index{Contrapositive}contrapositive is defined to be the statement ``If \(Q\) is not true, then \(P\) is not true.'' For example, the contrapositive of the statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ . }
\]
is the statement
...
Q \Rightarrow  P
\]
which is called the \index{Converse}\index{Converse}converse of \(P \Rightarrow  Q\) . One must be careful to distinguish between a statement's converse and its contrapositive. Whereas a statement and its contrapositive are logically equivalent, the truth of a statement says nothing at all about the truth or falsity of its converse. For example, the true statement
\[
\text{ If }x > 0\text{ , then }{x}^{3} \neq  0\text{ , }
\]
...
To illustrate, suppose that \(X\) is a set, \(A\) is a subset of \(X\) , and \(P\) is a statement about the \index{Principle of recursive definition!general}\index{Principle of recursive definition!general}general element of \(X\) . Consider the following statement:
\[
( *) \hspace{1em} \text{ For every }x \in  A\text{ , statement }P\text{ holds. }
\]
How does one form the \index{Negation}\index{Negation}negation of this statement? Let us translate the problem into the language of sets. Suppose that we let \(B\) denote the set of all those elements \(x\) of \(X\) for which \(P\) holds. Then statement \(( *)\) is just the statement that \(A\) is a subset of \(B\) . What is its negation? Obviously, the statement that \(A\) is not a subset of \(B\) ; that is, the statement that there exists at least one element of \(A\) that does not belong to \(B\) . Translating back into ordinary language, this becomes
\[
...
\]
It is sometimes called the \index{complement} of \(B\) relative to \(A\) , or the \index{Complement}\index{Complement}complement of \(B\) in \(A\) . Our three set operations are represented schematically in Figure 1.1.
...
\end{aligned}
\]
We leave it to you to check these rules. One can state other rules of set theory, but these are the most important ones. \index{DeMorgan's laws}\index{DeMorgan's laws}DeMorgan's laws are easier to remember if you verbalize them as follows:
...
\item If a set \(A\) has two elements, show that \(\mathcal{P}( A)\) has four elements. How many elements does \(\mathcal{P}( A)\) have if \(A\) has one element? Three elements? No elements? Why is \(\mathcal{P}( A)\) called the \index{Power set}\index{Power set}power set of \(A\) ?
...
\section*{§ 2 Functions}
The concept of \index{Function}\index{Function}function is one you have seen many times already, so it is hardly necessary to remind you how central it is to all mathematics. In this section, we give the precise mathematical definition, and we explore some of the associated concepts.

A function is usually thought of as a rule that assigns to each \index{Element of a set}\index{Element of a set}element of a set \(A\) , an element of a set \(B\) . In calculus, a function is often given by a simple formula such as \(f( x)  = 3{x}^{2} + 2\) or perhaps by a more complicated formula such as
...
\begin{definition} 
    ...
    Thus, a subset \(r\) of \(C \times  D\) is a \index{Rule of assignment}\index{Rule of assignment}rule of assignment if
    \[
    ...
    \end{aligned}
    \]
    Note that given a rule of assignment \(r\) , its \index{Domain}\index{Domain}domain and \index{Image}\index{Image}image are entirely determined.
\end{definition}

Now we can say what a function is.

\begin{definition}
    A function \(f\) is a rule of assignment \(r\) , together with a set \(B\) that \index{Contains}\index{Contains}contains the image set of \(r\) . The domain \(A\) of the rule \(r\) is also called the domain of the function \(f\) ; the image set of \(r\) is also called the image set of \(f\) ; and the set \(B\) is called the \index{Range of a function} range of \(f{.}^{\dagger}\) .
\end{definition}
