...
One of the useful properties of the set \({\mathbb{Z}}_{ + }\) of positive integers is the fact that each of its nonempty subsets has a smallest element. Generalizing this property leads to the concept of a \index{Well-ordered set}.
...
\item Use Exercises 1-5 to prove the following. \\ \textsl{Theorem. The choice axiom is equivalent to the well-ordering theorem.} \\ \textsl{Proof.} Let \(X\) be a set, let \(c\) be a fixed \index{choice function@Choice function}choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(( {T, < })\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) , \[ x = c( {X - {S}_{x}( T) }) , \] where \({S}_{x}( T)\) is the section of \(T\) by \(x\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
