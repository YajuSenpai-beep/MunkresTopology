...
% 书脊图案：缩放在盒内完成（使 \wd 即书脊内实际宽度，供自动堆叠量高使用）
% 注：scope/pic 的 scale 对图案不生效，必须用 \scalebox
\newsavebox{\spinepatternbox}
\sbox{\spinepatternbox}{\scalebox{0.17}{\begin{tikzpicture}\coverpattern\end{tikzpicture}}}
...
% ═══ 书脊内容：自动堆叠 ═══
% 各元素先装盒量"沿脊高度"(=盒宽 \wd)，再自顶向下按固定间隔累加定位。
% 好处：改任一元素的字号/kern → 其下方元素自动重排，无需手工重算 yshift。
% 颜色在盒内设定（盒定型时取色）；调整项集中在下方「书脊几何参数」与各 \sbox。
\newsavebox{\spineMunkres}
\sbox{\spineMunkres}{\color{linecolor}\Trajan\fontsize{20}{24}\selectfont MUNKRES}
\newsavebox{\spineTopology}
\sbox{\spineTopology}{\color{white}\SenatorTall\fontsize{72}{78}\selectfont
    T\kern0.10em O\kern0.10em P\kern0.10em O\kern0.10em L\kern0.10em O\kern0.10em G\kern0.10em Y}
\newsavebox{\spineSecond}
\sbox{\spineSecond}{\color{linecolor}\RebarBold\fontsize{20}{24}\selectfont
...
\newlength{\spinecx}    % 书脊横向中线
\newlength{\spiney}     % 当前元素垂直位置（距页底，逐次累加）
\newlength{\spinetop}   % 内容顶端（距页底）—— 控制书脊顶部留白
\newlength{\spinegap}   % 相邻元素间隔（沿脊方向）
\setlength{\spinetop}{28.22cm}
\setlength{\spinegap}{1.07cm}

% 放置一个书脊元素：\spiney 由「上一元素底」推进 → 本元素中心 → 本元素底+间隔
\newcommand{\spineplace}[1]{%
    \addtolength{\spiney}{-0.5\wd#1}%
    \node[rotate=-90, anchor=center, inner sep=0pt]
        at ([xshift=\spinecx, yshift=\spiney]current page.south west) {\usebox{#1}};%
    \addtolength{\spiney}{-0.5\wd#1}%
    \addtolength{\spiney}{-\spinegap}%
}

% ── 书脊（TikZ overlay：分界线 + 内容）──
\begin{tikzpicture}[remember picture,overlay]
    ...
    % 自顶向下依序放置（顺序 = 书脊上从上到下的顺序）
    \setlength{\spiney}{\spinetop}
    \spineplace{\spineMunkres}
    \spineplace{\spinepatternbox}
    \spineplace{\spineTopology}
    \spineplace{\spineSecond}
