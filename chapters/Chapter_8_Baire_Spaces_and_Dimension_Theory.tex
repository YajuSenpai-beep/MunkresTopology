...
where exactly \(M\) of the sets \({A}_{i}\) belong to \(\mathcal{J}\) , and the remainder belong to \(\mathcal{K}\) . If \(M > 0\) , then \(C\) is homeomorphic to the product \({\left( 0,1\right) }^{M}\) and will be called an \index{Cube in \({\mathbb{R}}^{n}\)} \(M\) -cube. If \(M = 0\) , then \(C\) consists of a single point and will be called a 0 -cube
