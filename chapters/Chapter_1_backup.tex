...
\begin{definition} A rule of assignment is a subset \(r\) of the cartesian product \(\mathbf{C} \times  \mathbf{D}\) of two sets, having the property that each element of \(C\) appears as the first coordinate of at most one ordered pair belonging to \(r\) .
...
\begin{definition} A function \(f\) is a rule of assignment \(r\) , together with a set \(B\) that contains the image set of \(r\) . The domain \(A\) of the rule \(r\) is also called the domain of the function \(f\) ; the image set of \(r\) is also called the image set of \(f\) ; and the set \(B\) is called the range of \(f{.}^{\dagger}\) .
\end{definition}
...
\begin{definition} Given functions \(f : A \rightarrow  B\) and \(g : B \rightarrow  C\) , we define the composite \(g \circ  f\) of \(f\) and \(g\) as the function \(g \circ  f : A \rightarrow  C\) defined by the equation \(\left( {g \circ  f}\right) \left( a\right)  =\)  \(g\left( {f\left( a\right) }\right)\) .
\end{definition}
...
\begin{centeredblock}
\begin{example} The composite of the function \(f.\mathbb{R} \rightarrow  \mathbb{R}\) given by \(f\left( x\right)  = 3{x}^{2} + 2\) and the function \(g : \mathbb{R} \rightarrow  \mathbb{R}\) given by \(g\left( x\right)  = {5x}\) is the function \(g \circ  f : \mathbb{R} \rightarrow  \mathbb{R}\) given by
...
\end{proof}
\begin{definition} A partition of a set \(A\) is a collection of disjoint nonempty subsets of \(A\) whose union is all of \(A\) .
\end{definition}
...
\begin{definition} Suppose that \(A\) and \(B\) are two sets with order relations \({ < }_{A}\) and \({ < }_{B}\) respectively. We say that \(A\) and \(B\) have the same order type if there is a bijective correspondence between them that preserves order; that is, if there exists a bijective function \(f : A \rightarrow  B\) such that
...
\begin{centeredblock}
\begin{example} Consider the set \(\lbrack 0,1)\) of real numbers and the set \({\mathbb{Z}}_{ + }\) of positive integers, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
...
\begin{theorem}[Strong induction principle] Let \(A\) be a set of positive integers. Suppose that for each posiave integer \(n\) , the statement \({S}_{n} \subset  A\) implies the statement \(n \in  A\) . Then \(A = {\mathbb{Z}}_{ + }\) .
\end{theorem}
...
\begin{definition} Let \(\mathcal{A}\) be a nonempty collection of sets. An indexing function for \(\mathcal{A}\) is a surjective function \(f\) from some set \(J\) , called the index set, to \(\mathcal{A}\) . The collection \(\mathcal{A}\) , together with the indexing function \(f\) , is called an indexed family of sets. Given \(\alpha  \in  J\) , we shall denote the set \(f\left( \alpha \right)\) by the symbol \({A}_{\alpha }\) . And we shall denote the indexed family itself by the symbol
...
\end{proof}
\begin{lemma}[Existence of a choice function] Given a collection \(\mathcal{B}\) of nonempty sets (not necessarily disjoint), there exists a function
...
\begin{definition} Let \(X\) be a well-ordered set Given \(\alpha  \in  X\) , let \({S}_{\alpha }\) denote the set
...
\begin{lemma} There exists a well-ordered set \(A\) having a largest element \(\Omega\) , such that the section \({S}_{\Omega }\) of \(A\) by \(\Omega\) is uncountable but every other section of \(A\) is countable.
\end{lemma}
...
\begin{theorem}[The principle of transfinite induction] If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .
\end{theorem}
...
\begin{definition} Let \(A\) be a set and let \(\prec\) be a strict partial order on \(A\) . If \(B\) is a subset of \(A\) , an upper bound on \(B\) is an element \(c\) of \(A\) such that for every \(b\) in \(B\) , either \(b = c\) or \(b \prec  c\) . A maximal element of \(A\) is an element \(m\) of \(A\) such that for no element \(a\) of \(A\) does the relation \(m \prec  a\) hold.
\end{definition}
