\chapteropening[Applications to Group Theory]{Applications to Group Theory}
...
\begin{center}
\begin{tabular}{@{}cc@{}}
\includegraphics[width=0.3\linewidth]{images/83.1}\label{fig:83.1} & \includegraphics[width=0.2\linewidth]{images/83.2}\label{fig:83.2}\\
\textbf{\textsl{Figure 83.1}} & \textbf{\textsl{Figure 83.2}}\\
\end{tabular}
\end{center}
\begin{definition}
...
A linear graph consisting of a single edge is a tree. The graph in \hyperref[fig:84.2]{Figure 84.2} is not a tree, but deletion of the edge \(e\) would make it a tree. The graph in \hyperref[fig:84.3]{Figure 84.3} is a tree; deletion of the edge \(A\) would leave a tree remaining.
 ...
 \begin{center}
  \begin{tabular}{c@{\hspace{3em}}c}
   \includegraphics[width=0.45\linewidth]{images/84.2}\label{fig:84.2} &
   \includegraphics[width=0.45\linewidth]{images/84.3}\label{fig:84.3} \\[1em]
...
It follows that \({\pi }_{1}( {X,x_{0}})\) is generated by \(\hat{\delta }[ {\alpha * \beta }]\), where \(\delta\) is the path \(\bar{f}_{1} * \bar{\gamma }_{0}\) from \(a\) to \(x_{0}\). We compute this path-homotopy class as follows:

\[
\hat{\delta }[ {\alpha * \beta }] = [ {{\gamma }_{0} * f_{1}}] * [ {\alpha * \beta }] * [ {\bar{f}_{1} * \bar{\gamma }_{0}}]
\]
