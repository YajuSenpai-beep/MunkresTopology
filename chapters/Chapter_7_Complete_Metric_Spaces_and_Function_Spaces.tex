...
\item (a) If \({\mathbb{R}}^{\omega }\) is given the product topology, show there is no continuous surjective map \(f : \mathbb{R} \rightarrow  {\mathbb{R}}^{\omega }\) . [Hint: Show that \({\mathbb{R}}^{\omega }\) is not a countable union of compact subspaces.]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item (a) Let \(X\) be a Hausdorff space. Show that if there is a continuous surjective map \(f : I \rightarrow  X\) , then \(X\) is compact, connected, weakly locally connected, and metrizable. [Hint: Show \(f\) is a perfect map.]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\end{proof}
\begin{definition}
	If(Y, d)is a metric space, a subset \(\mathcal{F}\) of \(\mathcal{C}\left( {X,Y}\right)\) is said to be \idx{Pointwise convergence topology (see also Point-open topology)} pointwise bounded under \(d\) if for each \(x \in  X\) , the subset
...
\item (a) Let \({f}_{n} : I \rightarrow  \mathbb{R}\) be the function \({f}_{n}\left( x\right)  = {x}^{n}\) . The collection \(\mathcal{F} = \left\{  {f}_{n}\right\}\) is pointwise bounded but the sequence \(\left( {f}_{n}\right)\) has no uniformly convergent subsequence; at what point or points does \(\mathcal{F}\) fail to be equicontinuous?
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Consider the sequence of functions \({f}_{n} : \left( {-1,1}\right)  \rightarrow  \mathbb{R}\) , defined by \[ {f}_{n}\left( x\right)  = \mathop{\sum }\limits_{{k = 1}}^{n}k{x}^{k} \]
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \(\left( {f}_{n}\right)\) converges in the topology of compact convergence; conclude that the lumit function is continuous. (This is a standard fact about power senes.)
