...
Before defining the fundamental group of a space \(X\) , we shall consider paths on \(X\) and an equivalence relation called path \index{homotopy} between them. And we shall define a certain operation on the collection of the equivalence classes that makes it into what is called in algebra a groupoid.
...
for each \(x\) . (Here \(I = [  {0,1}]\) .) The map \(F\) is called a \index{Homotopy}\index{Homotopy}homotopy between \(f\) and \({f}^{\prime }\) . If \(f\) is homotopic to \({f}^{\prime }\) , we write \(f \simeq  {f}^{\prime }\) . If \(f \simeq  {f}^{\prime }\) and \({f}^{\prime }\) is a constant map, we say that \(f\) is \index{Nulhomotopic map} nulhomotopic.
\end{definition}
...
for each \(s \in  I\) and each \(t \in  I\) . We call \(F\) a \index{Path homotopy}\index{Path homotopy}path homotopy between \(f\) and \({f}^{\prime }\) See Figure 51.1. If \(f\) is path homotopic to \({f}^{\prime }\) , we write \(f{ \simeq  }_{p}{f}^{\prime }\) .
\end{definition}
...
\begin{definition}
	The projective plane \(\index{P@\({P}^{2}\)} \index{P@${P}^{2}$}\index{P@${P}^{2}$}{P}^{2}\) is the quotient space obtained from \({S}^{2}\) by identifying each point \(x\) of \({S}^{2}\) with its antipodal point \(- x\) .
\end{definition}
