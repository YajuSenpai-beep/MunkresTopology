...
(2) (Right and left identities) Given \(x \in X\), let \( {e}_{x}\) denote the constant path \({e}_{x} : I \rightarrow X\) carrying all of \(I\) to the point \(x\). If \(f\) is a path in \(X\) from \({x}_{0}\) to \({x}_{1}\), then
