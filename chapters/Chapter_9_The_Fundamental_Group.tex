...
for each \(x\) . (Here \(I = [  {0,1}]\) .) The map \(F\) is called a \index{homotopy@Homotopy}homotopy between \(f\) and \({f}^{\prime }\) . If \(f\) is homotopic to \({f}^{\prime }\) , we write \(f \simeq  {f}^{\prime }\) . If \(f \simeq  {f}^{\prime }\) and \({f}^{\prime }\) is a constant map, we say that \(f\) is \index{nulhomotopic map@Nulhomotopic map} nulhomotopic.
\end{definition}
...
for each \(s \in  I\) and each \(t \in  I\) . We call \(F\) a \index{path homotopy@Path homotopy}path homotopy between \(f\) and \({f}^{\prime }\) See Figure 51.1. If \(f\) is path homotopic to \({f}^{\prime }\) , we write \(f{ \simeq  }_{p}{f}^{\prime }\) .
\end{definition}
...
(3) (Inverse) Given the path \(f\) in \(X\) from \({x}_{0}\) to \({x}_{1}\) , let \(\bar{f}\) be the path defined by \(\bar{f}( s)  = f( {1 - s})\) . It is called the \index{Reverse of a path}reverse of \(f\) Then
...
The set of path-homotopy classes of paths in a space \(X\) does not form a group under the operation \(*\) because the product of two path-homotopy classes is not always defined. But suppose we pick out a point \({x}_{0}\) of \(X\) to serve as a \index{Base point choice!effect on \({\pi }_{1}\)} ``\index{Base point}'' and restrict ourselves to those paths that begin and end at \({x}_{0}\) The set of these path-homotopy classes does form a group under \(*\) . It will be called the fundamental group of \(X\) .
...
An immediate question one asks is the extent to which the fundamental group depends on the \index{Base point choice}base point. We consider that question now.
...
\index{Topological group!\({\pi}_{1}\) is abelian}It turns out that the isomorphism of \({\pi }_{1}( {X,{x}_{0}})\) with \({\pi }_{1}( {X,{x}_{1}})\) \index{alpha@\(\widehat{\alpha}\)!independent of path}is independent of path if and only if the \index{Fundamental group!when abelian}fundamental group is abelian. (See Exercise 3.) This is a stringent requirement on the space \(X\) .
...
It is intuitively clear that the fundamental group is a topological invariant of the space \(X\) . A convenient way to prove this fact formally is to introduce the notion of the \index{Homomorphism!induced by a map (see also \({h}_{ * }\))}``homomorphism induced by a continuous map.''
...
for the first homomorphism and \({( {h}_{{x}_{1}}) }_{ * }\) for the second. If there is only one \index{Base point choice!effect on \({h}_{ * }\)}base point under consideration, we shall omit mention of the base point and denote the induced homomorphism merely by \({h}_{ * }\) .
...
\item A subset \(A\) of \({\mathbb{R}}^{n}\) is said to be \index{star-convex set@Star-convex set} star convex if for some point \({a}_{0}\) of \(A\) , all the line segments joining \({a}_{0}\) to other points of \(A\) lie in \(A\) .
  ...
  \item Show that if \(A\) is star convex, \(A\) is \index{Simply connected!star-convex set}simply connected.
  \end{enumerate}
...
If \(p : E \rightarrow  B\) is a covering map, then \(p\) is a \index{Covering map!is local homeomorphism}\index{Local homeomorphism} of \(E\) with \(B\) . That is, each point \(e\) of \(E\) has a neighborhood that is mapped homeomorphically by \(p\) onto an open subset of \(B\) . The condition that \(p\) be a local homeomorphism does not suffice, however, to ensure that \(p\) is a covering map, as the following example shows.
...
\item Let \(p : E \rightarrow  B\) be a covering map; let \(B\) be connected. Show that if \({p}^{-1}( {b}_{0})\) has \(k\) elements for some \({b}_{0} \in  B\) , then \({p}^{-1}( b)\) has \(k\) elements for every \(b \in  B\) . In such a case, \(E\) is called a \index{Covering space!k-fold}\index{k-fold covering@\(k\)-fold covering}\(k\) -fold covering of \(B\) .
\item Let \(q : X \rightarrow  Y\) and \(r : Y \rightarrow  Z\) be covering maps; let \(p = r \circ  q\) . Show that if \({r}^{-1}( z)\) is finite for each \(z \in  Z\) , then \(p\) is a \index{Composite: of functions!of covering maps}covering map.
...
\end{proof}
\begin{theorem}
	The \index{\({S}^{1}\)!fundamental group}\index{Fundamental group!of \({S}^{1}\)}fundamental group of \({S}^{1}\) is isomorphic to the additive group of integers.
\end{theorem}
...
\begin{proof}
	If \({S}^{1}\) were a retract of \({B}^{2}\) , then the homomorphism induced by inclusion \(j : {S}^{1} \rightarrow  {B}^{2}\) would be injective. But the fundamental group of \({S}^{1}\) is nontrivial and the \index{Fixed point theorem: for \({B}^{n}\)!for retract of \({B}^{2}\)}\index{B^n@\({B}^{n}\)!fundamental group}fundamental group of \({B}^{2}\) is trivial.
\end{proof}
\begin{lemma}
...
We have already seen that every continuous map \(f : [  {0,1}]   \rightarrow  [  {0,1}]\) has a \index{Fixed point theorem: for \({B}^{n}\)!for \([0,1]\)}fixed point (see Exercise 3 of \(§{24}\) ). The same is \index{Fixed point theorem: for \({B}^{n}\)!for \({B}^{2}\)}true for the ball \({B}^{2}\) , although the proof is deeper:
\end{proof}
...
\end{proof}
\begin{theorem}
	The inclusion map \(j : {S}^{n} \rightarrow  {\mathbb{R}}^{n + 1} - \mathbf{0}\) induces an \index{Fundamental group!of \({\mathbb{R}}^{n} - 0\)}isomorphism of fundamental groups.
\end{theorem}
...
\begin{theorem}
	Let \(A\) be a \index{Deformation retract!fundamental group of}deformation retract of \(X\) ; let \({x}_{0} \in  A\) . Then the inclusion map
...
It is straightforward to show that if \(f : X \rightarrow  Y\) is a homotopy equivalence of \(X\) with \(Y\) and \(h : Y \rightarrow  Z\) is a homotopy equivalence of \(Y\) with \(Z\) , then \(h \circ  f : X \rightarrow  Z\) is a homotopy equivalence of \(X\) with \(Z\) . It follows that the relation of homotopy equivalence is an equivalence relation. Two spaces that are homotopy equivalent are said to have the same \index{Homotopy equivalence!induces isomorphism of \({\pi }_{1}\)}homotopy type.

Note that if \(A\) is a \index{Homotopy equivalence!vs. deformation retraction}\index{Deformation retraction!vs. homotopy equivalence}deformation retract of \(X\) , then \(A\) has the same homotopy type as \(X\) . For let \(j : A \rightarrow  X\) be the inclusion mapping and let \(r : X \rightarrow  A\) be the retraction mapping. Then the composite \(r \circ  j\) equals the identity map of \(A\) , and the composite \(j \circ  r\) is by hypothesis homotopic to the identity map of \(X\) (and in fact each point of \(A\) remains fixed during the homotopy).
...
\item Recall that a space \(X\) is said to be contractible if the identity map of \(X\) to itself is nulhomotopic. Show that \(X\) is contractible if and only if \(X\) has the \index{Homotopy type!of contractible space}homotopy type of a one-point space.
...
\begin{theorem}
	If \(n \geq  2\) , the \(n\) -sphere \({S}^{n}\) is \index{Simply connected!\({S}^{n}\)}\index{\({S}^{n}\) (Unit sphere)!fundamental group}\index{Simply connected!of \({S}^{n}\)}simply connected.
\end{theorem}
...
\end{proof}
\begin{corollary}
	The \index{Fundamental group!of torus}fundamental group of the torus \(T = {S}^{1} \times  {S}^{1}\) is isomorphic to the group \(\mathbb{Z} \times  \mathbb{Z}\) .
\end{corollary}

Now we define a surface called the \index{projective n-space@Projective n-space} projective plane and compute its \index{Fundamental group!of \({P}^{2}\)}fundamental group.

\begin{definition}
	The projective plane \(\index{P@\({P}^{2}\)} \index{P@${P}^{2}$}{P}^{2}\) is the quotient space obtained from \({S}^{2}\) by identifying each point \(x\) of \({S}^{2}\) with its antipodal point \(- x\) .
\end{definition}
...
\end{proof}
\section*{Lemma 60.5. The \index{Figure-eight space!fundamental group}fundamental group of the figure eight is not abelian.}
...
We assert that \(f * g\) and \(g * f\) are not path homotopic, so that the \index{Figure-eight space!covering space}fundamental group \index{Covering space!of figure eight}of the figure eight is not abelian.
...
We shall prove later that the \index{Fundamental group!of figure eight}fundamental group of the figure eight is, in fact, the group that algebraists call the ``free group on two generators.''
\end{proof}
