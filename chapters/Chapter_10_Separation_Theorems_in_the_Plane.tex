...
\end{proof}
\begin{lemma}[Nul{\idx[free homotopy]{Free homotopy of loops}} homotopy lemma]
...
An arc \(A\) is a space homeomorphic to the unit interval \(\left\lbrack  {0,1}\right\rbrack\) . The end points of \(A\) are the two points \(p\) and \(q\) of \(A\) such that \(A - p\) and \(A - q\) are connected; the other points of \(A\) are called \idx[interior point]{Interior point: of an arc} interior points of \(A\) .
...
\end{proof}
\begin{theorem}[A \idx{general separation theorem}]
...
\item Give examples to show that a simple closed curve in the \idx{Double torus} torus may or may not separate the torus.
...
\begin{lemma}[\idx{Homotopy extension lemma}]
...
\end{proof}
\begin{theorem}[A \idx{general nonseparation theorem}]
...
\end{proof}
\begin{definition}
	Let \(f\) be a loop in \(X\) . We call \(f\) a \idx{simple loop} provided \(f\left( s\right)  = f\left( {s}^{\prime }\right)\) only if \(s = {s}^{\prime }\) or if one of the points \(s,{s}^{\prime }\) is 0 and the other is 1 . If \(f\) is a simple loop, its image set is a simple closed curve in \(X\) .
\end{definition}
...
\end{proof}
\begin{definition}
	Let \(f\) be a simple loop in \({\mathbb{R}}^{2}\) . We say \(f\) is a \idx{counterclockwise loop} if \(n\left( {f,a}\right)  =  + 1\) for some \(a\) (and hence for every \(a\) ) in the bounded component of \({\mathbb{R}}^{2} - f\left( I\right)\) . We say it is a clockwise loop if \(n\left( {f,a}\right)  =  - 1\) . The standard loop \(p\left( s\right)  =\)  \(\left( {\cos {2\pi s},\sin {2\pi s}}\right)\) is thus a counterclockwise loop.
\end{definition}
