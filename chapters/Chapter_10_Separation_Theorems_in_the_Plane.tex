...
\end{proof}
\begin{lemma}[Nul{} homotopy lemma]
...
\end{proof}
\begin{definition}
	Let \(f\) be a simple loop in \({\mathbb{R}}^{2}\). We say \(f\) is a if \(n( {f,a}) = + 1\) for some \(a\) (and hence for every \(a\) ) in the bounded component of \({\mathbb{R}}^{2} - f( I)\). We say it is a clockwise loop if \(n( {f,a}) = - 1\). The standard loop \(p( s) =\) \(( {\cos {2\pi s},\sin {2\pi s}})\) is thus a counterclockwise loop.
\end{definition}
