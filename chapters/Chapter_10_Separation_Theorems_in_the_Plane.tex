...
Now we prove the \index{Jordan separation theorem}\index{Jordan separation theorem}\index{jordan separation theorem@Jordan separation theorem}Jordan separation theorem. In \index{Nonseparation theorem: arc in \({S}^{2}\)!general}\index{Nonseparation theorem: arc in \({S}^{2}\)!general}\index{Nonseparation theorem: arc in \({S}^{2}\)!general}general, if \(X\) is a connected space and \(A \subset  X\) , we say that \(A\) separates \(X\) if \(X - A\) is not connected; if \(X - A\) has \(n\) components, we say that \(A\) \index{separates a space@Separates a space}separates \(X\) \index{Separates a space!into \(n\) components}\index{Separates a space!into \(n\) components}\index{Separates a space!into \(n\) components}into n components.
...
First proof. Because \(D\) is contractible, the identity map i : \(D \rightarrow  D\) is nulhomo-topic. Hence if \(a\) and \(b\) are any two points of \({S}^{2}\) not in \(D\) , the inclusion \(j : D \rightarrow\)  \({S}^{2} - a - b\) is nulhomotopic. The \index{Borsuk lemma}\index{Borsuk lemma}\index{borsuk lemma@Borsuk lemma}Borsuk lemma then implies that \(a\) and \(b\) lie in the same component of \({S}^{2} - D\) .
...
Linear graphs are used in mathematics to model many real-life phenomena; however, we shall look at them simply as interesting spaces that in some sense are generalizations \index{Winding number!of simple closed curve}\index{Winding number!of simple closed curve}\index{Winding number!of simple closed curve}of simple closed curves.
...
\end{proof}
\begin{lemma}
	Let \(X\) be a subspace of \({S}^{2}\) that is a \index{Complete graph}\index{Complete graph}\index{complete graph@Complete graph}complete graph on four vertices \({a}_{1}\) , \({a}_{2},{a}_{3}\) , and \({a}_{4}\) . Then \(X\) separates \({S}^{2}\) into four components. The boundaries of these components are the sets \({X}_{1},{X}_{2},{X}_{3}\) , and \({X}_{4}\) , where \({X}_{i}\) is the union of those edges of \(X\) that do not have \({a}_{i}\) as a vertex.
\end{lemma}
...
\end{proof}
\begin{definition}
	Let \(f\) be a loop in \(X\) . We call \(f\) a \index{simple loop} provided \(f( s)  = f( {s}^{\prime })\) only if \(s = {s}^{\prime }\) or if one of the points \(s,{s}^{\prime }\) is 0 and the other is 1 . If \(f\) is a \index{Simple loop}\index{Simple loop}\index{simple loop@Simple loop}simple loop, its image set is a simple closed curve in \(X\) .
\end{definition}
