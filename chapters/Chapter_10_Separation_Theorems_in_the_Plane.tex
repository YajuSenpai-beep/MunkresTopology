...
For each integer \(n\), let \(e_{n}\) be the point \(\pi ( {a \times {2n}})\) of \(E\). Then the points \(e_{n}\) are distinct, and they constitute the set \(p^{-1}( a)\). We define a lifting \(\tilde{f}_{n}\) of \(f\) that begins at \(e_{n}\) and ends at \(e_{n + 1}\).

Since \(\alpha\) and \(\beta\) are paths in \(U\) and \(V\), respectively, we can define

\[
\begin{aligned} & \tilde{\alpha }_{n}( s) = \pi ( {\alpha ( s) \times {2n}}), \\ & \tilde{\beta }_{n}( s) = \pi ( {\beta ( s) \times ( {{2n} + 1}) }) ; \end{aligned}
\]

then \(\tilde{\alpha }_{n}\) and \(\tilde{\beta }_{n}\) are liftings of \(\alpha\) and \(\beta\), respectively. (The case \(n = 0\) is illustrated in \hyperref[fig:63.1]{Figure 63.1} ) The product \(\tilde{\alpha }_{n} * \tilde{\beta }_{n}\) is defined, since \(\tilde{\alpha }_{n}\) ends at \(\pi ( {b \times {2n}})\) and \(\tilde{\beta }_{n}\) begins at \(\pi ( {b \times ( {{2n} + 1}) })\). We set \(\tilde{f}_{n} = \tilde{\alpha }_{n} * \tilde{\beta }_{n}\), and note that \(\tilde{f}_{n}\) begins at \(\tilde{\alpha }_{n}( 0) = \pi ( {a \times {2n}}) = e_{n}\) and ends at \(\tilde{\beta }_{n}(1) = \pi ( {a \times ( {{2n} + 1}) }) = \pi ( {a \times ( {{2n} + 2}) }) = e_{n + 1}\).
...
\[
\tilde{h} = \tilde{f}_{0} * ( {\tilde{f}_{1} * ( {\cdots * \tilde{f}_{m - 1}}) })
\]
...
Then \(\tilde{\gamma }\) and \(\tilde{\delta }\) are liftings of \(\gamma\) and \(\delta\). The product \(\tilde{g} = \tilde{\gamma } * \tilde{\delta }\) is defined, since \(\tilde{\gamma }\) ends at \(\pi ( {a^{\prime } \times 0})\) and \(\tilde{\delta }\) begins at \(\pi ( {a^{\prime } \times ( {-1}) })\) ; and it is a lifting of \(g\). Note that \(\tilde{g}\) is a loop in \(E\), for it begins and ends at \(\pi ( {a \times 0}) = \pi ( {a \times ( {-1}) }) = e_{0}\).
...
then \(g\) is a loop in \(S^{1}\). Let \(p : \mathbb{R} \rightarrow {S}^{1}\) be the standard covering map, and let \(\tilde{g}\) be a lifting of \(g\) to \(\mathbb{R}\). Because \(g\) is a loop, the difference \(\tilde{g}(1) - \tilde{g}( 0)\) is an integer. This integer is called the winding number of \(f\) with respect to \(a\), and is denoted \( n( {f,a})\).
\end{definition}
...
\begin{proof}
(a) To compute \(n( {\bar{f},a})\), one replaces \(s\) by \(1 - s\) throughout the definition. This has the effect of changing \(\tilde{g}(1) - \tilde{g}( 0)\) by a sign.
...
\begin{proof}
The proof is a simple exercise in computation. Let \(p : \mathbb{R} \rightarrow {S}^{1}\) be the standard covering map. Let \(r( s) = \parallel f( s) - a\parallel\) and \(g( s) = [ {f( s) - a}] /r( s)\). Let \(\tilde{g}\) be a lifting of \(g\) to \(\mathbb{R}\). Set \(\theta ( s) = {2\pi }\tilde{g}( s)\). Then \(f( s) - a = r( s) \exp ( {{i\theta }( s) })\), so that
\[
