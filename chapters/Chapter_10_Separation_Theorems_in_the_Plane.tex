...
Because \(\widetilde{h}\) begins at \({e}_{0}\) and ends at \({e}_{m}\), the class \([ h] = {[ f] }^{m}\) cannot be trivial.
...
then \(g\) is a loop in \({S}^{1}\). Let \(p : \mathbb{R} \rightarrow {S}^{1}\) be the standard covering map, and let \(\widetilde{g}\) be a lifting of \(g\) to \(\mathbb{R}\). Because \(g\) is a loop, the difference \(\widetilde{g}(1) - \widetilde{g}( 0)\) is an integer. This integer is called the winding number of \(f\) with respect to \(a\), and is denoted \( n( {f,a})\).
\end{definition}
...
\begin{proof}
(a) To compute \(n( {\bar{f},a})\), one replaces \(s\) by \(1 - s\) throughout the definition. This has the effect of changing \(\widetilde{g}(1) - \widetilde{g}( 0)\) by a sign.
