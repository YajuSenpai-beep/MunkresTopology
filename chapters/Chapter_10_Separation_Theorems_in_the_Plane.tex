...
There is a fourth theorem that is often considered along with these three separation theorems. It is called the , and it states that if \(C\) is a simple closed curve in \({S}^{2}\) and \(U\) and \(V\) are the components of \({S}^{2} - C\) , then \(\bar{U}\) and \(\bar{V}\) are each homeomorphic to the closed unit ball \({B}^{2}\) . A proof may be found in 		\cite{HallSpencer1955}.
...
then \(g\) is a loop in \({S}^{1}\) . Let \(p : \mathbb{R} \rightarrow {S}^{1}\) be the standard covering map, and let \(\widetilde{g}\) be a lifting of \(g\) to \({S}^{1}\) . Because \(g\) is a loop, the difference \(\widetilde{g}( 1) - \widetilde{g}( 0)\) is an integer. This integer is called the winding number of \(f\) with respect to \(a\) , and is denoted \( n( {f,a})\) .
\end{definition}
...
\end{proof}
\begin{definition}
	Let \(f\) be a loop in \(X\) . We call \(f\) a provided \(f( s) = f( {s}^{\prime })\) only if \(s = {s}^{\prime }\) or if one of the points \(s,{s}^{\prime }\) is 0 and the other is 1 . If \(f\) is a simple loop, its image set is a simple closed curve in \(X\) .
\end{definition}
