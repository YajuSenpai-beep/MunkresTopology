...
\begin{centeredblock}
 \begin{example}
\label{ex:\thesection.\theexample}
  If \(X\) is any set, the collection of all subsets of \(X\) is a topology on \(X\); it is called the \idx[discrete topology]{Discrete topology}. The collection consisting of \(X\) and \(\varnothing\) only is also a topology on \(X\) ; we shall call it the \idx[indiscrete topology]{Indiscrete topology}, or the \idx[trivial topology]{Trivial topology}.
 \end{example}
\end{centeredblock}
...
\booksection{14}{The \idx[Order Topology]{Order topology}}
...
\booksection{15}{The \idx[Product Topology]{Product topology} on \(X \times Y\)}
...
\booksection{16}{The \idx[Subspace Topology]{Subspace topology}}
...
Now that we have a few examples at hand, we can introduce some of the basic concepts associated with topological spaces. In this section, we treat the notions of \idx[closed set]{Closed set}, \idx[closure]{Closure} of a set, and \idx[limit point]{Limit point}. These lead naturally to consideration of a certain axiom for topological spaces called the Hausdorff axiom.
...
\[
\text{"}U\text{ is a \idx[neighborhood]{Neighborhood} of }x\text{."}
\]
...
Given \({X}^{ * }\), there is an equivalence relation on \(X\) of which the elements of \({X}^{ * }\) are the equivalence classes. One can think of \({X}^{ * }\) as having been obtained by ``identifying'' each pair of equivalent points. For this reason, the quotient space \({X}^{ * }\) is often called an \idx[identification space]{Identification space}, or a \idx[decomposition space]{Decomposition space}, of the space \(X\).
...
\item Recall that \({\mathbb{R}}_{K}\) denotes the real line in the \(K\) -topology. (See \(\S\ {13}\).) Let \(Y\) be the quotient space obtained from \({\mathbb{R}}_{K}\) by collapsing the set \(K\) to a point; let \(p : {\mathbb{R}}_{K} \rightarrow Y\) be the quotient map.
  ...
  \item Show that \(p \times p.{\mathbb{R}}_{K} \times {\mathbb{R}}_{K} \rightarrow Y \times Y\) is not a quotient map. [Hint: The \idx[diagonal]{Diagonal} is not closed in \(Y \times Y\), but its inverse image is closed in \({\mathbb{R}}_{K} \times {\mathbb{R}}_{K}\).]
  \end{enumerate}
\end{enumerate}
