...
\item Let \(X\) and \(Y\) be metric spaces with metrics \({d}_{X}\) and \({d}_{Y}\), respectively. Let \(f\) : \(X \rightarrow Y\) have the property that for every pair of points \({x}_{1},{x}_{2}\) of \(X\), \[ {d}_{Y}( {f( {x}_{1}),f( {x}_{2}) }) = {d}_{X}( {{x}_{1},{x}_{2}}) . \] Show that \(f\) is an imbedding. It is called an of \(X\) in \(Y\) .
