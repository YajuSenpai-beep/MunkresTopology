...
\item Find the boundary and the interior of each of the following subsets of \(\mathbb{R}^{2}\) :
  ...
  \item \(F = \{ x \times y \mid x \neq 0\) and \(y \leq 1/x\}\) *21. ( Kuratowski) Consider the collection of all subsets \(A\) of the topological space \(X\). The operations of closure \(A \rightarrow \bar{A}\) and complementation \(A \rightarrow X - A\) are functions from this collection to itself.
...
\label{ex:\thesection.\theexample}
 ...
 Let \(\mathbf{0}\) be the ``origin'' in \(\mathbb{R}^{\omega }\), that is, the point \((0,0,\ldots)\) each of whose coordinates is zero. In the box topology, \(\mathbf{0}\) belongs to \(A\) ; for if
