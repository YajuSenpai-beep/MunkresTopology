...
If \({\mathcal{B}}^{\prime }\) is the collection of all of the form
...
If \(X\) is a simply ordered set, there is a standard topology for \(X\), defined using the order relation. It is called the ; in this section, we consider it and study some of its properties.
...
(3) All intervals of the form \(( {a,{b}_{0}}]\), where \({b}_{0}\) is the largest element (if any) of \(X\) . The collection \(\mathcal{B}\) is a basis for a topology on \(X\), which is called the order topology.
\end{definition}
...
Sets of the first two types are called, and sets of the last two types are called .
\end{definition}
...
\begin{definition}
	Let \(X\) and \(Y\) be topological spaces. The on \(X \times Y\) is the topology having as basis the collection \(\mathcal{B}\) of all sets of the form \(U \times V\), where \(U\) is an open subset of \(X\) and \(V\) is an open subset of \(Y\) .
\end{definition}
...
\item A map \(f : X \rightarrow Y\) is said to be an if for every open set \(U\) of \(X\), the set \(f( U)\) is open in \(Y\) . Show that \({\pi }_{1} : X \times Y \rightarrow X\) and \({\pi }_{2} : X \times Y \rightarrow Y\) are open maps.
...
The interior of \(A\) is denoted by Int \(A\) and the closure of \(A\) is denoted by \(\mathrm{{Cl}}A\) or by \(\) . Obviously Int \(A\) is an open set and \(\bar{A}\) is a closed set; furthermore,
...
If \(A\) is a subset of the topological space \(X\), then \(x \in \widehat{A}\) if and only if every of \(x\) intersects \(A\) .
\end{proof}
...
The condition that finite point sets be closed is in fact weaker than the For example, the real line \(\mathbb{R}\) in the finite complement topology is not a Hausdorff space, but it is a space in which finite point sets are closed The condition that finite point sets be closed has been given a name of its own: it is called the \({T}_{1}{ax}\) - iom. (We shall explain the reason for this strange terminology in Chapter 4.) The \({T}_{1}\) axiom will appear in this book in a few exercises, and in just one theorem, which is the following:
\end{proof}
...
Each of them has a notion of continuity defined for it. Our general definition of continuity includes all these as special cases, this fact will be a consequence of general theorems we shall prove concerning continuous functions on and on metric spaces.
\end{example}
\end{centeredblock}
...
The maps \({f}_{1}\) and \({f}_{2}\) are called the of \(f\) .
...
it is called the ing associated with the index \(\beta\) .
...
\begin{definition}
	If \(X\) is a topological space, \(X\) is said to be metrizable if there exists a metric \(d\) on the set \(X\) that induces the topology of \(X\) . A metric space is a \(X\) together with a specific metric \(d\) that gives the topology of \(X\) .
\end{definition}
...
\begin{definition}
	Given \(\mathbf{x} = ( {{x}_{1},\ldots,{x}_{n}})\) in \({\mathbb{R}}^{n}\), we define the of \(\mathbf{x}\) by the equation
...
\item Let \(X\) be the subset of \({\mathbb{R}}^{\omega }\) consisting of all sequences \(\mathbf{x}\) such that \(\sum {x}_{i}^{2}\) converges. Then the formula \[ d( {\mathbf{x},\mathbf{y}}) = {[ \mathop{\sum }\limits_{{i = 1}}^{\infty }{( {x}_{i} - {y}_{i}) }^{2}] }^{1/2} \] defines a metric on \(X\) . (See Exercise 10.) On \(X\) we have the three topologies it inherits from the box, uniform, and product topologies on \({\mathbb{R}}^{\omega }\) . We have also the topology given by the metric \(d\), which we call the \({\ell }^{2}\) -topology. (Read ``little ell two.'')
 ...
 \item The set \[ H = \mathop{\prod }\limits_{{n \in {\mathbb{Z}}_{ + }}}[ {0,1/n}] \] is contained in \(X\), it is called the . Compare the four topologies that \(H\) inherits as a subspace of \(X\) .
 \end{enumerate}
...
Finally, we come to the notion of .
\end{proof}
...
\begin{definition}
	Let \(X\) and \(Y\) be topological spaces; let \(p : X \rightarrow Y\) be a surjective map. The map \(p\) is said to be a provided a subset \(U\) of \(Y\) is open in \(Y\) if and only if \({p}^{-1}( U)\) is open in \(X\) .
\end{definition}

This condition is stronger than continuity; some mathematicians call it ``.'' An equivalent condition is to require that a subset \(A\) of \(Y\) be closed in \(Y\) if and only if \({p}^{-1}( A)\) is closed in \(X\) . Equivalence of the two conditions follows from equation
...
Suppose \(Z\) is Hausdorff. Given distinct pomts of \({X}^{ * }\), their images under \(f\) are distinct and thus possess disjoint neighborhoods \(U\) and \(V\) . Then \({f}^{-1}( U)\) and \({f}^{-1}( V)\) are disjoint neighborhoods of the two given points of \({X}^{ * }\) .
\end{proof}
