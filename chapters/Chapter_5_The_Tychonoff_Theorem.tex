...
\item Let \(X\) be a space. Let \(\mathcal{D}\) be a collection of subsets of \(X\) that is maximal with respect to the finite intersection property
  ...
  \item Show that if \(X\) satisfies the \({T}_{1}\) axiom, there is at most one point beloriging to \(\mathop{\bigcap }\limits_{{D \in  \mathcal{D}}}\bar{D}\) .
  \end{enumerate}
\item A collection \(\mathcal{A}\) of subsets of \(X\) has the \index{Countable intersection property}countable intersection property if every countable intersection of elements of \(\mathcal{A}\) is nonempty. Show that \(X\) is a Lindelöf space if and only if for every collection \(\mathcal{A}\) of subsets of \(X\) having the countable intersection property,
\end{enumerate}
...
\begin{proof}
	Let \({\left\{  {X}_{\alpha }\right\}  }_{\alpha  \in  J}\) be an indexed family of compact spaces, let
\[
X = \mathop{\prod }\limits_{{\alpha  \in  J}}{X}_{\alpha }
\]
...
\begin{definition}
	A compactification of a space \(X\) is a compact Hausdorff space \(Y\) containing \(X\) as a subspace such that \(\bar{X} = Y\) . Two compactifications \({Y}_{1}\) and \({Y}_{2}\) of \(X\) are said to be equivalent if there is a homeomorphism \(h : {Y}_{1} \rightarrow  {Y}_{2}\) such that \(h\left( x\right)  = x\) for every \(x \in  X\) .
\end{definition}
...
\begin{lemma}
	Let \(X\) be a space; suppose that \(h : X \rightarrow  Z\) is an imbedding of \(X\) in the compact Hausdorff space \(Z\) . Then there exists a corresponding compactification \(Y\) of \(X\) ; it has the property that there is an imbedding \(H : Y \rightarrow  Z\) that equals \(h\) on \(X\) . The compactification \(Y\) is uniquely determined up to \index{equivalence of compactifications@Equivalence of compactifications}Equivalence of compactifications equivalence.
\end{lemma}

We call \(Y\) the compactification induced by the imbedding \(h\) .

\begin{proof}
	Given \(h\) , let \({X}_{0}\) denote the subspace \(h\left( X\right)\) of \(Z\) , and let \({Y}_{0}\) denote its closure in \(Z\) . Then \({Y}_{0}\) is a compact Hausdorff space and \({\bar{X}}_{0} = {Y}_{0}\) ; therefore, \({Y}_{0}\) is a compactification of \({X}_{0}\) .
...
\begin{centeredblock}
\begin{example}
	Take the unit circle \({S}^{1}\) in \({\mathbb{R}}^{2}\) and let \(h.\left( {0,1}\right)  \rightarrow  {S}^{1}\) be the map
...
\begin{centeredblock}
\begin{example}
	Let \(Y\) be the space \(\left\lbrack  {0,1}\right\rbrack\) Then \(Y\) is a compacufication of \(X\) , it is obtained by ``adding one point at each end of(0,1)''
\end{example}
\end{centeredblock}

\begin{centeredblock}
\begin{example}
	Consider the square \({\left\lbrack  -1,1\right\rbrack  }^{2}\) in \({\mathbb{R}}^{2}\) and let \(h\;\left( {0,1}\right)  \rightarrow  {\left\lbrack  -1,1\right\rbrack  }^{2}\) be the map
...
\begin{centeredblock}
\begin{example}
	Let \(X = \left( {0,1}\right)\) Consider the one-point compactification of \(X\) given in Example I A bounded continuous function \(f : \left( {0,1}\right)  \rightarrow  \mathbb{R}\) is extendable to this compacutfication if and only if the limits
...
\begin{proof}
	Let \({\left\{  {f}_{\alpha }\right\}  }_{\alpha  \in  J}\) be the collection of all bounded continuous real-valued functions on \(X\) , indexed by some index set \(J\) For each \(\alpha  \in  J\) , choose a closed interval \({I}_{\alpha }\) in \(\mathbb{R}\) containing \({f}_{\alpha }\left( X\right)\) . To be definite, choose
\[
...
\item Show that the bounded continuous function \(g\left( {0,1}\right)  \rightarrow  \mathbb{R}\) defined by \(g\left( x\right)  =\)  \(\cos \left( {1/x}\right)\) cannot be extended to the compactification of Example 3. Define an imbedding \(h : \left( {0,1}\right)  \rightarrow  {\left\lbrack  0,1\right\rbrack  }^{3}\) such that the functions \(x,\sin \left( {1/x}\right)\) , and \(\cos \left( {1/x}\right)\) are all extendable to the compactification induced by \(h\) .
\item Under what conditions does a metrizable space have a metrizable compactification?
\item Let \(Y\) be an arbitrary compactification of \(X\) ; let \(\beta \left( X\right)\) be the Stone-Čech compactification. Show there is a continuous surjective closed map \(g : \beta \left( X\right)  \rightarrow  Y\) that equals the identity on \(X\) [This exercise makes precise what we mean by saying that \(\beta \left( X\right)\) is the ``maximal'' compactification of \(X\) . It shows that every compactification of \(X\) is equivalent to a quotient space of \(\beta \left( X\right)\) .]
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Let \(X\) be completely regular. Show that \(X\) is connected if and only if \(\beta \left( X\right)\) is connected. [Hint: If \(X = A \cup  B\) is a separation of \(X\) , let \(f\left( x\right)  = 0\) for \(x \in  A\) and \(f\left( x\right)  = 1\) for \(x \in  B\) .]
\item Let \(X\) be a discrete space; consider the space \(\beta \left( X\right)\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that if \(A \subset  X\) , then \(\bar{A}\) and \(\overline{X - A}\) are disjoint, where the closures are taken in \(\beta \left( X\right)\) .
  \item Show that if \(U\) is open in \(\beta \left( X\right)\) , then \(\bar{U}\) is open in \(\beta \left( X\right)\) .
  \item Show that \(\beta \left( X\right)\) is totally disconnected.
  \end{enumerate}
\item Show that \(\beta \left( {\mathbb{Z}}_{ + }\right)\) has cardinality at least as great as \({I}^{I}\) , where \(I = \left\lbrack  {0,1}\right\rbrack\) . [Hint: The space \({I}^{\prime }\) has a countable dense subset.]
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(X\) is normal and \(y\) is a point of \(\beta \left( X\right)  - X\) , show that \(y\) is not the limit of a sequence of points of \(X\) .

  \item Show that if \(X\) is completely regular and noncompact, then \(\beta \left( X\right)\) is not metrizable.
  \end{enumerate}
\item We have constructed a correspondence \(X \rightarrow  \beta \left( X\right)\) that assigns, to each completely regular space, its Stone-Čech compactification. Now let us assign, to each continuous map \(f : X \rightarrow  Y\) of completely regular spaces, the unique continuous map \(\beta \left( f\right)  : \beta \left( X\right)  \rightarrow  \beta \left( Y\right)\) that extends the map \(i \circ  f\) , where \(i : Y \rightarrow  \beta \left( Y\right)\) is the inclusion map. Verify the following:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \({1}_{X} : X \rightarrow  X\) is the identity map of \(X\) , then \(\beta \left( {1}_{X}\right)\) is the identity map of \(\beta \left( X\right)\) . (ii) If \(f : X \rightarrow  Y\) and \(g : Y \rightarrow  Z\) , then \(\beta \left( {g \circ  f}\right)  = \beta \left( g\right)  \circ  \beta \left( f\right)\) . These properties tell us that the correspondence we have constructed is what is called a functor; it is a functor from the ``category'' of completely regular spaces and continuous maps of such spaces, to the ``category'' of compact Hausdorff spaces and continuous maps of such spaces. You will see these properties again in Part II of the book; they are fundamental in algebra and in algebraic topology.
  \end{enumerate}
\end{enumerate}