...
of subsets of \({X}_{1}\) also has the finite intersection property, and so does the collection of their closures \(\overline{{\pi }_{1}\left( A\right) }\) . Compactness of \({X}_{1}\) guarantees that the intersection of all the sets \(\overline{{\pi }_{1}\left( A\right) }\) is nonempty. Let us choose a point \({x}_{1}\) beloriging to this intersection. Similarly, let us choose a point \({x}_{2}\) beloriging to all the sets \(\overline{{\pi }_{2}\left( A\right) }\) . The obvious conclusion we would like to draw is that the point \({x}_{1} \times {x}_{2}\) lies in \(\mathop{\bigcap }\limits_{{A \in A}}A\) , for then our theorem would be proved.
...
which is the union of the collections beloriging to \(\mathbb{B}\) , is an element of \(\mathbb{A}\) , then it is the required upper bound on \(\mathbb{B}\) .
...
(c) Show that if \(X\) satisfies the \({T}_{1}\) axiom, there is at most one point beloriging to \(\mathop{\bigcap }\limits_{{D \in \mathcal{D}}}\bar{D}\) .
