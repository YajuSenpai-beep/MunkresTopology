...
of subsets of \({X}_{1}\) also has the finite intersection property, and so does the collection of their closures \(\overline{{\pi }_{1}\left( A\right) }\) . Compactness of \({X}_{1}\) guarantees that the intersection of all the sets \(\overline{{\pi }_{1}\left( A\right) }\) is nonempty. Let us choose a point \({x}_{1}\) belonging to this intersection. Similarly, let us choose a point \({x}_{2}\) belonging to all the sets \(\overline{{\pi }_{2}\left( A\right) }\) . The obvious conclusion we would like to draw is that the point \({x}_{1} \times  {x}_{2}\) lies in \(\mathop{\bigcap }\limits_{{A \in  A}}A\) , for then our theorem would be proved.

But that is unfortunately not true. Consider the following example, in which \({X}_{1} =\)  \({X}_{2} = \left\lbrack  {0,1}\right\rbrack\) and the collection \(\mathcal{A}\) consists of all closed elliptical regions bounded by ellipses that have the points \(p = \left( {\frac{1}{3},\frac{1}{3}}\right)\) and \(q = \left( {\frac{1}{2},\frac{2}{3}}\right)\) as their foci. See Figure 37.1. Certainly \(\mathcal{A}\) has the finite intersection property. Now let us pick a point \({x}_{1}\) in the intersection of the sets \(\left\{  {\left. \overline{{\pi }_{1}\left( A\right) }\right| \;A \in  \mathcal{A}}\right\}\) Any point of the interval \(\left\lbrack  {\frac{1}{3},\frac{1}{2}}\right\rbrack\) will do; suppose we choose \({x}_{1} = \frac{1}{2}\) . Similarly, choose a point \({x}_{2}\) in the intersection of the sets \(\left\{  {\overline{{\pi }_{2}\left( A\right) } \mid  A \in  \mathcal{A}}\right\}\) . Any point of the interval \(\left\lbrack  {\frac{1}{3},\frac{2}{3}}\right\rbrack\) will do; suppose we pick \({x}_{2} = \frac{1}{2}\) . This proves to be an unfortunate choice, for the point
...
A final remark. The assumption that the elements of the collection \(\mathcal{A}\) were closed sets was irrelevant in this discussion. For even if the set \(A \in  \mathcal{A}\) is closed, the set \({\pi }_{1}\left( A\right)\) need not be closed, so we had to take its closure in order to apply the closed set formulation of compactness. Therefore, we may as well begin with an arbitrary collection of subsets of \(X\) having the finite intersection property, and prove that the intersection of their closures is nonempty. This approach actually proves to be more convenient.

\begin{lemma} Let \(X\) be a set; let \(\mathcal{A}\) be a collection of subsets of \(X\) having the finite intersection property. Then there is a collection \(\mathcal{D}\) of subsets of \(X\) such that \(\mathcal{D}\) contains \(\mathcal{A}\) , and \(\mathcal{D}\) has the finite intersection property, and no collection of subsets of \(X\) that properly contains \(\mathcal{D}\) has this property.
\end{lemma}
...
\begin{proof}As you might expect, we construct \(\mathcal{D}\) by using Zorn’s lemma. It states that, given a set \(A\) that is strictly partially ordered, in which every simply ordered subset has an upper bound, \(A\) itself has a maximal element.
The set \(A\) to which we shall apply Zorn’s lemma is not a subset of \(X\) , nor even a collection of subsets of \(X\) , but a set whose elements are collections of subsets of \(X\) . For purposes of this proof, we shall call a set whose elements are collections of subsets of \(X\) a "\idx{superset}" and shall denote it by an outline letter. To summarize the rotation:
...
\end{proof}
\begin{lemma} Let \(X\) be a set; let \(\mathcal{D}\) be a collection of subsets of \(X\) that is maximal with respect to the finite intersection property. Then:
...
\end{proof}
\begin{theorem}[Tychonoff theorem] An arbitrary product of compact spaces is compact in the product topology
\end{theorem}
...
2. A collection \(\mathcal{A}\) of subsets of \(X\) has the \idx{countable intersection property} if every countable intersection of elements of \(\mathcal{A}\) is nonempty. Show that \(X\) is a Lindelöf space if and only if for every collection \(\mathcal{A}\) of subsets of \(X\) having the countable intersection property,
...
\begin{theorem} Let \(X\) be a compact Hausdorff space. Then \(x\) and \(y\) belong to the same quasicomponent of \(X\) if and only if they belong to the same component of \(X\) .
\end{theorem}
...
\begin{lemma} Let \(\mathcal{A}\) be a collection of basis elements for the topology of the product space \(X \times  Y\) , such that no finite subcollection of \(\mathcal{A}\) covers \(X \times  Y\) . If \(X\) is compact, there is a point \(x \in  X\) such that no finite subcollection of \(\mathcal{A}\) covers the slice \(\{ x\}  \times  Y\) .
\end{lemma}

\begin{theorem} An arbitrary product of compact spaces is compact in the product topology.
\end{theorem}

\begin{proof}Let \({\left\{  {X}_{\alpha }\right\}  }_{\alpha  \in  J}\) be an indexed family of compact spaces, let
\[
X = \mathop{\prod }\limits_{{\alpha  \in  J}}{X}_{\alpha }
\]
...
then \(\mathcal{A}\) actually covers \({Y}_{\alpha }\) for some \(\alpha  < \beta\) . [Hint. If \(\beta\) has an immediate predecessor in \(J\) , let \(\alpha\) be that immediate predecessor Otherwise, for each \(A \in  \mathcal{A}\) , let \({J}_{A}\) denote the set of those indices \(i < \beta\) for which \({\pi }_{i}\left( A\right)  \neq  {X}_{i}\) ; the union of the sets \({J}_{A}\) , for \(A \in  \mathcal{A}\) , is finite; let \(\alpha\) be the largest element of this union.]
...
\begin{definition} A compactification of a space \(X\) is a compact Hausdorff space \(Y\) containing \(X\) as a subspace such that \(\bar{X} = Y\) . Two compactifications \({Y}_{1}\) and \({Y}_{2}\) of \(X\) are said to be equivalent if there is a homeomorphism \(h : {Y}_{1} \rightarrow  {Y}_{2}\) such that \(h\left( x\right)  = x\) for every \(x \in  X\) .
\end{definition}
...
\begin{lemma} Let \(X\) be a space; suppose that \(h : X \rightarrow  Z\) is an imbedding of \(X\) in the compact Hausdorff space \(Z\) . Then there exists a corresponding compactification \(Y\) of \(X\) ; it has the property that there is an imbedding \(H : Y \rightarrow  Z\) that equals \(h\) on \(X\) . The compactification \(Y\) is uniquely determined up to \idx{Equivalence of compactifications} equivalence.
\end{lemma}

We call \(Y\) the compactification induced by the imbedding \(h\) .

\begin{proof}Given \(h\) , let \({X}_{0}\) denote the subspace \(h\left( X\right)\) of \(Z\) , and let \({Y}_{0}\) denote its closure in \(Z\) . Then \({Y}_{0}\) is a compact Hausdorff space and \({\bar{X}}_{0} = {Y}_{0}\) ; therefore, \({Y}_{0}\) is a compactification of \({X}_{0}\) .
...
Now suppose \({Y}_{i}\) is a compactification of \(X\) and that \({H}_{i} : {Y}_{i} \rightarrow  Z\) is an imbedding that is an extension of \(h\) , for \(i = 1,2\) . Now \({H}_{i}\) maps \(X\) onto \(h\left( X\right)  = {X}_{0}\) Because \({H}_{t}\) is continuous, it must map \({Y}_{t}\) into \({\bar{X}}_{0}\) ; because \({H}_{i}\left( {Y}_{i}\right)\) contains \({X}_{0}\) and is closed (being compact), it contains \({\bar{X}}_{0}\) . Hence \({H}_{i}\left( {Y}_{i}\right)  = {\bar{X}}_{0}\) , and \({H}_{2}^{-1} \circ  {H}_{1}\) defines a homeomorphism of \({Y}_{1}\) with \({Y}_{2}\) that equals the identity on \(X\) .
...
\begin{example} Take the unit circle \({S}^{1}\) in \({\mathbb{R}}^{2}\) and let \(h.\left( {0,1}\right)  \rightarrow  {S}^{1}\) be the map
...
\begin{example} Let \(Y\) be the space \(\left\lbrack  {0,1}\right\rbrack\) Then \(Y\) is a compacufication of \(X\) , it is obtained by "adding one point at each end of(0,1)"
\end{example}

\begin{example} Consider the square \({\left\lbrack  -1,1\right\rbrack  }^{2}\) in \({\mathbb{R}}^{2}\) and let \(h\;\left( {0,1}\right)  \rightarrow  {\left\lbrack  -1,1\right\rbrack  }^{2}\) be the map

\[
h\left( x\right)  = x \times  \sin \left( {1/x}\right) .
\]

The space \({Y}_{0} = \overline{h\left( X\right) }\) is the topologist’s sine curve (see Example 7 of §24). The imbedding \(h\) gives rise to a compactification of(0,1)quite different from the other two. It is obtained by adding one point at the right-hand end of(0,1), and an entire line segment of points at the left-hand end \({}^{1}\)
\end{example}
...
\begin{example} Let \(X = \left( {0,1}\right)\) Consider the one-point compactification of \(X\) given in Example I A bounded continuous function \(f : \left( {0,1}\right)  \rightarrow  \mathbb{R}\) is extendable to this compacutfication if and only if the limits
...
\end{proof}
\begin{theorem} Let \(X\) be a completely regular space. There exists a compactification \(Y\) of \(X\) having the property that every bounded continuous map \(f : X \rightarrow  \mathbb{R}\) extends uniquely to a continuous map of \(Y\) into \(\mathbb{R}\) .
\end{theorem}

\begin{proof}Let \({\left\{  {f}_{\alpha }\right\}  }_{\alpha  \in  J}\) be the collection of all bounded continuous real-valued functions on \(X\) , indexed by some index set \(J\) For each \(\alpha  \in  J\) , choose a closed interval \({I}_{\alpha }\) in \(\mathbb{R}\) containing \({f}_{\alpha }\left( X\right)\) . To be definite, choose
\[
...
\end{proof}
\begin{lemma} Let \(A \subset  X\) ; let \(f : A \rightarrow  Z\) be a continuous map of \(A\) into the Hausdorff space \(Z\) . There is at most one extension of \(f\) to a continuous function \(g : \bar{A} \rightarrow  Z\) .
\end{lemma}

\begin{proof}This lemma was given as an exercise in \(\$ {18}\) ; we give a proof here. Suppose that \(g,{g}^{\prime } \cdot  \bar{A} \rightarrow  X\) are two different extensions of \(f\) , choose \(x\) so that \(g\left( x\right)  \neq  {g}^{\prime }\left( x\right)\) . Let \(U\) and \({U}^{\prime }\) be disjoint neighborhoods of \(g\left( x\right)\) and \({g}^{\prime }\left( x\right)\) , respectively. Choose a neighborhood \(V\) of \(x\) so that \(g\left( V\right)  \subset  U\) and \({g}^{\prime }\left( V\right)  \subset  {U}^{\prime }\) Now \(V\) intersects \(A\) in some point \(y\) ; then \(g\left( y\right)  \in  U\) and \({g}^{\prime }\left( y\right)  \in  {U}^{\prime }\) . But since \(y \in  A\) , we have \(g\left( y\right)  = f\left( y\right)\) and \({g}^{\prime }\left( y\right)  = f\left( y\right)\) . This contradicts the fact that \(U\) and \({U}^{\prime }\) are disjoint.\end{proof}
\begin{theorem} Let \(X\) be a completely regular space; let \(Y\) be a compactification of \(X\) satisfying the extension property of Theorem 38.2 Given any continuous map \(f : X \rightarrow  C\) of \(X\) into a compact Hausdorff space \(C\) , the map \(f\) extends uniquely to a continuous map \(g \cdot  Y \rightarrow  C\) .
\end{theorem}

\begin{proof}Note that \(C\) is completely regular, so that it can be imbedded in \({\left\lbrack  0,1\right\rbrack  }^{J}\) for some \(J\) So we may as well assume that \(C \subset  \{ 0,1{\} }^{J}\) . Then each component function \({f}_{\alpha }\) of the map \(f\) is a bounded continuous real-valued function on \(X\) , by hypothesis, \({f}_{\alpha }\) can be extended to a continuous map \({g}_{\alpha }\) of \(Y\) into \(\mathbb{R}\) . Define \(g : Y \rightarrow  {\mathbb{R}}^{J}\) by setting \(g\left( y\right)  = {\left( {g}_{\alpha }\left( y\right) \right) }_{\alpha  \in  J}\) ; then \(g\) is continuous because \({\mathbb{R}}^{J}\) has the product topology. Now in fact \(g\) maps \(Y\) into the subspace \(C\) of \({\mathbb{R}}^{J}\) . For continuity of \(g\) implies that
\[
...
\end{proof}
\begin{theorem} Let \(X\) be a completely regular space. If \({Y}_{1}\) and \({Y}_{2}\) are two compact-ifications of \(X\) satisfying the extension property of Theorem 38.2, then \({Y}_{1}\) and \({Y}_{2}\) are equivalent.
\end{theorem}
...
8. Show that \(\beta \left( {\mathbb{Z}}_{ + }\right)\) has cardinality at least as great as \({I}^{I}\) , where \(I = \left\lbrack  {0,1}\right\rbrack\) . [Hint: The space \({I}^{\prime }\) has a countable dense subset.]
