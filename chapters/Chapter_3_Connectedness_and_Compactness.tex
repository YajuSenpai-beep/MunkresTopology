...
\begin{theorem}
	The \index{Topologist's sine curve (cont.): connectedness!path components}path components of \(X\) are path-connected disjoint subspaces of \(X\) whose union is \(X\) , such that each nonempty path-connected subspace of \(X\) intersects only one of them.
...
\begin{centeredblock}
\begin{example}
	The ``\index{topologist's sine curve (cont.): connectedness@Topologist's sine curve (cont.): connectedness} topologist's sine curve'' \(\bar{S}\) of the preceding section is a space that has a single component (since it is connected) and two path components One path component is the curve \(S\) and the other is the vertical interval \(V = 0 \times  [  {-1,1}]\) . Note that \(S\) is open in \(\bar{S}\) but not closed, while \(V\) is closed but not open
...
\item Let \(G\) be a \index{topological group@Topological group}topological group.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Let \(A\) and \(B\) be subspaces of \(G\) . If \(A\) is closed and \(B\) is compact, show \(A \cdot  B\) is closed. [Hint: If \(c\) is not in \(A \cdot  B\) , find a neighborhood \(W\) of \(c\) such that \(W \cdot  {B}^{-1}\) is disjoint from \(A\) .]
