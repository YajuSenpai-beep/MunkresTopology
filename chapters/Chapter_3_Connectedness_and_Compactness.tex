...
In the study of calculus, there are three basic \index{intermediate-value theorem@Intermediate-value theorem}Intermediate-value theorem theorems about \index{continuous image: of a compact space@Continuous image: of a compact space}continuous image continuous functions, and on these theorems the rest of calculus depends. They are the following:
...
\index{maximum value theorem: of calculus@Maximum value theorem: of calculus}maximum value theorem Maximum value theorem. If \(f : \left\lbrack  {a,b}\right\rbrack   \rightarrow  R\) is continuous, then there exists an element \(c \in  \left\lbrack  {a,b}\right\rbrack\) such that \(f\left( x\right)  \leq  f\left( c\right)\) for every \(x \in  \left\lbrack  {a,b}\right\rbrack\) .

\index{uniform continuity theorem@Uniform continuity theorem}Uniform continuity theorem. If \(f : \left\lbrack  {a,b}\right\rbrack   \rightarrow  \mathbb{R}\) is continuous, then given \(\epsilon  > 0\) , there exists \(\delta  > 0\) such that \(\left| {f\left( {x}_{1}\right)  - f\left( {x}_{2}\right) }\right|  < \epsilon\) for every pair of numbers \({x}_{1},{x}_{2}\) of \(\left\lbrack  {a,b}\right\rbrack\) for which \(\left| {{x}_{1} - {x}_{2}}\right|  < \delta\) .
...
We have spoken of these three theorems as theorems about continuous functions. But they can also be considered as theorems about the \index{closed graph theorem@Closed graph theorem}Closed graph theorem closed interval \(\left\lbrack  {a,b}\right\rbrack\) of real numbers. The theorems depend not only on the continuity of \(f\) but also on properties of the topological \index{homogeneous space@Homogeneous space}Homogeneous space space \(\left\lbrack  {a,b}\right\rbrack\) .

The property of the space \(\left\lbrack  {a,b}\right\rbrack\) on which the intermediate value theorem depends is the property called \index{Connectedness}connectedness, and the property on which the other two depend is the property called \index{Compactness}compactness. In this chapter, we shall define these properties for arbitrary topological spaces, and shall prove the appropriate \index{general linear group@General linear group}General linear group generalized versions of these theorems.
...
\begin{definition}
	Let \(X\) be a topological space. A \index{Separation}separation of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a separation of \(X\) .
\end{definition}
...
\end{theorem}
\begin{proof}
	Let \(f : X \rightarrow  Y\) be a continuous map; let \(X\) be connected. We wish to prove the image space \(Z = f\left( X\right)\) is connected. Since the map obtained from \(f\) by restricting its range to the space \(Z\) is also continuous, it suffices to consider the case of a continuous surjective map
...
\item Let \(\left\{  {A}_{n}\right\}\) be a sequence of connected subspaces of \(X\) , such that \({A}_{n} \cap  {A}_{n + 1} \neq  \varnothing\) for all \(n\) . Show that \(\bigcup {A}_{n}\) is connected.
\item Let \(\left\{  {A}_{\alpha }\right\}\) be a collection of connected subspaces of \(X\) ; let \(A\) be a connected subspace of \(X\) . Show that if \(A \cap  {A}_{\alpha } \neq  \varnothing\) for all \(\alpha\) , then \(A \cup  \left( {\bigcup {A}_{\alpha }}\right)\) is connected.
\item Show that if \(X\) is an infinite set, it is connected in the finite complement topology.
\item A space is \index{Totally disconnected}totally disconnected if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is totally disconnected. Does the converse hold?
...
\item Let \(A\) be a proper subset of \(X\) , and let \(B\) be a proper subset of \(Y\) . If \(X\) and \(Y\) are connected, show that \[ \left( {X \times  Y}\right)  - \left( {A \times  B}\right) \] is connected.
\item Let \({\left\{  {X}_{\alpha }\right\}  }_{\alpha  \in  J}\) be an indexed family of connected spaces; let \(X\) be the product space \[ X = \mathop{\prod }\limits_{{\alpha  \in  J}}{X}_{\alpha } \] Let \(\mathbf{a} = \left( {a}_{\alpha }\right)\) be a \index{Fixed point}fixed point of \(X\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Given any finite subset \(K\) of \(J\) , let \({X}_{K}\) denote the subspace of \(X\) consisting of all points \(\mathbf{x} = \left( {x}_{\alpha }\right)\) such that \({x}_{\alpha } = {a}_{\alpha }\) for \(\alpha  \notin  K\) . Show that \({X}_{K}\) is connected.
  ...
  \end{enumerate}
\item Let \(p : X \rightarrow  Y\) be a quotient map. Show that if each set \({p}^{-1}\left( {\{ y\} }\right)\) is connected, and if \(Y\) is connected, then \(X\) is connected.
...
One application is the intermediate value theorem of calculus, suitably generalized. Another is the result that such familiar spaces as balls and spheres in euclidean space are connected; the proof involves a new notion, called \index{Path connectedness}path connectedness, which we also discuss.
...
\begin{theorem}[Intermediate value theorem]
	Let \(f : X \rightarrow  Y\) be a continuous map, where \(X\) is a connected space and \(Y\) is an ordered set in the order topology. If a and \(b\) are two points of \(X\) and if \(r\) is a point of \(Y\) lying between \(f\left( a\right)\) and \(f\left( b\right)\) , then there exists a point \(c\) of \(X\) such that \(f\left( c\right)  = r\) .
...
\begin{centeredblock}
\begin{example}
	Define the unit ball \(\index{B^n@\({B}^{n}\)} {B}^{n}\) in \({\mathbb{R}}^{n}\) by the equation
...
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that no two of the spaces \(\left( {0,1}\right) ,(0,1\rbrack\) , and \(\left\lbrack  {0,1}\right\rbrack\) are homeomorphic. [Hint: What happens if you remove a point from each of these spaces?)]
  ...
  \end{enumerate}
\item Let \(f : {S}^{1} \rightarrow  \mathbb{R}\) be a continuous map. Show there exists a point \(x\) of \({S}^{1}\) such that \(f\left( x\right)  = f\left( {-x}\right)\) .
\item Let \(f : X \rightarrow  X\) be continuous. Show that if \(X = \left\lbrack  {0,1}\right\rbrack\) , there is a point \(x\) such that \(f\left( x\right)  = x\) . The point \(x\) is called a fixed point of \(f\) . What happens if \(X\) equals \(\lbrack 0,1)\) or \(\left( {0,1}\right) {}^{7}\)
...
\item Consider the following sets in the dictionary order. Which are linear continua?
  ...
  \item \(\lbrack 0,1) \times  \left\lbrack  {0,1}\right\rbrack\)
  \item \(\left\lbrack  {0,1}\right\rbrack   \times  \lbrack 0,1)\)
  \end{enumerate}
...
\item
  ...
  \item Let \(X = Y = {\overline{\mathbb{R}}}_{ + }\) . Given a positive integer \(n\) , show that the function \(f\left( x\right)  =\)  \({x}^{n}\) is order preserving and surjective. Conclude that its inverse, the nth root function, is continuous.
  \item Let \(X\) be the subspace \(\left( {-\infty , - 1}\right)  \cup  \lbrack 0,\infty )\) of \(\mathbb{R}\) . Show that the function \(f : X \rightarrow  \mathbb{R}\) defined by setting \(f\left( x\right)  = x + 1\) if \(x <  - 1\) , and \(f\left( x\right)  = x\) if \(x \geq  0\) , is order preserving and surjective. Is \(f\) a homeomorphism? Compare with (a).
  \end{enumerate}
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item If \(f : X \rightarrow  Y\) is continuous and \(X\) is path connected, is \(f\left( X\right)\) necessarily path connected?
  \item If \(\left\{  {A}_{\alpha }\right\}\) is a collection of path-connected subspaces of \(X\) and if \(\bigcap {A}_{\alpha } \neq  \varnothing\) , is \(\bigcup {A}_{\alpha }\) necessarily path connected?
  \end{enumerate}
...
\item *12. Recall that \({S}_{\Omega }\) denotes the minimal uncountable well-ordered set. Let \(L\) denote the ordered set \({S}_{\Omega } \times  \lbrack 0,1)\) in the dictionary order, with its smallest element deleted. The set \(L\) is a classical example in topology called the \index{Long line}long line. \textbf{Theorem.} The long line is path connected and \index{local path connectedness@Local path connectedness}Local path connectedness locally homeomorphic to \(\mathbb{R}\) , but it cannot be imbedded in \(\mathbb{R}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Let \(X\) be an ordered set. Let \({x}_{0} < {x}_{1} < \cdots\) be an increasing sequence of points of \(X\) ; suppose \(b = \sup \left\{  {x}_{t}\right\}\) Show that \(\left\lbrack  {{x}_{0},b}\right)\) has the order type of \(\lbrack 0,1)\) if and only if each interval \(\left\lbrack  {{x}_{i},{x}_{i + 1}}\right)\) has the order type of \(\lbrack 0,1)\) .
  ...
  \item Show that every point of \(L\) has a \index{symmetric neighborhood@Symmetric neighborhood}Symmetric neighborhood neighborhood homeomorphic with an open interval in \(\mathbb{R}\) .
...
\begin{definition}
	Given \(X\) , define an equivalence relation on \(X\) by setting \(x \sim  y\) if there is a connected subspace of \(X\) containing both \(x\) and \(y\) . The equivalence classes are called the \index{Component}components (or the ``\index{Connected component}connected components'') of \(X\) .
\end{definition}
...
\begin{definition}
	We define another equivalence relation on the space \(X\) by defining \(x \sim  y\) if there is a path in \(X\) from \(x\) to \(y\) . The equivalence classes are called the \index{Path component}path components of \(X\) .
\end{definition}
...
\item
  ...
  \item Consider \({\mathbb{R}}^{\omega }\) in the uniform topology. Show that \(\mathbf{x}\) and \(\mathbf{y}\) lie in the same component of \({\mathbb{R}}^{\omega }\) if and only if the sequence \[ \mathbf{x} - \mathbf{y} = \left( {{x}_{1} - {y}_{1},{x}_{2} - {y}_{2},\ldots }\right) \] is bounded. [Hint: It suffices to consider the case where \(\mathbf{y} = \mathbf{0}\) .]
...
\item Let \(X\) denote the rational points of the interval \(\left\lbrack  {0,1}\right\rbrack   \times  0\) of \({\mathbb{R}}^{2}\) . Let \(T\) denote the union of all line segments joining the point \(p = 0 \times  1\) to points of \(X\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Consider the ``\index{Infinite broom}infinite broom'' \(X\) pictured in Figure 25.1. Show that \(X\) is not locally connected at \(p\) , but is weakly locally connected at \(p\) . [Hint: Any connected neighborhood of \(p\) must contain all the points \({a}_{i}\) .] \begin{center} \includegraphics[width=1.0\textwidth]{images/25.1.jpg} \end{center} \hspace*{3em} Figure 25.1
\item Let \(p : X \rightarrow  Y\) be a quotient map. Show that if \(X\) is locally connected, then \(Y\) is locally connected. [Hint: If \(C\) is a component of the open set \(U\) of \(Y\) , show that \({p}^{-1}\left( C\right)\) is a union of components of \({p}^{-1}\left( U\right)\) .]
\item Let \(G\) be a \index{Topological group}topological group; let \(C\) be the component of \(G\) containing the identity element \(e\) . Show that \(C\) is a normal subgroup of \(G\) . [Hint: If \(x \in  G\) , then \({xC}\) is the component of \(G\) containing \(x\) .]
\item Let \(X\) be a space. Let us define \(x \sim  y\) if there is no separation \(X = A \cup  B\) of \(X\) into disjoint open sets such that \(x \in  A\) and \(y \in  B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show this relation is an equivalence relation. The equivalence classes are called the \index{Quasicomponent}quasicomponents of \(X\) .
...
\[
\mathcal{A} = \left\{  {(1/n,1\rbrack  \mid  n \in  {\mathbb{Z}}_{ + }}\right\}
\]
...
The set \(W \times  Y\) is often called a \index{Tube}tube about \({x}_{0} \times  Y\) .
...
\begin{lemma}[The \index{Tube lemma}tube lemma]
...
\begin{definition}
	A collection \(\mathcal{C}\) of subsets of \(X\) is said to have the \index{Finite intersection property}finite intersection property if for every finite subcollection
...
A special case of this theorem occurs when we have a \index{nested sequence of sets@Nested sequence of sets}Nested sequence of sets nested sequence \({C}_{1} \supset  {C}_{2} \supset\)  \(\cdots  \supset  {C}_{n} \supset  {C}_{n + 1} \supset  \ldots\) of closed sets in a compact space \(X\) . If each of the sets \({C}_{n}\) is nonempty, then the collection \(\mathcal{C} = {\left\{  {C}_{n}\right\}  }_{n \in  {\mathbb{Z}}_{ + }}\) automatically has the finite intersection property. Then the intersection
...
\item
  ...
  \item If \(\mathbb{R}\) has the topology consisting of all sets \(A\) such that \(\mathbb{R} - A\) is either countable or all of \(\mathbb{R}\) , is \(\left\lbrack  {0,1}\right\rbrack\) a compact subspace?
  \end{enumerate}
...
\item Theorem. Let \(f : X \rightarrow  Y\) ; let \(Y\) be compact Hausdorff. Then \(f\) is continuous if and only if the \index{graph of a function@Graph of a function}Graph of a function graph of \(f\) , \[ {G}_{f} = \{ x \times  f\left( x\right)  \mid  x \in  X\} \] is closed in \(X \times  Y\) . [Hint: If \({G}_{f}\) is closed and \(V\) is a neighborhood of \(f\left( {x}_{0}\right)\) , then the intersection of \({G}_{f}\) and \(X \times  \left( {Y - V}\right)\) is closed. Apply Exercise 7.]
...
\item Let \(G\) be a topological group.
  ...
  \item Let \(H\) be a subgroup of \(G\) ; let \(p : G \rightarrow  \index{G@\(G/H\)} G/H\) be the quotient map. If \(H\) is compact, show that \(p\) is a closed map.
...
\[
\rho \left( {x,y}\right)  \leq  \index{d@\(d\left( {x,A}\right)\)} d\left( {x,y}\right)  \leq  \sqrt{n}\rho \left( {x,y}\right)
\]
...
\begin{theorem}[Extreme value theorem]
	Let \(f : X \rightarrow  Y\) be continuous, where \(Y\) is an ordered set in the order topology. If \(X\) is compact, then there exist points \(c\) and \(d\) in \(X\) such that \(f\left( c\right)  \leq  f\left( x\right)  \leq  f\left( d\right)\) for every \(x \in  X\) .
...
Now we prove the uniform continuity theorem of calculus. In the process, we are led to introduce a new notion that will prove to be surprisingly useful, that of a \index{lebesgue number@Lebesgue number}Lebesgue number for an open covering of a metric space. First, a preliminary notion:
\end{proof}

\begin{definition}
	Let(X, d)be a metric space; let \(A\) be a nonempty subset of \(X\) . For each \(x \in  X\) , we define the \index{distance from x to a@Distance from x to A}Distance from x to A distance from \(x\) to \(A\) by the equation
...
\begin{lemma}[The \index{lebesgue number lemma@Lebesgue number lemma}Lebesgue number lemma]
...
\begin{definition}
	A function \(f\) from the metric space \(\left( {X,{d}_{X}}\right)\) to the metric space \(\left( {Y,{d}_{Y}}\right)\) is said to be \index{Uniformly continuous}uniformly continuous if given \(\epsilon  > 0\) , there is a \(\delta  > 0\) such that for every pair of points \({x}_{0},{x}_{1}\) of \(X\) ,
...
\begin{theorem}[Uniform continuity theorem]
	Let \(f : X \rightarrow  Y\) be a continuous map of the compact metric space \(\left( {X,{d}_{X}}\right)\) to the metric space \(\left( {Y,{d}_{Y}}\right)\) . Then \(f\) is uniformly continuous.
...
\begin{definition}
	If \(X\) is a space, a point \(x\) of \(X\) is said to be an \index{Isolated point}isolated point of \(X\) if the one-point set \(\{ x\}\) is open in \(X\)
\end{definition}
...
\item Let \(X\) be a metric space with metric \(d\) ; let \(A \subset  X\) be nonempty.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \(d\left( {x,A}\right)  = 0\) if and only if \(x \in  \bar{A}\) .
  \item Show that if \(A\) is compact, \(d\left( {x,A}\right)  = d\left( {x,a}\right)\) for some \(a \in  A\) .
  \item Define the \(\epsilon\) -neighborhood of \(A\) in \(X\) to be the set \[ \index{U@\(U\left( {A,\epsilon }\right)\)} U\left( {A,\epsilon }\right)  = \{ x \mid  d\left( {x,A}\right)  < \epsilon \} . \] Show that \(U\left( {A,\epsilon }\right)\) equals the union of the open balls \({B}_{d}\left( {a,\epsilon }\right)\) for \(a \in  A\)
...
\item Recall that \({\mathbb{R}}_{K}\) denotes \(\mathbb{R}\) in the \(K\) -topology.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \(\left\lbrack  {0,1}\right\rbrack\) is not compact as a subspace of \({\mathbb{R}}_{K}\) .
  \item Show that \({\mathbb{R}}_{K}\) is connected. [Hint \(\left( {-\infty ,0}\right)\) and \(\left( {0,\infty }\right)\) inherit their usual topologies as subspaces of \({\mathbb{R}}_{K}\) .]
...
\item Let \(X\) be a compact Hausdorff space, let \(\left\{  {A}_{n}\right\}\) be a countable collection of closed sets of \(X\) . Show that if each set \({A}_{n}\) has empty interior in \(X\) , then the union \(\cup  {A}_{n}\) has empty interior in \(X\) . [Hint: Imitate the proof of Theorem 27.7.] This is a special case of the Baire category theorem, which we shall study in Chapter 8.
\item Let \({A}_{0}\) be the closed interval \(\left\lbrack  {0,1}\right\rbrack\) in \(\mathbb{R}\) . Let \({A}_{1}\) be the set obtained from \({A}_{0}\) by deleting its ``middle third'' \(\left( {\frac{1}{3},\frac{2}{3}}\right)\) . Let \({A}_{2}\) be the set obtained from \({A}_{1}\) by deleting its ``middle thirds'' \(\left( {\frac{1}{9},\frac{2}{9}}\right)\) and \(\left( {\frac{7}{9},\frac{8}{9}}\right)\) In general, define \({A}_{n}\) by the equation \[ {A}_{n} = {A}_{n - 1} - \mathop{\bigcup }\limits_{{k = 0}}^{\infty }\left( {\frac{1 + {3k}}{{3}^{n}},\frac{2 + {3k}}{{3}^{n}}}\right) . \] The intersection \[ C = \mathop{\bigcap }\limits_{{n \in  {\mathbb{Z}}_{ + }}}{A}_{n} \] is called the \index{cantor set@Cantor set}Cantor set; it is a subspace of \(\left\lbrack  {0,1}\right\rbrack\)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
In some ways this property is more natural and intuitive than that of compactness. In the early days of topology, it was given the name ``compactness,'' while the open covering formulation was called ``\index{Bicompactness}bicompactness.'' Later, the word ``compact'' was shifted to apply to the open covering definition, leaving this one to search for a new name It still has not found a name on which everyone agrees On historical grounds, some call it ``Fréchet compactness'', others call it the ``Bolzano-Weierstrass property'' We have invented the term ``\index{Limit point compactness}limit point compactness'' It seems as good a term as any; at least it describes what the property is about.
...
We now show these two versions of compactness coincide for metrizable spaces; for this purpose, we introduce yet another version of compactness called \index{Sequential compactness}sequential compactness. This result will be used in Chapter 7.

\begin{definition}
	Let \(X\) be a topological space. If \(\left( {x}_{n}\right)\) is a sequence of points of \(X\) , and if

\[
{n}_{1} < {n}_{2} < \cdots  < {n}_{i} < \cdots
\]

is an increasing sequence of positive integers, then the sequence \(\left( {y}_{i}\right)\) defined by setting \({y}_{i} = {x}_{{n}_{i}}\) is called a \index{Subsequence}subsequence of the sequence \(\left( {x}_{n}\right)\) . The space \(X\) is said to be sequentially compact if every sequence of points of \(X\) has a convergent subsequence.
\end{definition}
...
\section*{Exercises}
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item Give \({\left\lbrack  0,1\right\rbrack  }^{\omega }\) the uniform topology. Find an infinite subset of this space that has no limit point
\item Show that \(\left\lbrack  {0,1}\right\rbrack\) is not limit point compact as a subspace of \({\mathbb{R}}_{\ell }\) .
\item Let \(X\) be limit point compact.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(f : X \rightarrow  Y\) is continuous, does it follow that \(f\left( X\right)\) is limit point compact?
  ...
  \end{enumerate}
\item A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \index{Countable compactness}countable compactness is equivalent to limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
\item Let(X, d)be a metric space. If \(f : X \rightarrow  X\) satisfies the condition \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  = d\left( {x,y}\right) \] for all \(x,y \in  X\) , then \(f\) is called an \index{Isometry}isometry of \(X\) . Show that if \(f\) is an isometry and \(X\) is compact, then \(f\) is bijective and hence a homeomorphism. [Hint: If \(a \notin  f\left( X\right)\) , choose \(\epsilon\) so that the \(\epsilon\) -neighborhood of \(a\) is disjoint from \(f\left( X\right)\) Set \({x}_{1} = a\) , and \({x}_{n + 1} = f\left( {x}_{n}\right)\) in general. Show that \(d\left( {{x}_{n},{x}_{m}}\right)  \geq  \epsilon\) for \(n \neq  m\) .]
\item Let(X, d)be a metric space. If \(f\) satisfies the condition \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  < d\left( {x,y}\right) \] for all \(x,y \in  X\) with \(x \neq  y\) , then \(f\) is called a \index{Shrinking map}shrinking map If there is a number \(\alpha  < 1\) such that \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  \leq  {\alpha d}\left( {x,y}\right) \] for all \(x,y \in  X\) , then \(f\) is called a \index{Contraction}contraction. A fixed point of \(f\) is a point \(x\) such that \(f\left( x\right)  = x\)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(f\) is a contraction and \(X\) is compact, show \(f\) has a unique fixed point. [Hint: Define \({f}^{1} = f\) and \({f}^{n + 1} = f \circ  {f}^{n}\) . Consider the intersection \(A\) of the sets \(\left. {{A}_{n} = {f}^{n}\left( X\right) \text{ . }}\right\rbrack\)
  ...
  \item Let \(X = \left\lbrack  {0,1}\right\rbrack\) . Show that \(f\left( x\right)  = x - {x}^{2}/2\) maps \(X\) into \(X\) and is a shrinking map that is not a contraction. [Hint Use the mean-value theorem of calculus.]
  \item The result in (a) holds if \(X\) is a complete metric space, such as \(\mathbb{R}\) ; see the exercises of \(§{43}\) . The result in (b) does not. Show that the map \(f : \mathbb{R} \rightarrow\)  \(\mathbb{R}\) given by \(f\left( x\right)  = \left\lbrack  {x + {\left( {x}^{2} + 1\right) }^{1/2}}\right\rbrack  /2\) is a shrinking map that is not a contraction and has no fixed point.
  \end{enumerate}
\end{enumerate}
\section*{§ 29 Local Compactness}

In this section we study the notion of \index{Local compactness}local compactness, and we prove the basic theorem that any locally compact Hausdorff space can be imbedded in a certain compact Hausdorff space that is called its one-point \index{Compactification}compactification.
...
\begin{centeredblock}
\begin{example}
	The real line \(\mathbf{R}\) is locally compact. The point \(x\) lies in some interval(a, b), which in turn is contained in the compact subspace \(\left\lbrack  {a,b}\right\rbrack\) The subspace \(\mathbb{Q}\) of rational numbers is not locally compact, as you can check.
\end{example}
\end{centeredblock}
...
\item Let \(\left\{  {X}_{\alpha }\right\}\) be an indexed family of nonempty spaces.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \end{enumerate}
\item Let \(X\) be a locally compact space. If \(f : X \rightarrow  Y\) is continuous, does it follow that \(f\left( X\right)\) is locally compact? What if \(f\) is both continuous and open? Justify your answer.
\item Show that \({\left\lbrack  0,1\right\rbrack  }^{\omega }\) is not locally compact in the uniform topology.
...
\item Show that if \(X\) is a Hausdorff space that is locally compact at the point \(x\) , then for each neighborhood \(U\) of \(x\) , there is a neighborhood \(V\) of \(x\) such that \(\bar{V}\) is compact and \(\bar{V} \subset  U\) . -11. Prove the following:
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Lemma. If \(p : X \rightarrow  Y\) is a quotient map and if \(Z\) is a locally compact Hausdorff space, then the map \[ \pi  = p \times  {i}_{Z} : X \times  Z \rightarrow  Y \times  Z \] is a quotient map. [Hint: If \({\pi }^{-1}\left( A\right)\) is open and contains \(x \times  y\) , choose open sets \({U}_{1}\) and \(V\) with \(\bar{V}\) compact, such that \(x \times  y \in  {U}_{1} \times  V\) and \({U}_{1} \times  \bar{V} \subset  {\pi }^{-1}\left( A\right)\) . Given \({U}_{i} \times  \bar{V} \subset  {\pi }^{-1}\left( A\right)\) , use the tube lemma to choose an open set \({U}_{i + 1}\) containing \({p}^{-1}\left( {p\left( {U}_{i}\right) }\right)\) such that \({U}_{i + 1} \times  \bar{V} \subset  {\pi }^{-1}\left( A\right)\) . Let \(U = \bigcup {U}_{i}\) ; show that \(U \times  V\) is a saturated neighborhood of \(x \times  y\) that is contained in \({\pi }^{-1}\left( A\right)\) . An entirely different proof of this result will be outlined in the exercises of §46.
...
\textsl{Lemma}
	The net \(\left( {x}_{\alpha }\right)\) has the point \(x\) as an accumulation point if and only if some subnet of \(\left( {x}_{\alpha }\right)\) converges to \(x\) .
