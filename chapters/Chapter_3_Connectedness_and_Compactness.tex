...
In the study of calculus, there are three basic \idx{Intermediate-value theorem} theorems about \idx{Continuous image: of a compact space} continuous functions, and on these theorems the rest of calculus depends. They are the following:
...
\idx{Maximum value theorem: of calculus} Maximum value theorem. If \(f : \left\lbrack  {a,b}\right\rbrack   \rightarrow  R\) is continuous, then there exists an element \(c \in  \left\lbrack  {a,b}\right\rbrack\) such that \(f\left( x\right)  \leq  f\left( c\right)\) for every \(x \in  \left\lbrack  {a,b}\right\rbrack\) .
...
\begin{definition}
	Given \(X\) , define an equivalence relation on \(X\) by setting \(x \sim  y\) if there is a connected subspace of \(X\) containing both \(x\) and \(y\) . The equivalence classes are called the \idx{component}s (or the ``\idx{connected component}s'') of \(X\) .
\end{definition}
...
\begin{definition}
	We define another equivalence relation on the space \(X\) by defining \(x \sim  y\) if there is a path in \(X\) from \(x\) to \(y\) . The equivalence classes are called the \idx{path component}s of \(X\) .
\end{definition}
...
\item Let \(X\) be a space. Let us define \(x \sim  y\) if there is no separation \(X = A \cup  B\) of \(X\) into disjoint open sets such that \(x \in  A\) and \(y \in  B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show this relation is an equivalence relation. The equivalence classes are called the \idx{quasicomponent}s of \(X\) .
