...
These theorems are used in a number of places. The intermediate value theorem is used for instance in constructing inverse functions, such as \(\sqrt[3]{x}\) and arcsin \(x\) ; and the maximum value theorem is used for proving the mean value theorem for derivatives, upon which the two fundamental theorems of calculus depend. The \index{uniform continuity theorem@Uniform continuity theorem}\index{uniform continuity theorem@Uniform continuity theorem}\index{uniform continuity theorem@Uniform continuity theorem}uniform continuity theorem is used, among other things, for proving that every continuous function is integrable.
...
\begin{definition}
	Let \(X\) be a topological space. A \index{Separation} of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a \index{separation@Separation}\index{separation@Separation}\index{separation@Separation}separation of \(X\) .
\end{definition}
...
\begin{theorem}
	The image \index{Continuous image: of a compact space!of a connected space}\index{Continuous image: of a compact space!of a connected space}\index{Continuous image: of a compact space!of a connected space}of a connected space under a continuous map is connected.
...
\item Show that if \(X\) is an \index{Compactness!in finite complement topology}infinite set, it is connected in the finite complement topology.
\item A space is \index{Totally disconnected} if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is \index{totally disconnected@Totally disconnected}\index{totally disconnected@Totally disconnected}\index{totally disconnected@Totally disconnected}totally disconnected. Does the converse hold?
...
The intermediate value theorem of calculus is the \index{Baire category theorem!special case}\index{Baire category theorem!special case}\index{Baire category theorem!special case}special case of this theorem that occurs when we take \(X\) to be a closed interval in \(\mathbb{R}\) and \(Y\) to be \(\mathbb{R}\) .
...
Connectedness of intervals in \(\mathbb{R}\) gives rise to an especially useful criterion for showing that a space \(X\) is connected; namely, the condition that every pair of points of \(X\) can be joined by a \index{path@Path}\index{path@Path}\index{path@Path}path in \(X\) :
...
\begin{centeredblock}
\begin{example}
	Define the \index{Unit ball (see also \({B}^{2},{B}^{n}\))}\index{Unit ball (see also \({B}^{2},{B}^{n}\))}unit ball \(\index{B^n@\({B}^{n}\)}{B}^{n}\) in \({\mathbb{R}}^{n}\) by the equation
...
\begin{centeredblock}
\begin{example}
	The ordered square \({I}_{o}^{2}\) is connected but not path connected.
...
\begin{centeredblock}
\begin{example}
	Each interval and each ray in the real line is both connected and locally connected The subspace \([  {-1,0)\cup (0,1}]\) of \(\mathbb{R}\) is not connected, but it is locally connected The topologist's sine curve is connected but not locally connected. The rationals \(\mathbb{Q}\) are neither connected nor locally connected.
\end{example}
\end{centeredblock}
...
\item Let \(p : X \rightarrow  Y\) be a quotient map. Show that if \(X\) is \index{Quotient topology!local connectedness}locally connected, then \(Y\) is locally connected. [Hint: If \(C\) is a component of the open set \(U\) of \(Y\) , show that \({p}^{-1}( C)\) is a union of components of \({p}^{-1}( U)\) .]
...
\item Let \(X\) be a space. Let us define \(x \sim  y\) if there is no separation \(X = A \cup  B\) of \(X\) into disjoint open sets such that \(x \in  A\) and \(y \in  B\) .
  ...
  \item Show that each component of \(X\) lies in a \index{quasicomponent@Quasicomponent!vs. component}}_{ + }}\}\) and let \(- K\) denote the set \(\{  - 1/n \mid  n \in\)  \(. {\mathbb{Z}}_{ + }\}\) . Determine the components, path components, and quasicomponents of the following subspaces of \({\mathbb{R}}^{2}\) : \[ A = ( {K \times  [  {0,1}]  })  \cup  \{ 0 \times  0\}  \cup  \{ 0 \times  1\} . \] \[ B = A \cup  ( {[  {0,1}]   \times  \{ 0\rbrack }) \text{ . } \] \[ C = ( {K \times  [  {0,1}]  })  \cup  ( {-K \times  [  {-1,0}]  })  \cup  ( {[  {0,1}]   \times   - K})  \cup  ( {[  {-1,0}]   \times  K}) . \]
  \end{enumerate}
\end{enumerate}
...
\section*{Theorem 26.2. Every \index{Compactness!of subspace}closed subspace of a compact space is compact.}
...
\begin{centeredblock}
\begin{example}
	One needs the Hausdorff condition in the hypothesis of Theorem 263 Consider, for example, the finite complement topology on the real line The only proper subsets of \(\mathbb{R}\) that are closed in this topology are the finite sets. But every subset of \(\mathbb{R}\) is compact in this topology, as you can check.
\end{example}
\end{centeredblock}
...
\item Theorem. Let \(f : X \rightarrow  Y\) ; let \(Y\) be compact Hausdorff. Then \(f\) is continuous if and only if the \index{graph of a function@Graph of a function} graph of \(f\) , \[ {G}_{f} = \{ x \times  f( x)  \mid  x \in  X\} \] is closed in \(X \times  Y\) . [Hint: If \({G}_{f}\) is closed and \(V\) is a neighborhood of \(f( {x}_{0})\) , then the intersection of \({G}_{f}\) and \(X \times  ( {Y - V})\) is closed. Apply Exercise 7.]
...
\item Let \(p : X \rightarrow  Y\) be a closed continuous surjective map such that \({p}^{-1}( {\{ y\} })\) is compact, for each \(y \in  Y\) . (Such a map is called a \index{Perfect map} ) Show that if \(Y\) is compact, then \(X\) is compact. [Hint: If \(U\) is an open set containing \({p}^{-1}( {\{ y\} })\) , there is a neighborhood \(W\) of \(y\) such that \({p}^{-1}( W)\) is contained in \(U\) ]
\item Let \(G\) be a \index{topological group@Topological group}topological group.
  ...
  \item Let \(H\) be a subgroup of \(G\) ; let \(p : G \rightarrow  \index{G@\(G/H\)} \index{G@$G/H$}\index{G@$G/H$}\index{G@$G/H$}G/H\) be the quotient map. If \(H\) is compact, show that \(p\) is a closed map.
...
The theorems of the preceding section enable us to construct new compact spaces from existing ones, but in order to get very far we have to find some compact spaces to start with. The natural place to begin is the real line; we shall prove that every \index{Compactness!in \(\mathbb{R}\) and \({\mathbb{R}}^{n}\)}closed interval in \(\mathbb{R}\) is compact. Applications include the \index{Extreme value theorem} and the uniform continuity theorem of calculus, suitably generalized. We also give a characterization of all \index{Linear continuum!compact subspaces}\index{Linear continuum!compact subspaces}compact subspaces \index{Path connectedness!of \({\mathbb{R}}^{n} - 0\)}of \({\mathbb{R}}^{n}\) , and a proof of the \index{\(\mathbb{R}\) (reals)!uncountability}\index{\(\mathbb{R}\) (reals)!uncountability}uncountability of the set of real numbers.
...
\begin{corollary}
	Every \index{Compactness!in \(\mathbb{R}\) and \({\mathbb{R}}^{n}\)}closed interval in \(\mathbb{R}\) is compact.
\end{corollary}
...
\begin{centeredblock}
\begin{example}
	The unit sphere \({S}^{n - 1}\) and the closed unit ball \({B}^{n}\) in \({\mathbb{R}}^{n}\) are \index{B^n@\({B}^{n}\)!compactness}compact because they are closed and bounded. The set
...
Now we prove the \index{extreme value theorem@Extreme value theorem}\index{extreme value theorem@Extreme value theorem}\index{extreme value theorem@Extreme value theorem}extreme value theorem of calculus, in suitably generalized form.
...
forms an \index{open covering@Open covering}\index{open covering@Open covering}open covering of \(A\) . Since \(A\) is compact, some finite subcollection
...
Now we introduce the notion of \index{lebesgue number@Lebesgue number}\index{lebesgue number@Lebesgue number}\index{lebesgue number@Lebesgue number}Lebesgue number. Recall that the diameter of a bounded subset \(A\) of a metric space(X, d)is the number
...
\begin{theorem}[Uniform continuity theorem]
	Let \(f : X \rightarrow  Y\) be a continuous map of the compact metric space \(( {X,{d}_{X}})\) to the metric space \(( {Y,{d}_{Y}})\) . Then \(f\) is \index{uniformly continuous@Uniformly continuous}\index{uniformly continuous@Uniformly continuous}\index{uniformly continuous@Uniformly continuous}uniformly continuous.
...
\begin{theorem}
	Let \(X\) be a nonempty compact Hausdorff space. If \(X\) has no \index{isolated point@Isolated point}\index{isolated point@Isolated point}\index{isolated point@Isolated point}isolated points, then \(X\) is uncountable.
...
is an increasing sequence of positive integers, then the sequence \(( {y}_{i})\) defined by setting \({y}_{i} = {x}_{{n}_{i}}\) is called a \index{Subsequence} of the sequence \(( {x}_{n})\) . The space \(X\) is said to be sequentially compact if every sequence of points of \(X\) has a convergent \index{subsequence@Subsequence}\index{subsequence@Subsequence}\index{subsequence@Subsequence}subsequence.
\end{definition}
...
\item Let \(X\) be limit point compact.
  ...
  \item If \(X\) is a subspace of the Hausdorff space \(Z\) , does it follow that \(X\) is closed in \(Z\) ? We comment that it is not in general true that the product of two limit point compact spaces is limit point compact, even if the \index{Topological group!Hausdorff condition}\index{Topological group!Hausdorff condition}Hausdorff condition is assumed. But the examples are fairly sophisticated. See \cite{SteenSeebach1970}, Example II2.
  \end{enumerate}
\item A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \index{Countable compactness} is equivalent to \index{limit point compactness@Limit point compactness}\index{limit point compactness@Limit point compactness}\index{limit point compactness@Limit point compactness}limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
\item Let(X, d)be a metric space. If \(f : X \rightarrow  X\) satisfies the condition \[ d( {f( x) ,f( y) })  = d( {x,y}) \] for all \(x,y \in  X\) , then \(f\) is called an \index{Isometry} of \(X\) . Show that if \(f\) is an \index{isometry@Isometry}\index{isometry@Isometry}\index{isometry@Isometry}isometry and \(X\) is compact, then \(f\) is bijective and hence a homeomorphism. [Hint: If \(a \notin  f( X)\) , choose \(\epsilon\) so that the \(\epsilon\) -neighborhood of \(a\) is disjoint from \(f( X)\) Set \({x}_{1} = a\) , and \({x}_{n + 1} = f( {x}_{n})\) in general. Show that \(d( {{x}_{n},{x}_{m}})  \geq  \epsilon\) for \(n \neq  m\) .]
\item Let(X, d)be a metric space. If \(f\) satisfies the condition \[ d( {f( x) ,f( y) })  < d( {x,y}) \] for all \(x,y \in  X\) with \(x \neq  y\) , then \(f\) is called a \index{Shrinking map} If there is a number \(\alpha  < 1\) such that \[ d( {f( x) ,f( y) })  \leq  {\alpha d}( {x,y}) \] \index{Fixed point theorem: for \({B}^{n}\)!for a contraction}for all \(x,y \in  X\) , then \(f\) is called a \index{Contraction}. A \index{fixed point@Fixed point}\index{fixed point@Fixed point}fixed point of \(f\) is a point \(x\) such that \(f( x)  = x\)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(f\) is a \index{contraction@Contraction}\index{contraction@Contraction}\index{contraction@Contraction}contraction and \(X\) is compact, show \(f\) has a unique fixed point. [Hint: Define \({f}^{1} = f\) and \({f}^{n + 1} = f \circ  {f}^{n}\) . Consider the intersection \(A\) of the sets \(. {{A}_{n} = {f}^{n}( X) \text{ . }}]\)
  \item Show more generally that if \(f\) is a \index{shrinking map@Shrinking map}\index{shrinking map@Shrinking map}\index{shrinking map@Shrinking map}shrinking map and \(X\) is compact, then \(f\) has a unique fixed point. [Hint: Let \(A\) be as before. Given \(x \in  A\) , choose \({x}_{n}\) so that \(x = {f}^{n + 1}( {x}_{n})\) . If \(a\) is the limit of some subsequence of the sequence \({y}_{n} = {f}^{n}( {x}_{n})\) , show that \(a \in  A\) and \(f( a)  = x\) . Conclude that \(A = f( A)\) , so that diam \(A = 0\) .]
  \item Let \(X = [  {0,1}]\) . Show that \(f( x)  = x - {x}^{2}/2\) maps \(X\) into \(X\) and is a shrinking map that is not a contraction. [Hint Use the mean-value theorem of calculus.]
...
\begin{centeredblock}
\begin{example}
	The space \({\mathbb{R}}^{n}\) is locally compact, the point \(x\) lies in some basis element \(( {{a}_{1},{b}_{1}})  \times   \cdot   \times  ( {{a}_{n},{b}_{n}})\) , which in turn lies in the compact subspace \([  {{a}_{1},{b}_{1}}]   \times   \cdot   \times  [  {{a}_{n},{b}_{n}}]\) . The space \({\mathbb{R}}^{\omega }\) is not locally compact, none of its basis elements are contained in compact subspaces For if
...
\end{theorem}
\begin{proof}
Step 1. We first verify \index{One-point compactification!uniqueness}\index{One-point compactification!uniqueness}\index{One-point compactification!uniqueness}uniqueness. Let \(Y\) and \({Y}^{\prime }\) be two spaces satisfying these conditions. Define \(h : Y \rightarrow  {Y}^{\prime }\) by letting \(h\) map the single point \(p\) of \(Y - X\) to the point \(q\) of \({Y}^{\prime } - X\) , and letting \(h\) equal the identity on \(X\) . We show that if \(U\) is open in \(Y\) , then \(h( U)\) is open in \({Y}^{\prime }\) . Symmetry then implies that \(h\) is a homeomorphism.
...
\begin{definition}
	If \(Y\) is a compact Hausdorff space and \(X\) is a proper subspace of \(Y\) whose closure equals \(Y\) , then \(Y\) is said to be a compactification of \(X\) . If \(Y - X\) equals a single point, then \(Y\) is called the \index{one-point compactification@One-point compactification}\index{one-point compactification@One-point compactification}\index{one-point compactification@One-point compactification}one-point compactification of \(X\) .
\end{definition}
...
\item Let \(\{  {X}_{\alpha }\}\) be an indexed family of nonempty spaces.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that if \(\prod {X}_{\alpha }\) is locally compact, then each \({X}_{\alpha }\) is locally compact and \({X}_{\alpha }\) is compact for all but finitely many values of \(\alpha\) .
...
\item Show that if \(G\) is a locally compact \index{topological group@Topological group}\index{topological group@Topological group}topological group and \(H\) is a subgroup, then \(G/H\) is locally compact.
...
10. Theorem. X is compact if and only if every net in \(X\) has a convergent subnet. [Hint: To prove the implication \(\Rightarrow\) , let \({B}_{\alpha } = \{  {{x}_{\beta } \mid  \alpha  \preccurlyeq  \beta }\}\) and show that \(\{  {B}_{\alpha }\}\) has the \index{finite intersection property@Finite intersection property}\index{finite intersection property@Finite intersection property}finite intersection property. To prove \(\Leftarrow\) , let \(\mathcal{A}\) be a collection of closed sets having the finite intersection property, and let \(\mathcal{B}\) be the collection of all finite intersections of elements of \(\mathcal{A}\) , partially ordered by reverse inclusion.]
