...
\begin{definition} Let \(X\) be a topological space. A \idx{separation} of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a separation of \(X\) .
\end{definition}
...
5. A space is \idx{totally disconnected} if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is totally disconnected. Does the converse hold?
...
Let \(\mathbf{a} = \left( {a}_{\alpha }\right)\) be a \idx{fixed point} of \(X\) .
...
\begin{example} Define the \index{Unit ball (see also \({B}^{2},{B}^{n}\))} unit ball \(\idxmath{B^n}{\({B}^{n}\)} {B}^{n}\) in \({\mathbb{R}}^{n}\) by the equation
...
\begin{example} Define the \index{Unit sphere (see also \({S}^{n}\))} \index{Sphere, unit (see also \({S}^{2}\))} unit sphere \({S}^{n - 1}\) in \({\mathbb{R}}^{n}\) by the equation
...
*12. Recall that \({S}_{\Omega }\) denotes the minimal uncountable well-ordered set. Let \(L\) denote the ordered set \({S}_{\Omega } \times  \lbrack 0,1)\) in the dictionary order, with its smallest element deleted. The set \(L\) is a classical example in topology called the \idx{long line}.

\begin{theorem} The long line is path connected and \idx{Local path connectedness} locally homeomorphic to \(\mathbb{R}\) , but it cannot be imbedded in \(\mathbb{R}\) .
...
(e) Show that every point of \(L\) has a \idx{Symmetric neighborhood} neighborhood homeomorphic with an open interval in \(\mathbb{R}\) .
...
\begin{definition} Given \(X\) , define an equivalence relation on \(X\) by setting \(x \sim  y\) if there is a connected subspace of \(X\) containing both \(x\) and \(y\) . The equivalence classes are called the \idx{component}s (or the "\idx{connected component}s") of \(X\) .
\end{definition}
...
\begin{definition} We define another equivalence relation on the space \(X\) by defining \(x \sim  y\) if there is a path in \(X\) from \(x\) to \(y\) . The equivalence classes are called the \idx{path component}s of \(X\) .
\end{definition}
...
\begin{example} The "\idx{topologist's sine curve}" \(\bar{S}\) of the preceding section is a space that has a single component (since it is connected) and two path components One path component is the curve \(S\) and the other is the vertical interval \(V = 0 \times  \left\lbrack  {-1,1}\right\rbrack\) . Note that \(S\) is open in \(\bar{S}\) but not closed, while \(V\) is closed but not open
...
9. Let \(G\) be a \idx{topological group}; let \(C\) be the component of \(G\) containing the identity element \(e\) . Show that \(C\) is a normal subgroup of \(G\) . [Hint: If \(x \in  G\) , then \({xC}\) is the component of \(G\) containing \(x\) .]
...
(a) Show this relation is an equivalence relation. The equivalence classes are called the \idx{quasicomponent}s of \(X\) .
...
\begin{lemma}[The \idx{tube lemma}] Consider the product space \(X \times  Y\) , where \(Y\) is compact. If \(N\) is an open set of \(X \times  Y\) containing the slice \({x}_{0} \times  Y\) of \(X \times  Y\) , then \(N\) contains some tube \(W \times  Y\) about \({x}_{0} \times  Y\) , where \(W\) is a neighborhood of \({x}_{0}\) in \(X\) .
\end{lemma}
...
\begin{definition} A collection \(\mathcal{C}\) of subsets of \(X\) is said to have the \idx{finite intersection property} if for every finite subcollection
...
8. Theorem. Let \(f : X \rightarrow  Y\) ; let \(Y\) be compact Hausdorff. Then \(f\) is continuous if and only if the \idx{Graph of a function} graph of \(f\) ,
...
(b) Let \(H\) be a subgroup of \(G\) ; let \(p \cdot  G \rightarrow  \idxmath{G}{\(G/H\)} G/H\) be the quotient map. If \(H\) is compact, show that \(p\) is a closed map.
...
\begin{definition} Let(X, d)be a metric space; let \(A\) be a nonempty subset of \(X\) . For each \(x \in  X\) , we define the \idx{Distance from x to A} distance from \(x\) to \(A\) by the equation
...
\begin{lemma}[The \idx{Lebesgue number lemma}] Let \(\mathcal{A}\) be an open covering of the metric space(X, d). If \(X\) is compact, there is a \(\delta  > 0\) such that for each subset of \(X\) having diameter less than \(\delta\) , there exists an element of \(\mathcal{A}\) containing it.
\end{lemma}
...
\begin{definition} A function \(f\) from the metric space \(\left( {X,{d}_{X}}\right)\) to the metric space \(\left( {Y,{d}_{Y}}\right)\) is said to be \idx{uniformly continuous} if given \(\epsilon  > 0\) , there is a \(\delta  > 0\) such that for every pair of points \({x}_{0},{x}_{1}\) of \(X\) ,
...
\begin{definition} If \(X\) is a space, a point \(x\) of \(X\) is said to be an \idx{isolated point} of \(X\) if the one-point set \(\{ x\}\) is open in \(X\)
\end{definition}
...
\[
\idxmath{U}{\(U\left( {A,\epsilon }\right)\)} U\left( {A,\epsilon }\right)  = \{ x \mid  d\left( {x,A}\right)  < \epsilon \} .
\]
...
is called the \idx{Cantor set}; it is a subspace of \(\left\lbrack  {0,1}\right\rbrack\)
...
4. A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \idx{countable compactness} is equivalent to limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
for all \(x,y \in  X\) with \(x \neq  y\) , then \(f\) is called a \idx{shrinking map} If there is a number \(\alpha  < 1\) such that

\[
d\left( {f\left( x\right) ,f\left( y\right) }\right)  \leq  {\alpha d}\left( {x,y}\right)
\]

for all \(x,y \in  X\) , then \(f\) is called a \idx{contraction}. A fixed point of \(f\) is a point \(x\) such that \(f\left( x\right)  = x\)
...
\begin{definition} If \(Y\) is a compact Hausdorff space and \(X\) is a proper subspace of \(Y\) whose closure equals \(Y\) , then \(Y\) is said to be a compactification of \(X\) . If \(Y - X\) equals a single point, then \(Y\) is called the \idx{One-point compactification} one-point compactification of \(X\) .
\end{definition}
...
3. Let \(X\) be a topological space. A \idx{Net} net in \(X\) is a function \(f\) from a directed set \(J\) into \(X\) . If \(\alpha  \in  J\) , we usually denote \(f\left( \alpha \right)\) by \({x}_{\alpha }\) . We denote the net \(f\) itself by the symbol \({\left( {x}_{\alpha }\right) }_{\alpha  \in  J}\) , or merely by \(\left( {x}_{\alpha }\right)\) if the index set is understood.
...
then the composite function \(f \circ  g : K \rightarrow  X\) is called a \idx{Subnet} subnet of \(\left( {x}_{\alpha }\right)\) . Show that if the net \(\left( {x}_{\alpha }\right)\) converges to \(x\) , so does any subnet.

9. Let \({\left( {x}_{\alpha }\right) }_{\alpha  \in  J}\) be a net in \(X\) . We say that \(x\) is an \idx{Accumulation point of a net} accumulation point of the net \(\left( {x}_{\alpha }\right)\) if for each neighborhood \(U\) of \(x\) , the set of those \(\alpha\) for which \({x}_{\alpha } \in  U\) is cofinal in \(J\) .
