...
In the study of calculus, there are three basic \idx{Intermediate-value theorem} theorems about \idx[continuous image]{Continuous image: of a compact space} continuous functions, and on these theorems the rest of calculus depends. They are the following:
...
\idx[maximum value theorem]{Maximum value theorem: of calculus} Maximum value theorem. If \(f : \left\lbrack  {a,b}\right\rbrack   \rightarrow  R\) is continuous, then there exists an element \(c \in  \left\lbrack  {a,b}\right\rbrack\) such that \(f\left( x\right)  \leq  f\left( c\right)\) for every \(x \in  \left\lbrack  {a,b}\right\rbrack\) .

\idx{Uniform continuity theorem}. If \(f : \left\lbrack  {a,b}\right\rbrack   \rightarrow  \mathbb{R}\) is continuous, then given \(\epsilon  > 0\) , there exists \(\delta  > 0\) such that \(\left| {f\left( {x}_{1}\right)  - f\left( {x}_{2}\right) }\right|  < \epsilon\) for every pair of numbers \({x}_{1},{x}_{2}\) of \(\left\lbrack  {a,b}\right\rbrack\) for which \(\left| {{x}_{1} - {x}_{2}}\right|  < \delta\) .
...
We have spoken of these three theorems as theorems about continuous functions. But they can also be considered as theorems about the \idx{Closed graph theorem} closed interval \(\left\lbrack  {a,b}\right\rbrack\) of real numbers. The theorems depend not only on the continuity of \(f\) but also on properties of the topological \idx{Homogeneous space} space \(\left\lbrack  {a,b}\right\rbrack\) .

The property of the space \(\left\lbrack  {a,b}\right\rbrack\) on which the intermediate value theorem depends is the property called \idx{connectedness}, and the property on which the other two depend is the property called \idx{compactness}. In this chapter, we shall define these properties for arbitrary topological spaces, and shall prove the appropriate \idx{General linear group} generalized versions of these theorems.
...
\begin{definition}
	Let \(X\) be a topological space. A \idx{separation} of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a separation of \(X\) .
\end{definition}
...
\item A space is \idx{totally disconnected} if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is totally disconnected. Does the converse hold?
...
\item Let \({\left\{  {X}_{\alpha }\right\}  }_{\alpha  \in  J}\) be an indexed family of connected spaces; let \(X\) be the product space \[ X = \mathop{\prod }\limits_{{\alpha  \in  J}}{X}_{\alpha } \] Let \(\mathbf{a} = \left( {a}_{\alpha }\right)\) be a \idx{fixed point} of \(X\) .
...
One application is the intermediate value theorem of calculus, suitably generalized. Another is the result that such familiar spaces as balls and spheres in euclidean space are connected; the proof involves a new notion, called \idx{path connectedness}, which we also discuss.
...
\begin{centeredblock}
\begin{example}
	Define the unit ball \(\idxmath{B^n}{\({B}^{n}\)} {B}^{n}\) in \({\mathbb{R}}^{n}\) by the equation
...
\item *12. Recall that \({S}_{\Omega }\) denotes the minimal uncountable well-ordered set. Let \(L\) denote the ordered set \({S}_{\Omega } \times  \lbrack 0,1)\) in the dictionary order, with its smallest element deleted. The set \(L\) is a classical example in topology called the \idx{long line}. \textbf{Theorem.} The long line is path connected and \idx{Local path connectedness} locally homeomorphic to \(\mathbb{R}\) , but it cannot be imbedded in \(\mathbb{R}\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  ...
  \item Show that every point of \(L\) has a \idx{Symmetric neighborhood} neighborhood homeomorphic with an open interval in \(\mathbb{R}\) .
...
\begin{definition}
	Given \(X\) , define an equivalence relation on \(X\) by setting \(x \sim  y\) if there is a connected subspace of \(X\) containing both \(x\) and \(y\) . The equivalence classes are called the \idx[components]{component} (or the ``\idx[connected components]{connected component}'') of \(X\) .
\end{definition}
...
\begin{definition}
	We define another equivalence relation on the space \(X\) by defining \(x \sim  y\) if there is a path in \(X\) from \(x\) to \(y\) . The equivalence classes are called the \idx[path components]{path component} of \(X\) .
\end{definition}
...
Connectedness is a useful property for a space to possess. But for some purposes, it is more important that the space satisfy a connectedness condition locally. Roughly speaking, \idx{local connectedness} means that each point has ``arbitrarily small'' neighborhoods that are connected. More precisely, one has the following definition:
...
\item Let \(G\) be a \idx{topological group}; let \(C\) be the component of \(G\) containing the identity element \(e\) . Show that \(C\) is a normal subgroup of \(G\) . [Hint: If \(x \in  G\) , then \({xC}\) is the component of \(G\) containing \(x\) .]
\item Let \(X\) be a space. Let us define \(x \sim  y\) if there is no separation \(X = A \cup  B\) of \(X\) into disjoint open sets such that \(x \in  A\) and \(y \in  B\) .
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show this relation is an equivalence relation. The equivalence classes are called the \idx[quasicomponents]{quasicomponent} of \(X\) .
...
The set \(W \times  Y\) is often called a \idx{tube} about \({x}_{0} \times  Y\) .
...
\begin{lemma}[The \idx{tube lemma}]
...
\begin{definition}
	A collection \(\mathcal{C}\) of subsets of \(X\) is said to have the \idx{finite intersection property} if for every finite subcollection
...
\item Theorem. Let \(f : X \rightarrow  Y\) ; let \(Y\) be compact Hausdorff. Then \(f\) is continuous if and only if the \idx{Graph of a function} graph of \(f\) , \[ {G}_{f} = \{ x \times  f\left( x\right)  \mid  x \in  X\} \] is closed in \(X \times  Y\) . [Hint: If \({G}_{f}\) is closed and \(V\) is a neighborhood of \(f\left( {x}_{0}\right)\) , then the intersection of \({G}_{f}\) and \(X \times  \left( {Y - V}\right)\) is closed. Apply Exercise 7.]
...
\item Let \(G\) be a topological group.
  ...
  \item Let \(H\) be a subgroup of \(G\) ; let \(p : G \rightarrow  \idxmath{G}{\(G/H\)} G/H\) be the quotient map. If \(H\) is compact, show that \(p\) is a closed map.
...
\[
\rho \left( {x,y}\right)  \leq  \idxmath{d}{\(d\left( {x,A}\right)\)} d\left( {x,y}\right)  \leq  \sqrt{n}\rho \left( {x,y}\right)
\]
...
Now we prove the uniform continuity theorem of calculus. In the process, we are led to introduce a new notion that will prove to be surprisingly useful, that of a \idx{Lebesgue number} for an open covering of a metric space. First, a preliminary notion:
\end{proof}
...
\begin{lemma}[The \idx{Lebesgue number lemma}]
...
\begin{definition}
	A function \(f\) from the metric space \(\left( {X,{d}_{X}}\right)\) to the metric space \(\left( {Y,{d}_{Y}}\right)\) is said to be \idx{uniformly continuous} if given \(\epsilon  > 0\) , there is a \(\delta  > 0\) such that for every pair of points \({x}_{0},{x}_{1}\) of \(X\) ,
...
\begin{definition}
	If \(X\) is a space, a point \(x\) of \(X\) is said to be an \idx{isolated point} of \(X\) if the one-point set \(\{ x\}\) is open in \(X\)
\end{definition}
...
\item Let \(X\) be a metric space with metric \(d\) ; let \(A \subset  X\) be nonempty.
  ...
  \item Define the \(\epsilon\) -neighborhood of \(A\) in \(X\) to be the set \[ \idxmath{U}{\(U\left( {A,\epsilon }\right)\)} U\left( {A,\epsilon }\right)  = \{ x \mid  d\left( {x,A}\right)  < \epsilon \} . \] Show that \(U\left( {A,\epsilon }\right)\) equals the union of the open balls \({B}_{d}\left( {a,\epsilon }\right)\) for \(a \in  A\)
...
In some ways this property is more natural and intuitive than that of compactness. In the early days of topology, it was given the name ``compactness,'' while the open covering formulation was called ``\idx{bicompactness}.'' Later, the word ``compact'' was shifted to apply to the open covering definition, leaving this one to search for a new name It still has not found a name on which everyone agrees On historical grounds, some call it ``Fréchet compactness'', others call it the ``Bolzano-Weierstrass property'' We have invented the term ``\idx{limit point compactness}'' It seems as good a term as any; at least it describes what the property is about.
...
We now show these two versions of compactness coincide for metrizable spaces; for this purpose, we introduce yet another version of compactness called \idx{sequential compactness}. This result will be used in Chapter 7.
...
is an increasing sequence of positive integers, then the sequence \(\left( {y}_{i}\right)\) defined by setting \({y}_{i} = {x}_{{n}_{i}}\) is called a \idx{subsequence} of the sequence \(\left( {x}_{n}\right)\) . The space \(X\) is said to be sequentially compact if every sequence of points of \(X\) has a convergent subsequence.
\end{definition}
...
\item Let \(X\) be limit point compact.
  ...
  \end{enumerate}
\item A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \idx{countable compactness} is equivalent to limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
\item Let(X, d)be a metric space. If \(f : X \rightarrow  X\) satisfies the condition \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  = d\left( {x,y}\right) \] for all \(x,y \in  X\) , then \(f\) is called an \idx{isometry} of \(X\) . Show that if \(f\) is an isometry and \(X\) is compact, then \(f\) is bijective and hence a homeomorphism. [Hint: If \(a \notin  f\left( X\right)\) , choose \(\epsilon\) so that the \(\epsilon\) -neighborhood of \(a\) is disjoint from \(f\left( X\right)\) Set \({x}_{1} = a\) , and \({x}_{n + 1} = f\left( {x}_{n}\right)\) in general. Show that \(d\left( {{x}_{n},{x}_{m}}\right)  \geq  \epsilon\) for \(n \neq  m\) .]
\item Let(X, d)be a metric space. If \(f\) satisfies the condition \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  < d\left( {x,y}\right) \] for all \(x,y \in  X\) with \(x \neq  y\) , then \(f\) is called a \idx{shrinking map} If there is a number \(\alpha  < 1\) such that \[ d\left( {f\left( x\right) ,f\left( y\right) }\right)  \leq  {\alpha d}\left( {x,y}\right) \] for all \(x,y \in  X\) , then \(f\) is called a \idx{contraction}. A fixed point of \(f\) is a point \(x\) such that \(f\left( x\right)  = x\)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
In this section we study the notion of \idx{local compactness}, and we prove the basic theorem that any locally compact Hausdorff space can be imbedded in a certain compact Hausdorff space that is called its one-point \idx{compactification}.
