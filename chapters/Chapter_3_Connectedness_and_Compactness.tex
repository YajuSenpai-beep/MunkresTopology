...
\item Let(X, d)be a metric space. If \(f\) satisfies the condition \[ d( {f( x) ,f( y) }) < d( {x,y}) \] for all \(x,y \in X\) with \(x \neq y\) , then \(f\) is called a If there is a number \(\alpha < 1\) such that \[ d( {f( x) ,f( y) }) \leq {\alpha d}( {x,y}) \] for all \(x,y \in X\) , then \(f\) is called a . A fixed point of \(f\) is a point \(x\) such that \(f( x) = x\)
 \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
