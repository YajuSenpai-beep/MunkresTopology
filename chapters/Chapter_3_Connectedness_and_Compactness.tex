...
These theorems are used in a number of places. The intermediate value theorem is used for instance in constructing inverse functions, such as \(\sqrt[3]{x}\) and arcsin \(x\) ; and the maximum value theorem is used for proving the mean value theorem for derivatives, upon which the two fundamental theorems of calculus depend. The \index{Uniform continuity theorem}\index{Uniform continuity theorem}\index{uniform continuity theorem@Uniform continuity theorem}uniform continuity theorem is used, among other things, for proving that every continuous function is integrable.
...
\begin{definition}
	Let \(X\) be a topological space. A \index{separation} of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a \index{Separation}\index{Separation}\index{separation@Separation}separation of \(X\) .
\end{definition}
...
\item A space is \index{totally disconnected} if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is \index{Totally disconnected}\index{Totally disconnected}\index{totally disconnected@Totally disconnected}totally disconnected. Does the converse hold?
...
Connectedness of intervals in \(\mathbb{R}\) gives rise to an especially useful criterion for showing that a space \(X\) is connected; namely, the condition that every pair of points of \(X\) can be joined by a \index{Path}\index{Path}\index{path@Path}path in \(X\) :
...
\begin{definition}
	Let(X, d)be a metric space; let \(A\) be a nonempty subset of \(X\) . For each \(x \in  X\) , we define the \index{Distance from x to A} \index{distance from $x$ to $a$@Distance from $x$ to $A$}distance from \(x\) to \(A\) by the equation
...
Now we introduce the notion of \index{Lebesgue number}\index{Lebesgue number}\index{lebesgue number@Lebesgue number}Lebesgue number. Recall that the diameter of a bounded subset \(A\) of a metric space(X, d)is the number
...
\begin{theorem}[Uniform continuity theorem]
	Let \(f : X \rightarrow  Y\) be a continuous map of the compact metric space \(( {X,{d}_{X}})\) to the metric space \(( {Y,{d}_{Y}})\) . Then \(f\) is \index{Uniformly continuous}\index{Uniformly continuous}\index{uniformly continuous@Uniformly continuous}uniformly continuous.
...
\begin{theorem}
	Let \(X\) be a nonempty compact Hausdorff space. If \(X\) has no \index{Isolated point}\index{Isolated point}\index{isolated point@Isolated point}isolated points, then \(X\) is uncountable.
...
is an increasing sequence of positive integers, then the sequence \(( {y}_{i})\) defined by setting \({y}_{i} = {x}_{{n}_{i}}\) is called a \index{subsequence} of the sequence \(( {x}_{n})\) . The space \(X\) is said to be sequentially compact if every sequence of points of \(X\) has a convergent \index{Subsequence}\index{Subsequence}\index{subsequence@Subsequence}subsequence.
\end{definition}
...
\item Let \(X\) be limit point compact.
  ...
  \end{enumerate}
\item A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \index{countable compactness} is equivalent to \index{Limit point compactness}\index{Limit point compactness}\index{limit point compactness@Limit point compactness}limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
\item Let(X, d)be a metric space. If \(f\) satisfies the condition \[ d( {f( x) ,f( y) })  < d( {x,y}) \] for all \(x,y \in  X\) with \(x \neq  y\) , then \(f\) is called a \index{shrinking map} If there is a number \(\alpha  < 1\) such that \[ d( {f( x) ,f( y) })  \leq  {\alpha d}( {x,y}) \] \index{Fixed point theorem: for \({B}^{n}\)!for a contraction}for all \(x,y \in  X\) , then \(f\) is called a \index{contraction}. A \index{Fixed point}\index{fixed point@Fixed point}fixed point of \(f\) is a point \(x\) such that \(f( x)  = x\)
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item If \(f\) is a \index{Contraction}\index{Contraction}\index{contraction@Contraction}contraction and \(X\) is compact, show \(f\) has a unique fixed point. [Hint: Define \({f}^{1} = f\) and \({f}^{n + 1} = f \circ  {f}^{n}\) . Consider the intersection \(A\) of the sets \(. {{A}_{n} = {f}^{n}( X) \text{ . }}]\)
  \item Show more generally that if \(f\) is a \index{Shrinking map}\index{Shrinking map}\index{shrinking map@Shrinking map}shrinking map and \(X\) is compact, then \(f\) has a unique fixed point. [Hint: Let \(A\) be as before. Given \(x \in  A\) , choose \({x}_{n}\) so that \(x = {f}^{n + 1}( {x}_{n})\) . If \(a\) is the limit of some subsequence of the sequence \({y}_{n} = {f}^{n}( {x}_{n})\) , show that \(a \in  A\) and \(f( a)  = x\) . Conclude that \(A = f( A)\) , so that diam \(A = 0\) .]
...
\item Show that if \(G\) is a locally compact \index{Topological group}\index{topological group@Topological group}topological group and \(H\) is a subgroup, then \(G/H\) is locally compact.
...
10. Theorem. X is compact if and only if every net in \(X\) has a convergent subnet. [Hint: To prove the implication \(\Rightarrow\) , let \({B}_{\alpha } = \{  {{x}_{\beta } \mid  \alpha  \preccurlyeq  \beta }\}\) and show that \(\{  {B}_{\alpha }\}\) has the \index{Finite intersection property}\index{finite intersection property@Finite intersection property}finite intersection property. To prove \(\Leftarrow\) , let \(\mathcal{A}\) be a collection of closed sets having the finite intersection property, and let \(\mathcal{B}\) be the collection of all finite intersections of elements of \(\mathcal{A}\) , partially ordered by reverse inclusion.]
