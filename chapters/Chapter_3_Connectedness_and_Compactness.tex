\chapter[\index{connectedness@Connectedness}\index{connectedness@Connectedness}Connectedness \index{Least upper bound property!and compactness}\index{Least upper bound property!and compactness}and Compactness]{Connectedness \\and Compactness}
\vspace{6.5cm}
\chaptermark{Connectedness and Compactness}


In the study of calculus, there are three basic \index{Intermediate-value theorem} theorems about \index{Continuous image: of a compact space} continuous functions, and on these theorems the rest of calculus depends. They are the following:
...
\index{Maximum value theorem: of calculus} Maximum value theorem. If \(f : [  {a,b}]   \rightarrow  R\) is continuous, then there exists an element \(c \in  [  {a,b}]\) such that \(f( x)  \leq  f( c)\) for every \(x \in  [  {a,b}]\) .

\index{Uniform continuity theorem}. If \(f : [  {a,b}]   \rightarrow  \mathbb{R}\) is continuous, then given \(\epsilon  > 0\) , there exists \(\delta  > 0\) such that \(| {f( {x}_{1})  - f( {x}_{2}) }|  < \epsilon\) for every pair of numbers \({x}_{1},{x}_{2}\) of \([  {a,b}]\) for which \(| {{x}_{1} - {x}_{2}}|  < \delta\) .

These theorems are used in a number of places. The intermediate value theorem is used for instance in constructing inverse functions, such as \(\sqrt[3]{x}\) and arcsin \(x\) ; and the maximum value theorem is used for proving the mean value theorem for derivatives, upon which the two fundamental theorems of calculus depend. The \index{Uniform continuity theorem}\index{Uniform continuity theorem}\index{uniform continuity theorem@Uniform continuity theorem}\index{uniform continuity theorem@Uniform continuity theorem}uniform continuity theorem is used, among other things, for proving that every continuous function is integrable.

We have spoken of these three theorems as theorems about continuous functions. But they can also be considered as theorems about the \index{Closed graph theorem} closed interval \([  {a,b}]\) of real numbers. The theorems depend not only on the continuity of \(f\) but also on properties of the topological \index{Homogeneous space} space \([  {a,b}]\) .

The property of the space \([  {a,b}]\) on which the intermediate value theorem depends is the property called \index{connectedness}, and the property on which the other two depend is the property called \index{compactness}. In this chapter, we shall define these properties for arbitrary topological spaces, and shall prove the appropriate \index{General linear group} \index{Tube lemma!generalized}\index{Tube lemma!generalized}\index{Tube lemma!generalized}\index{Tube lemma!generalized}generalized versions of these theorems.

As the three quoted theorems are fundamental for the theory of calculus, so are the notions of \index{Connectedness}\index{Connectedness}connectedness \index{Least upper bound property!and compactness}\index{Least upper bound property!and compactness}and compactness fundamental in higher analysis, geometry, and topology-indeed, in almost any subject for which the notion of topological space itself is relevant.
...
\begin{definition}
	Let \(X\) be a topological space. A \index{separation} of \(X\) is a pair \(U,V\) of disjoint nonempty open subsets of \(X\) whose union is \(X\) . The space \(X\) is said to be connected if there does not exist a \index{Separation}\index{Separation}\index{separation@Separation}\index{separation@Separation}separation of \(X\) .
\end{definition}
...
For a subspace \(Y\) of a topological space \(X\) , there is another useful way \index{Closure!of a connected subspace}of formulating the definition \index{Closure!of a connected subspace}of connectedness:
...
\begin{centeredblock}
\begin{example}
	Let \(Y\) denote the subspace \([  {-1,0)\cup (0,1}]\) \index{Uncountability: of \(\mathcal{P}\left( {\mathbb{Z}}_{ + }\right)\)!of \(\mathbb{R}\)}\index{Uncountability: of \(\mathcal{P}\left( {\mathbb{Z}}_{ + }\right)\)!of \(\mathbb{R}\)}of the real line \(\mathbb{R}\) . Each of the sets \([  {-1,0)\text{ and }(0,1}]\) is nonempty and open in \(Y\) (although not in \(\mathbb{R}\) ); therefore, they form a separation of \(Y\) . Alternatively, note that neither of these sets contains a limit point of the other. (They do have a limit point 0 in common, but that does not matter.)
\end{example}
\end{centeredblock}
...
\beg\\index{Slice: in covering space!in product space}index{Slice: in covering space!in product space}in{pro\index{Connectedness!of a product space}of}
...
\begin{centeredblock}
\begin{example}
	Consider the cartesian product \({\mathbb{R}}^{\omega }\) \\index{Connectedness!in box topology}index{Connectedness!in box topology}in the box topology. We can write \({\mathbb{R}}^{\omega }\) as the union of the set \(A\) consisting of all bounded sequences of real numbers, and the set \(B\) of all unbounded sequences. These sets are disjoint, and each is open in the box topology For if \(\mathbf{a}\) is a point \index{Connectedness!of \({\mathbb{R}}^{\omega }\)}\index{Connectedness!of \({\mathbb{R}}^{\omega }\)}of \({\mathbb{R}}^{\omega }\) , the open set
...
\item Show that if \(X\) is an \\index{Compactness!in finite complement topology}index{Compactness!in finite complement topology}infinite set, it is connected in the finite complement topology.
\item A space is \index{totally disconnected} if its only connected subspaces are one-point sets. Show that if \(X\) has the discrete topology, then \(X\) is \index{Totally disconnected}\index{Totally disconnected}\index{totally disconnected@Totally disconnected}\index{totally disconnected@Totally disconnected}totally disconnected. Does the converse hold?
...
The fact that intervals and rays in \(\mathbb{R}\) are connected may be familiar to you from analysis. We prove it again here, in generalized form. It turns out that this fact does not depend on the algebraic properties of \(\mathbb{R}\) , but only on its order properties. To make this clear, we shall prove the theorem for an arbitrary ordered set that has the order properties \index{Connectedness!of ordered square}of \(\mathbb{R}\) . Such a set \index{Ordered square!is linear continuum}\index{Ordered square!is linear continuum}is called a linear continuum.
...
\beg\\index{Compactness!in order topology}index{Compactness!in order topology}in{theorem}
...
The intermediate value theorem of calculus is the \index{Baire category theorem!special case}\index{Baire category theorem!special case}\index{Baire category theorem!special case}\index{Baire category theorem!special case}special case of this theorem that occurs when we take \(X\) to be a closed interval in \(\mathbb{R}\) and \(Y\) to be \(\mathbb{R}\) .
...
Connectedness of intervals in \(\mathbb{R}\) gives rise to an especially useful criterion for showing that a space \(X\) is connected; namely, the condition that every pair of points of \(X\) can be joined by a \index{Path}\index{Path}\index{path@Path}\index{path@Path}path in \(X\) :
...
\item Let \(f : X \rightarrow  X\) be continuous. Show that if \(X = [  {0,1}]\) , there is a point \(x\) such that \(f( x)  = x\) . The point \(x\) is called a \index{Fixed point}fixed point of \(f\) . What happens if \(X\) equals \(\lbrack 0,1)\) or \(( {0,1}) {}^{7}\)
...
\item *12. Recall that \({S}_{\Omega }\) denotes the minimal uncountable well-ordered set. Let \(L\) denote the ordered set \({S}_{\Omega } \times  \lbrack 0,1)\) in the dictionary order, with its smallest element deleted. The set \(L\) is a classical example in topology called the \index{long line}. \textbf{Theorem.} The \index{Linear continuum!long line}long line is path connected and \index{Local path connectedness} locally homeomorphic to \(\mathbb{R}\) , but it cannot be imbedded in \(\mathbb{R}\) .
  ...
  \item Show that every point of \(L\) has a \index{Symmetric neighborhood} neighborhood homeomorphic with an open interval in \(\mathbb{R}\) .
...
\item Let \(X\) be a space. Let us define \(x \sim  y\) if there is no separation \(X = A \cup  B\) of \(X\) into disjoint open sets such that \(x \in  A\) and \(y \in  B\) .
  ...
  \item Show that each component of \(X\) lies in a \index{Quasicomponent}quasicomponent of \(X\) , and that the components and quasicomponents of \(X\) are the same if \(X\) is locally connected.
...
\begin{centeredblock}
\begin{example}
	The \index{Tube lemma}tube lemma is certainly not true if \(Y\) is not compact For example, let \(Y\) be the \(y\) -axis in \({\mathbb{R}}^{2}\) , and let
...
\begin{theorem}
	Let \(X\) be a topological space. Then \(X\) is compact if and only if for every collection \(\mathcal{C}\) of closed sets in \(X\) having the \index{Finite intersection property}finite intersection property, the intersection \(\mathop{\bigcap }\limits_{{C \in  \mathcal{C}}}C\) of all the elements of \(\mathcal{C}\) is nonempty.
\end{theorem}
\begin{proof}
...
\item Theorem. Let \(f : X \rightarrow  Y\) ; let \(Y\) be compact Hausdorff. Then \(f\) is continuous if and only if the \index{Graph of a function} graph of \(f\) , \[ {G}_{f} = \{ x \times  f( x)  \mid  x \in  X\} \] is closed in \(X \times  Y\) . [Hint: If \({G}_{f}\) is closed and \(V\) is a neighborhood of \(f( {x}_{0})\) , then the intersection of \({G}_{f}\) and \(X \times  ( {Y - V})\) is closed. Apply Exercise 7.]
...
\item Let \(G\) be a \index{Topological group}topological group.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\begin{centeredblock}
\begin{example}
	The unit sphere \({S}^{n - 1}\) and the closed unit ball \({B}^{n}\) in \({\mathbb{R}}^{n}\) are compact because they are closed and bounded. The set
...
Now we prove the \index{Extreme value theorem}\index{Extreme value theorem}\index{extreme value theorem@Extreme value theorem}extreme value theorem of calculus, in suitably generalized form.
...
forms an \index{Open covering}\index{open covering@Open covering}open covering of \(A\) . Since \(A\) is compact, some finite subcollection
...
Now we prove the uniform continuity theorem of calculus. In the process, we are led to introduce a new notion that will prove to be surprisingly useful, that of a \index{Lebesgue number} for an open covering of a metric space. First, a preliminary notion:
\end{proof}

\begin{definition}
	Let(X, d)be a metric space; let \(A\) be a nonempty subset of \(X\) . For each \(x \in  X\) , we define the \index{Distance from x to A} \index{distance from $x$ to $a$@Distance from $x$ to $A$}\index{distance from $x$ to $a$@Distance from $x$ to $A$}distance from \(x\) to \(A\) by the equation
...
Now we introduce the notion of \index{Lebesgue number}\index{Lebesgue number}\index{lebesgue number@Lebesgue number}\index{lebesgue number@Lebesgue number}Lebesgue number. Recall that the diameter of a bounded subset \(A\) of a metric space(X, d)is the number

\[
\sup \{  {d( {{a}_{1},{a}_{2}})  \mid  {a}_{1},{a}_{2} \in  A}\}  .
\]

\begin{lemma}[The \index{Lebesgue number lemma}]
	Let \(\mathcal{A}\) be an \index{open covering@Open covering}open covering of the metric space(X, d). If \(X\) is compact, there is a \(\delta  > 0\) such that for each subset of \(X\) having diameter less than \(\delta\) , there exists an element of \(\mathcal{A}\) containing it.
\end{lemma}
...
\begin{theorem}[Uniform continuity theorem]
	Let \(f : X \rightarrow  Y\) be a continuous map of the compact metric space \(( {X,{d}_{X}})\) to the metric space \(( {Y,{d}_{Y}})\) . Then \(f\) is \index{Uniformly continuous}\index{Uniformly continuous}\index{uniformly continuous@Uniformly continuous}\index{uniformly continuous@Uniformly continuous}uniformly continuous.
...
\begin{theorem}
	Let \(X\) be a nonempty compact Hausdorff space. If \(X\) has no \index{Isolated point}\index{Isolated point}\index{isolated point@Isolated point}\index{isolated point@Isolated point}isolated points, then \(X\) is uncountable.
...
\item Recall that \({\mathbb{R}}_{K}\) denotes \(\mathbb{R}\) in the \(K\) -topology.
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that \([  {0,1}]\) is not compact as a subspace \index{Connectedness!of \({\mathbb{R}}_{K}\)}\index{Connectedness!of \({\mathbb{R}}_{K}\)}of \({\mathbb{R}}_{K}\) .
...
is an increasing sequence of positive integers, then the sequence \(( {y}_{i})\) defined by setting \({y}_{i} = {x}_{{n}_{i}}\) is called a \index{subsequence} of the sequence \(( {x}_{n})\) . The space \(X\) is said to be sequentially compact if every sequence of points of \(X\) has a convergent \index{Subsequence}\index{Subsequence}\index{subsequence@Subsequence}\index{subsequence@Subsequence}subsequence.
\end{definition}
...
\item Let \(X\) be limit point compact.
  ...
  \item If \(X\) is a subspace of the Hausdorff space \(Z\) , does it follow that \(X\) is closed in \(Z\) ? We comment that it is not in general true that the product of two limit point compact spaces is limit point compact, even if the \index{Topological group!Hausdorff condition}\index{Topological group!Hausdorff condition}\index{Topological group!Hausdorff condition}Hausdorff condition is assumed. But the examples are fairly sophisticated. See \cite{SteenSeebach1970}, Example II2.
  \end{enumerate}
\item A space \(X\) is said to be countably compact if every countable open covering of \(X\) contains a finite subcollection that covers \(X\) . Show that for a \({T}_{1}\) space \(X\) , \index{countable compactness} is equivalent to \index{Limit point compactness}\index{Limit point compactness}\index{limit point compactness@Limit point compactness}\index{limit point compactness@Limit point compactness}limit point compactness. [Hint: If no finite subcollection of \({U}_{n}\) covers \(X\) , choose \({x}_{n} \notin  {U}_{1} \cup  \cdots  \cup  {U}_{n}\) , for each \(n\) .]
...
\begin{centeredblock}
\begin{example}
	The space \({\mathbb{R}}^{n}\) is locally compact, the point \(x\) lies in some basis element \(( {{a}_{1},{b}_{1}})  \times   \cdot   \times  ( {{a}_{n},{b}_{n}})\) , which in turn lies in the compact subspace \([  {{a}_{1},{b}_{1}}]   \times   \cdot   \times  [  {{a}_{n},{b}_{n}}]\) . The space \({\mathbb{R}}^{\omega }\) is not locally compact, none of its basis elements are contained in \index{Linear continuum!compact subspaces}compact subspaces For if
...
\end{theorem}
\begin{proof}
Step 1. We first verify \index{One-point compactification!uniqueness}\index{One-point compactification!uniqueness}\index{One-point compactification!uniqueness}\index{One-point compactification!uniqueness}uniqueness. Let \(Y\) and \({Y}^{\prime }\) be two spaces satisfying these conditions. Define \(h : Y \rightarrow  {Y}^{\prime }\) by letting \(h\) map the single point \(p\) of \(Y - X\) to the point \(q\) of \({Y}^{\prime } - X\) , and letting \(h\) equal the identity on \(X\) . We show that if \(U\) is open in \(Y\) , then \(h( U)\) is open in \({Y}^{\prime }\) . Symmetry then implies that \(h\) is a homeomorphism.
...
\begin{definition}
	If \(Y\) is a compact Hausdorff space and \(X\) is a proper subspace of \(Y\) whose closure equals \(Y\) , then \(Y\) is said to be a compactification of \(X\) . If \(Y - X\) equals a single point, then \(Y\) is called the \index{One-point compactification}\index{One-point compactification}\index{one-point compactification@One-point compactification}\index{one-point compactification@One-point compactification}one-point compactification of \(X\) .
\end{definition}
...
\item Show that if \(G\) is a locally compact \index{Topological group}\index{topological group@Topological group}\index{topological group@Topological group}topological group and \(H\) is a subgroup, then \(G/H\) is locally compact.
...
then the composite function \(f \circ  g : K \rightarrow  X\) is called a subnet of \(( {x}_{\alpha })\) . Show that if the net \(( {x}_{\alpha })\) converges to \(x\) , so does any subnet.

9. Let \({( {x}_{\alpha }) }_{\alpha  \in  J}\) be a net in \(X\) . We say that \(x\) is an accumulation point of the net \(( {x}_{\alpha })\) if for each neighborhood \(U\) of \(x\) , the set of those \(\alpha\) for which \({x}_{\alpha } \in  U\) is cofinal in \(J\) .
...
10. Theorem. X is compact if and only if every net in \(X\) has a convergent subnet. [Hint: To prove the implication \(\Rightarrow\) , let \({B}_{\alpha } = \{  {{x}_{\beta } \mid  \alpha  \preccurlyeq  \beta }\}\) and show that \(\{  {B}_{\alpha }\}\) has the \index{Finite intersection property}\index{finite intersection property@Finite intersection property}\index{finite intersection property@Finite intersection property}finite intersection property. To prove \(\Leftarrow\) , let \(\mathcal{A}\) be a collection of closed sets having the finite intersection property, and let \(\mathcal{B}\) be the collection of all finite intersections of elements of \(\mathcal{A}\) , partially ordered by reverse inclusion.]
