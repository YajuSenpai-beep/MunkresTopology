% 封面页
\cleardoublepage
\thispagestyle{empty}
\vspace*{0.05\paperheight}

% Topology和Second Edition部分 - 居中容器，内容右对齐
\begin{center}
\parbox{0.6\textwidth}{\raggedleft
\sbox{0}{\fontsize{40}{32}\selectfont\bfseries Topology}
\usebox{0}\par
\vspace{0.5cm}
\makebox[\wd0]{\fontsize{24}{30}\selectfont\textbf{\addfontfeature{FakeSlant=0.2}Second Edition}}\par
}
\end{center}

\vspace{6cm}

% James R. Munkres和MIT信息 - 居中容器，内容右对齐
\begin{center}
\parbox{0.6\textwidth}{\raggedleft
{\fontsize{24}{30}\selectfont\bfseries James R. Munkres} \\
\vspace{0.1cm}
{\fontsize{16}{10}\selectfont\textbf{\addfontfeature{FakeSlant=0.2}Massachusetts Institute of Technology}}\par
}
\end{center}

% 将出版社信息推到底部并居中
\vfill
\begin{center}
{\large\textsl{Prentice Hall, Upper Saddle River, NJ 07458}\par}
\end{center}
\clearpage
