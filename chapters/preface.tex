...
\begin{center}
 ...
 % 章节号与标题之间的间距（可调: \quad, \qquad, \hspace{<dim>} 等）
 \def\chaptersep{\qquad}

 \node (c9)  at (0,0)                {Chapter 9\quad The Fundamental Group};
 \node (c10) at (\indent, -\rowsep)   {Chapter 10\quad Separation Theorems in the Plane};
 \node (c11) at (\indent, -2*\rowsep) {Chapter 11\quad The Seifert-van Kampen Theorem};
 \node (c12) at (\indent, -3*\rowsep) {Chapter 12\quad Classification of Surfaces};
 \node (c13) at (\indent, -4*\rowsep) {Chapter 13\quad Classification of Covering Spaces};
 \node (c14) at (0, -5*\rowsep)       {Chapter 14\quad Applications to Group Theory};
 ...
 \draw[->] ($(c12.base) + (\hoffsetCone+0.23cm, \vtop+0.8cm)$) -- ($(c12.base) + (\hoffsetCone+0.23cm, \vtop)$);  % 起点x=终点x(同锚点引用), y高0.8cm为原c11底位置→竖直
