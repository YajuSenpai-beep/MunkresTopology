...
\begin{center}
	\munkrescomm{0.4755}{%
		...
		\node (U)  at (635,  -15) {$\pi_1(U, x_0)$};
		\node (X)  at (610, -260) {$\pi_1(X, x_0)$};
		\node (V)  at (640, -500) {$\pi_1(V, x_0)$};
		\node (H)  at (955, -260) {$H$};
		\draw[->] (UV) -- node[above left=-2pt,pos=.45] {$i_1$} (U);
		\draw[->] (UV) -- node[below left=-2pt,pos=.45] {$i_2$} (V);
		\draw[->] (UV) -- (X);
		\draw[->] (U) -- node[right] {$j_1$} (X);
		\draw[->] (V) -- node[right] {$j_2$} (X);
		\draw[->] (U) -- node[above right=-3pt,pos=.45] {$\phi_1$} (H);
		\draw[->] (V) -- node[below right=-3pt,pos=.45] {$\phi_2$} (H);
