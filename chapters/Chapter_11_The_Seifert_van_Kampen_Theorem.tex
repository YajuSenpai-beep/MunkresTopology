...
% 交换图 com_70_00：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.4755}{%
		\node (UV) at (143.5, -256.7) {$\pi_1(U \cap V, x_0)$};
		\node (U)  at (633.3,  -11.8) {$\pi_1(U, x_0)$};
		\node (X)  at (633.3, -256.7) {$\pi_1(X, x_0)$};
		\node (V)  at (633.3, -496.8) {$\pi_1(V, x_0)$};
		\node (H)  at (957.8, -256.7) {$H$};
		\draw[->] (UV) -- node[inner sep=0pt,above left=3pt,pos=.45] {$i_1$} (U);
		\draw[->] (UV) -- node[inner sep=0pt,below left=3pt,pos=.45] {$i_2$} (V);
		\draw[->] (UV) -- (X);
		\draw[->] (U) -- node[inner sep=0pt,right=6pt] {$j_1$} (X);   % 再往右挪一点
		\draw[->] (V) -- node[inner sep=0pt,right=6pt] {$j_2$} (X);   % 再往右挪一点
		\draw[->] (U) -- node[inner sep=0pt,above right=3pt,pos=.45] {$\phi_1$} (H);
		\draw[->] (V) -- node[inner sep=0pt,below right=3pt,pos=.45] {$\phi_2$} (H);
		\draw[->] (X) -- node[inner sep=0pt,above=3.5pt] {$\Phi$} (H);   % 默认 pos=.5。原书实测：X 右缘 203.3、H 左缘 228.7（间隙中点 216.0），Φ 墨迹中心 215.65 —— 就在间隙正中。fix3.62 曾改 .38，偏左。
	}
	\label{fig:com_70_00}
\end{center}
\par\nopagebreak 
...
% 交换图 com_70_01：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.4758}{%
		\node (UV) at (143.5, -256.7) {$\pi_1(U \cap V, x_0)$};
		\node (U)  at (633.3,  -11.9) {$\pi_1(U, x_0)$};
		\node (X)  at (633.3, -256.7) {$\pi_1(X, x_0)$};
		\node (V)  at (633.3, -496.9) {$\pi_1(V, x_0)$};
		\node (H)  at (957.8, -256.7) {$H$};
		\draw[->] (UV) -- node[inner sep=0pt,above left=3pt,pos=.45] {$i_1$} (U);
		\draw[->] (UV) -- node[inner sep=0pt,below left=3pt,pos=.45] {$i_2$} (V);
		\draw[->] (UV) -- node[inner sep=0pt,above=3.5pt,pos=.5] {$i_*$} (X);
		\draw[->] (U) -- node[inner sep=0pt,right=6pt] {$j_1$} (X);   % 再往右挪一点
		\draw[->] (V) -- node[inner sep=0pt,right=6pt] {$j_2$} (X);   % 再往右挪一点
		\draw[->] (U) -- node[inner sep=0pt,above right=3pt,pos=.45] {$\phi_1$} (H);
		\draw[->] (V) -- node[inner sep=0pt,below right=3pt,pos=.45] {$\phi_2$} (H);
		% ⚠ 这两支箭头之间只有 12pt，标签又被 φ₁/φ₂ 两条斜线穿过（斜线都收在 H 左缘）。
		%   拉开到 ±9pt，并把标签再推离线 2pt。
		% 用户要求：k（H→X）与 Φ（X→H）**合成一支双头箭头**（原来是上下两根）
		\draw[<->] (X.east) -- node[inner sep=0pt,above=3.5pt,pos=.35] {$k$}
		                     node[inner sep=0pt,below=4pt,pos=.35] {$\Phi$} (H.west);   % pos 与 k 同取 .35：原书 k、Φ 同在箭头中心一列（.45 时 Φ 比 k 靠右）
	}
	\label{fig:com_70_01}
\end{center}
\par\nopagebreak 
...
\begin{lemma}
\label{lem:\thetheorem}
Given an index set \(J\), there exists a space \(X\) that is a wedge of circles \(S_{\alpha }\) for \(\alpha \in J\).
\end{lemma}
...
% 交换图 com_72_10：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.2883}{%
		\node (A) at (187.1,  -49.1) {$\pi_1(U,b)$};
		\node (B) at (838.2,  -49.1) {$\pi_1(X,b)$};
		\node (C) at (187.1, -434.1) {$\pi_1(U,a)$};
		\node (D) at (838.2, -434.1) {$\pi_1(X,a)$};
		\draw[->] (A) -- (B);
		% ⚠ 符号是 δ̂ 不是 α̂：前文写「The isomorphisms induced by the path δ
		%   (both of which we denote by δ̂)」，原书图上也是 δ̂。原稿误作 \hat{\alpha}。
		%   两标签都在各自箭头**右侧**（原书如此；原先左边那支写在左侧）。
		\draw[->] (A) -- node[inner sep=0pt,right=6pt] {$\hat{\delta}$} (C);   % 往右挪一点
		\draw[->] (B) -- node[inner sep=0pt,right=6pt] {$\hat{\delta}$} (D);   % 往右挪一点
		\draw[->] (C) -- (D);
	}
	\label{fig:com_72_10}
\end{center}
...
Therefore, the homomorphism of \({\pi }_{1}( {U,a})\) into \({\pi }_{1}( {X,a})\) induced by inclusion is surjective, and its kernel is the least normal subgroup containing the element \(\hat{\delta }( [ g_{0}] )\).

\munkresfig[0.5]{72.2}[Figure 72.2]
The loop \(f_{0}\) represents a generator of the fundamental group of Int \(B^{2} - 0\) based at \(q\). Then the loop \(\bar{\gamma } * ( {f_{0} * \gamma })\) represents a generator of the fundamental group of \(B^{2} - 0\) based at \(p\). Therefore, it is path homotopic either to \(f\) or its reverse; suppose the former. Following this path homotopy by the map \(h\), we see that \(\bar{\delta } * ( {g_{0} * \delta })\) is path homotopic in \(U\) to \(g\). Then \(\hat{\delta }( [ g_{0}] ) = [ g]\), and the theorem follows.
