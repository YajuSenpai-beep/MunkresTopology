...
where each collection \({\mathcal{B}}_{n}\) is locally finite. We say that such a collection \(\mathcal{B}\) is \index{countably locally finite}. Surprisingly enough, this condition, along with regularity, is both necessary and sufficient for metrizability of \(X\) . This we shall we prove.

There is another concept in topology that involves the notion of local finiteness. It is a \index{Shrinking lemma!general}\index{Shrinking lemma!general}\index{Shrinking lemma!general}\index{Shrinking lemma!general}generalization of the concept of compactness called ``\index{paracompactness}.'' Although of fairly recent origin, it has proved useful in many parts of mathematics. We introduce it here so that we can give another set of necessary and sufficient conditions for a space \(X\) to be metrizable. It turns out that \(X\) is metrizable if and only if it is both paracompact and locally metrizable. This we prove in §42.
...
In this sections we prove some elementary properties of \index{locally finite collection} and a crucial lemma about metrizable spaces.
...
\begin{lemma}
	Let \(\mathcal{A}\) be a \index{Locally finite collection}\index{Locally finite collection}\index{locally finite collection@Locally finite collection}\index{locally finite collection@Locally finite collection}locally finite collection of subsets of \(X\) . Then:
...
\end{proof}
\begin{definition}
	A collection \(\mathcal{B}\) of subsets of \(X\) is said to be \index{Countably locally finite}\index{Countably locally finite}\index{countably locally finite@Countably locally finite}\index{countably locally finite@Countably locally finite}countably locally finite if \(\mathcal{B}\) can be written as the countable union of collections \({\mathcal{B}}_{n}\) , each of which is locally finite.
\end{definition}
...
\begin{definition}
	Let \(\mathcal{A}\) be a collection of subsets of the space \(X\) . A collection \(\mathcal{B}\) of subsets of \(X\) is said to be a \index{refinement} of \(\mathcal{A}\) (or is said to refine \(\mathcal{A}\) ) if for each element \(B\) of \(\mathcal{B}\) , there is an element \(A\) of \(\mathcal{A}\) containing \(B\) . If the elements of \(\mathcal{B}\) are open sets, we call \(\mathcal{B}\) an \index{open refinement} of \(\mathcal{A}\) ; if they are closed sets, we call \(\mathcal{B}\) a \index{closed refinement}
\end{definition}
...
\item Find a \index{Point-finite collection} point-finite open covering \(\mathcal{A}\) of \(\mathbb{R}\) that is not locally finite. (The collection \(\mathcal{A}\) is point-finite if each point of \(\mathbb{R}\) lies in only finitely many elements of \(\mathcal{A}\) .)
...
\item Let \(\mathcal{A}\) be the following collection of subsets of \(\mathbb{R}\) : \[ A = \{ ( {n,n + 2})  \mid  n \in  \mathbb{Z}\} \] Which of the following collections refine \(\mathcal{A}\) ? \[ \mathcal{B} = \{ ( {x,x + 1})  \mid  x \in  \mathbb{R}\} , \] \[ \mathcal{C} = \{  {( {n,n + \frac{3}{2}})  \mid  n \in  \mathbb{Z}}\}  , \] \[ \mathcal{D} = \{  {( {x,x + \frac{3}{2}})  \mid  x \in  \mathbb{R}}\}  . \]
...
\item Consider \({\mathbb{R}}^{\omega }\) in the uniform topology. Given \(n\) , let \({\mathcal{B}}_{n}\) be the collection \index{Paracompactness!of \({\mathbb{R}}^{\omega }\)}\index{Paracompactness!of \({\mathbb{R}}^{\omega }\)}of all subsets of \({\mathbb{R}}^{\omega }\) of the form \(\prod {A}_{i}\) , where \({A}_{i} = \mathbb{R}\) for \(i \leq  n\) and \({A}_{i}\) equals either \(\{ 0\}\) or \(\{ 1\}\) otherwise. Show that the collection \(\mathcal{B} = \bigcup {\mathcal{B}}_{n}\) is countably locally finite, but neither countable nor locally finite.
\end{enumerate}
...
\begin{proof}
	We gave this as an exercise in \(\$ {33}\) , we provide a pro\index{Metrizability: of compact Hausdorff space!of Stone-Čech compactification}of here. Write \(A\) as the intersection \index{Stone-Čech compactification!of \({\mathbb{Z}}_{ + }\)}\index{Stone-Čech compactification!of \({\mathbb{Z}}_{ + }\)}of the open sets \({U}_{n}\) , for \(n \in  {\mathbb{Z}}_{ + }\) For each \(n\) , choose a continuous function \({f}_{n} : X \rightarrow  [  {0,1}]\) such that \(f( x)  = 0\) for \(x \in  A\) and \(f( x)  = 1\) for \(x \in  X - {U}_{n}\) Define \(f( x)  = \sum {f}_{n}( x) /{2}^{n}\) The series converges uniformly, by comparison with \(\sum 1/{2}^{n}\) , so that \(f\) is continuous. Also, \(f\) vanishes on \(A\) and is positive on \(X - A\)
\end{proof}
\begin{theorem}[\index{Nagata-Smirnov metrization theorem}]
...
Note that on the subspace \({[  0,1]  }^{J}\) \index{Paracompactness!of \({\mathbb{R}}^{J}\)}\index{Paracompactness!of \({\mathbb{R}}^{J}\)}of \({\mathbb{R}}^{J}\) , the uniform metric equals the metric
...
\item A collection \(\mathcal{A}\) of subsets of \(X\) is said to be \index{locally discrete} if each point of \(X\) has a neighborhood that intersects at most one element of \(\mathcal{A}\) . A collection \(\mathcal{B}\) is \index{countably locally discrete} (or `` \(\sigma\) -\index{Locally discrete}\index{Locally discrete}\index{locally discrete@Locally discrete}\index{locally discrete@Locally discrete}locally discrete'') if it equals a countable union of locally discrete collections. Prove the following: \textsl{Theorem. (\index{Bing metrization theorem}).} A space \(X\) is metrizable if and only if it is regular and has a basis that is \index{Countably locally discrete}\index{Countably locally discrete}\index{countably locally discrete@Countably locally discrete}\index{countably locally discrete@Countably locally discrete}countably locally discrete. 
\end{enumerate}
\section*{§ 41 Paracompactness}

The concept \index{Normality!of paracompact Hausdorff space}\index{Normality!of paracompact Hausdorff space}of \index{Compact Hausdorff space: is Baire space!paracompactness}\index{Compact Hausdorff space: is Baire space!paracompactness}paracompactness is one of the most useful generalizations of compactness that has been discovered in recent years. It is particularly useful for applications in topology and differential geometry We shall give just one application, a metrization theorem that we prove in the next section.

Many of the spaces that are familiar to us already are paracompact. For instance, every compact space is paracompact, this will be an immediate consequence of the definition. It is also true that every metrizable space is paracompact, this is a theorem due to A. H. Stone, which we shall prove. Thus the class of \index{paracompact space} includes the two most important classes of spaces we have studied. It includes many other spaces as well.
...
A space \(X\) is compact if every open covering \(\mathcal{A}\) of \(X\) has a finite \index{Open refinement}\index{Open refinement}\index{open refinement@Open refinement}\index{open refinement@Open refinement}open refinement \(\mathcal{B}\) that covers \(X\)
...
\begin{centeredblock}
\begin{example}
	The space \({\mathbb{R}}^{n}\) is paracompact Let \(X = {\mathbb{R}}^{n}\) Let \(\mathcal{A}\) be an open covering of \(X\) . Let \({B}_{0} = \varnothing\) , and for each positive integer \(m\) , let \({B}_{m}\) denote the open ball of radius \(m\) centered at the origin. Given \(m\) , choose finitely many elements of \(\mathcal{A}\) that cover \({\widetilde{B}}_{m}\) and intersect each one with the open set \(X - {\bar{B}}_{m - 1}\) , let this finite collection of open sets be denoted \({\mathcal{C}}_{m}\) Then the collection \(\mathcal{C} = \bigcup {\mathcal{C}}_{m}\) is a refinement of \(\mathcal{A}\) It is clearly locally finite, for the open set \({B}_{m}\) intersects only finitely many elements of \(\mathcal{C}\) , namely those elements belongingg to the collection \({\mathcal{C}}_{1} \cup  \; \cup  {\mathcal{C}}_{m}\) . Finally, \(\mathcal{C}\) covers \(X\) For, given \(x\) , let \(m\) be the smallest integer such that \(x \in  {\bar{B}}_{m}\) . Then \(x\) belongs to an element of \({\mathcal{C}}_{m}\) , by definition.
\end{example}
\end{centeredblock}

Some of the properties of a \index{Paracompact space}\index{Paracompact space}\index{paracompact space@Paracompact space}\index{paracompact space@Paracompact space}paracompact space are similar to those \index{Paracompactness!of closed subspace}of a compact space. For instance, a subspace \index{Paracompactness!of closed subspace}of a paracompact space is not necessarily paracompact; but a closed subspace is paracompact. Also, a paracompact Hausdorff space is normal. In other ways, a paracompact space is not similar to a compact space; in particular, the product of two paracompact spaces need not be paracompact. We shall verify these facts shortly.
...
\(( 3)  \Rightarrow  ( 4)\) . Let \(\mathcal{A}\) be an open covering of \(X\) . Using(3), choose \(\mathcal{B}\) to be a refinement of \(\mathcal{A}\) that covers \(X\) and is locally finite (We can take \(\mathcal{B}\) to be a \index{Closed refinement}\index{Closed refinement}\index{closed refinement@Closed refinement}\index{closed refinement@Closed refinement}closed refinement if we like, but that is irrelevant.) We seek to expand each element \(B\) of \(\mathcal{B}\) slightly to an open set, making the expansion slight enough that the resulting collection of open sets will still be locally finite and will still refine \(\mathcal{A}\) .
...
(2) The indexed family \(\{  {\text{ Support }{\phi }_{\alpha }}\}\) is locally finite
...
-Lemma 41.6. Let \(X\) be a paracompact Hausdorff space; let \({\{  {U}_{\alpha }\}  }_{\alpha  \in  J}\) be an indexed family of open sets covering \(X\) . Then there exists a locally finite indexed family \({\{  {V}_{\alpha }\}  }_{\alpha  \in  J}\) of open sets covering \(X\) such that \({\widetilde{V}}_{\alpha } \subset  {U}_{\alpha }\) for each \(\alpha\) .

The condition that \({\widetilde{V}}_{\alpha } \subset  {U}_{\alpha }\) for each \(\alpha\) is sometimes expressed by saying that the family \(\{  {\widetilde{V}}_{\alpha }\}\) is a \index{precise refinement} of the family \(\{  {U}_{\alpha }\}\) .
...
-Theorem 41.7. Let \(X\) be a paracompact Hausdorff space; let \({\{  {U}_{\alpha }\}  }_{\alpha  \in  J}\) be an indexed open covering of \(X\) . Then there exists a partition of unity on \(X\) dominated by \(\{  {U}_{\alpha }\}\) .

Proof. We begin by applying the shrinking lemma twice, to find locally finite indexed familes of open sets \(\{  {W}_{\alpha }\}\) and \(\{  {V}_{\alpha }\}\) covering \(X\) , such that \({\bar{W}}_{\alpha } \subset  {V}_{\alpha }\) and \({\bar{V}}_{\alpha } \subset  {U}_{\alpha }\) for each \(\alpha\) Since \(X\) is normal, we may choose, for each \(\alpha\) , a continuous function \({\psi }_{\alpha } : X \rightarrow  [  {0,1}]\) such that \({\psi }_{\alpha }( {\bar{W}}_{\alpha })  = \{ 1\}\) and \({\psi }_{\alpha }( {X - {V}_{\alpha }})  = \{ 0\}\) . Since \({\psi }_{\alpha }\) is nonzero only at points of \({V}_{\alpha }\) , we have
...
\item
  ...
  \end{enumerate}
\item Let \(X\) be paracompact. We proved a ``shnnking lemma'' for arbitrary indexed open coverings of \(X\) . Here is an ``\index{expansion lemma}'' for arbitrary locally finite indexed families in \(X\) . \textsl{Lemma} Let \({\{  {B}_{\alpha }\}  }_{\alpha  \in  J}\) be a locally finite indexed family of subsets of the para-compact Hausdorff space \(X\) . Then there \(\iota\) s a locally finite indexed family \({\{  {U}_{\alpha }\}  }_{\alpha  \in  J}\) of open sets in \(X\) such that \({B}_{\alpha } \subset  {U}_{\alpha }\) for each \(\alpha\) . 
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\[
{B}_{{C}_{i}}( {x,\epsilon })  \subset  ( {U \cap  {C}_{i}}) .
\]
...
the point \(x\) belongs to \(C\) , so that \(C\) must be one of the sets \({C}_{1},\ldots ,{C}_{k}\) . Say \(C = {C}_{i}\) . Since \({B}_{C}( {y,1/m})\) has diameter at most \(2/m < {\epsilon }_{t}\) , it follows that

\[
x \in  D \subset  {B}_{{C}_{1}}( {y,1/m})  \subset  {B}_{{C}_{1}}( {x,{\epsilon }_{i}})  \subset  U,
\]
...
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that for each \(x \in  {S}_{\Omega }\) , the section \index{Paracompactness!of \({S}_{\Omega }\)}\index{Paracompactness!of \({S}_{\Omega }\)}of \({S}_{\Omega }\) by \(x\) has a countable basis and hence is metrizable.
