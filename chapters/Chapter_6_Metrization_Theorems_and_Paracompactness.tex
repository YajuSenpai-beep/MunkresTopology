...
% ⚠ 本图坐标记的是「该行基线左端」，与 \munkrescomm 默认的 anchor=center 不同，
%   故每个 \node 用 [anchor=base west] 覆盖（TikZ 中逐节点选项优先级高于 every node）。
%   §39 (243.7, 0)      §40 (0, −87.8)     The Nagata-Smirnov… (311.8, −85.4)
%   §41 (241.3, −175.0)                    §42 (242.5, −264.6)
%   箭头（自上而下）：§39→§40 斜 / §39→§41 竖 / §41→§42 短竖 / §40→§42 长斜
% 尺寸/字号/线宽/箭头头的推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.7043}{%
		\node[anchor=base west] (dg39) at (243.7,    0.0) {§39 Local finiteness};
		\node[anchor=base west] (dg40) at (  0.0,  -87.8) {§40};
		\node[anchor=base west] (dgng) at (311.8,  -85.4) {The Nagata-Smirnov metrization theorem};
		\node[anchor=base west] (dg41) at (241.3, -175.0) {§41 Paracompactness};
		\node[anchor=base west] (dg42) at (242.5, -264.6) {§42 The Smirnov metrization theorem};
