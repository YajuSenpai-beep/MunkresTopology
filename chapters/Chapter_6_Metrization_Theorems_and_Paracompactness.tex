...
where each collection \({\mathcal{B}}_{n}\) is locally finite. We say that such a collection \(\mathcal{B}\) is \idx[countably locally finite]{Countably locally finite}. Surprisingly enough, this condition, along with regularity, is both necessary and sufficient for metrizability of \(X\). This we shall prove.
...
\munkresfig[0.8]{38.1}[Figure 38.1]
...
\begin{lemma}
\label{lem:\thetheorem}
 Let \(\mathcal{A}\) be a \idx[locally finite collection]{Locally finite collection} of subsets of \(X\). Then:
...
There is an analogous concept of local finiteness for an indexed family of subsets of \(X\). The indexed family \({\{ {A}_{\alpha }\} }_{\alpha \in J}\) is said to be a locally finite indexed family in \(X\) if every \(x \in X\) has a neighborhood that intersects \({A}_{\alpha }\) for only finitely many values of \(\alpha\). What is the relation between the two formulations of local finiteness? It is easy to see that \({\{ {A}_{\alpha }\} }_{\alpha \in J}\) is a locally finite indexed family if and only if it is locally finite as a collection of sets and each nonempty subset \(A\) of \(X\) equals \({A}_{\alpha }\) for at most finitely many values of \(\alpha\).
...
\begin{definition}
 Let \(\mathcal{A}\) be a collection of subsets of the space \(X\). A collection \(\mathcal{B}\) of subsets of \(X\) is said to be a \idx[refinement]{Refinement} of \(\mathcal{A}\) (or is said to refine \(\mathcal{A}\) ) if for each element \(B\) of \(\mathcal{B}\), there is an element \(A\) of \(\mathcal{A}\) containing \(B\). If the elements of \(\mathcal{B}\) are open sets, we call \(\mathcal{B}\) an \idx[open refinement]{Open refinement} of \(\mathcal{A}\) ; if they are closed sets, we call \(\mathcal{B}\) a \idx[closed refinement]{Closed refinement}.
\end{definition}
...
Choose a metric for \(X\). Let \(n\) be a positive integer, fixed for the moment. Given an element \(U\) of \(\mathcal{A}\), let us define \({S}_{n}(U)\) to be the subset of \(U\) obtained by ``shrinking'' \(U\) a distance of \(1/n\). More precisely, let

\[
{S}_{n}(U) = \{ x \mid B({x,1/n}) \subset U\}.
\]

(It happens that \({S}_{n}(U)\) is a closed set, but that is not important for our purposes.) Now we use the well-ordering \(<\) of \(\mathcal{A}\) to pass to a still smaller set. For each \(U\) in \(\mathcal{A}\), define

\[
{T}_{n}(U) = {S}_{n}(U) - \mathop{\bigcup }\limits_{{V < U}}V
\]
...
To prove this fact, assume the notation has been so chosen that \(V < W\). Since \(x\) is in \({T}_{n}(V)\), then \(x\) is in \({S}_{n}(V)\), so the \(1/n\) -neighborhood of \(x\) lies in \(V\). On the other hand, since \(V < W\) and \(y\) is in \({T}_{n}(W)\), the definition of the latter set tells us that \(y\) is not in \(V\). It follows that \(y\) is not in the \(1/n\) -neighborhood of \(x\).

The sets \({T}_{n}(U)\) are not yet the ones we want, for we do not know that they are open sets. (In fact, they are closed.) So let us expand each of them slightly to obtain an open set \({E}_{n}(U)\). Specifically, let \({E}_{n}(U)\) be the \(1/{3n}\) -neighborhood of \({T}_{n}(U)\) ; that is, let \({E}_{n}(U)\) be the union of the open balls \(B({x,1/{3n}})\), for \(x \in {T}_{n}(U)\).
...
We claim that \({\mathcal{E}}_{n}\) is a locally finite collection of open sets that refines \(\mathcal{A}\). The fact that \({\mathcal{E}}_{n}\) refines \(\mathcal{A}\) comes from the fact that \({E}_{n}(V) \subset V\) for each \(V \in \mathcal{A}\). The fact that \({\mathcal{E}}_{n}\) is locally finite comes from the fact that for any \(x\) in \(X\), the \(1/{6n}\) -neighborhood of \(x\) can intersect at most one element of \({\mathcal{E}}_{n}\).
...
Let \(x\) be a point of \(X\). The collection \(\mathcal{A}\) with which we began covers \(X\) ; let us choose \(U\) to be the first element of \(\mathcal{A}\) (in the well-ordering \(<\) ) that contains \(x\). Since \(U\) is open, we can choose \(n\) so that \(B({x,1/n}) \subset U\). Then, by definition, \(x \in {S}_{n}(U)\). Now because \(U\) is the first element of \(\mathcal{A}\) that contains \(x\), the point \(x\) belongs to \({T}_{n}(U)\). Then \(x\) also belongs to the element \({E}_{n}(U)\) of \({\mathcal{E}}_{n}\), as desired.
\end{proof}
\section*{Exercises}
...
\begin{example}
\label{ex:\thesection.\theexample}
 Each open subset of \(X\) is a \({G}_{\delta }\) set, trivially. In a first-countable Hausdorff space, each one-point set is a \({G}_{\delta }\) set. The one-point subset \(\{ \Omega \}\) of \({\bar{S}}_{\Omega }\) is not a \({G}_{\delta }\) set, as you can check.
\end{example}
\end{centeredblock}
...
\begin{example}
\label{ex:\thesection.\theexample}
 In a metric space \(X\), each closed set is a \({G}_{\delta }\) set. Given \(A \subset X\), let \(U({A,\epsilon })\) denote the \(\epsilon\) -neighborhood of \(A\). If \(A\) is closed, you can check that
...
\begin{proof}
 Step 1. Let \(W\) be open in \(X\). We show there is a countable collection \(\{ {U}_{n}\}\) of open sets of \(X\) such that
\[
...
Then \({U}_{n}\) is an open set, and by \hyperref[lem:39.1]{Lemma 39.1},
...
\begin{lemma}
\label{lem:\thetheorem}
 Let \(X\) be normal; let \(A\) be a closed \({G}_{\delta }\) set in \(X\). Then there is a continuous function \(f : X \rightarrow [{0,1}]\) such that \(f(x) = 0\) for \(x \in A\) and \(f(x) > 0\) for \(x \notin A\).
\end{lemma}

\begin{proof}
 We gave this as an exercise in \S\ 33; we provide a proof here. Write \(A\) as the intersection of the open sets \({U}_{n}\), for \(n \in {\mathbb{Z}}_{ + }\). For each \(n\), choose a continuous function \({f}_{n} : X \rightarrow [{0,1}]\) such that \(f(x) = 0\) for \(x \in A\) and \(f(x) = 1\) for \(x \in X - {U}_{n}\). Define \(f(x) = \sum {f}_{n}(x) /{2}^{n}\). The series converges uniformly, by comparison with \(\sum 1/{2}^{n}\), so that \(f\) is continuous. Also, \(f\) vanishes on \(A\) and is positive on \(X - A\).
\end{proof}
\begin{theorem}
...
\begin{proof}
 Step 1. Assume \(X\) is regular with a countably locally finite basis \(\mathcal{B}\). Then \(X\) is normal, and every closed set in \(X\) is a \({G}_{\delta }\) set in \(X\). We shall show that \(X\) is metrizable by imbedding \(X\) in the metric space \(({{\mathbb{R}}^{J},\bar{\rho }})\) for some \(J\).
...
\[
{f}_{n,B} : X \rightarrow [{0,1/n}]
\]

such that \({f}_{n,B}(x) > 0\) for \(x \in B\) and \({f}_{n,B}(x) = 0\) for \(x \notin B\). The collection \(\{ {f}_{n,B}\}\) separates points from closed sets in \(X\). Given a point \({x}_{0}\) and a neighborhood \(U\) of \({x}_{0}\), there is a basis element \(B\) such that \({x}_{0} \in B \subset U\). Then \(B \in {\mathcal{B}}_{n}\) for some \(n\), so that \({f}_{n,B}({x}_{0}) > 0\) and \({f}_{n,B}\) vanishes outside \(U\).
...
\item A collection \(\mathcal{A}\) of subsets of \(X\) is said to be \idx[locally discrete]{Locally discrete} if each point of \(X\) has a neighborhood that intersects at most one element of \(\mathcal{A}\). A collection \(\mathcal{B}\) is \idx[countably locally discrete]{Countably locally discrete} (or `` \(\sigma\) -locally discrete'') if it equals a countable union of locally discrete collections. Prove the following: \textsl{Theorem. (Bing metrization theorem.)} A space \(X\) is metrizable if and only if it is regular and has a basis that is countably locally discrete. 
\end{enumerate}
...
\begin{example}
\label{ex:\thesection.\theexample}
 A subspace of a paracompact space need not be paracompact. The space \({\bar{S}}_{\Omega } \times {\bar{S}}_{\Omega }\) is compact and, therefore, paracompact. But the subspace \({S}_{\Omega } \times {\bar{S}}_{\Omega }\) is not paracompact, for it is Hausdorff but not normal.
\end{example}
\end{centeredblock}
...
\[
{S}_{n}(U) = U - \mathop{\bigcup }\limits_{{i < n}}{V}_{i}
\]

[Note that \({S}_{n}(U)\) is not necessarily open, nor closed.] Let
...
We have finally to prove that \(\mathcal{D}\) is locally finite. Given a point \(x\) of \(X\), choose a neighborhood \(W\) of \(x\) that intersects only finitely many elements of \(\mathcal{C}\), say \({C}_{1},\ldots ,{C}_{k}\). We show that \(W\) intersects only finitely many elements of \(\mathcal{D}\). Because \(\mathcal{C}\) covers \(X\), the set \(W\) is covered by \({C}_{1},\ldots ,{C}_{k}\). Thus, it suffices to show that each element \(C\) of \(\mathcal{C}\) intersects only finitely many elements of \(\mathcal{D}\). Now if \(C\) intersects the set \(E(B) \cap F(B)\), then it intersects \(E(B)\), so by definition of \(E(B)\) it is not contained in \(X - B\), hence \(C\) must intersect \(B\). Since \(C\) intersects only finitely many elements of \(\mathcal{B}\), it can intersect at most the same number of elements of the collection \(\mathcal{D}\).
\end{proof}
\begin{theorem}
\label{thm:\thetheorem}
...
\begin{definition}
 Let \({\{ {U}_{\alpha }\} }_{\alpha \in J}\) be an indexed open covering of \(X\). An indexed family of continuous functions
...
We now construct a partition of unity on an arbitrary paracompact Hausdorff space. We begin by proving a ``\idx[shrinking lemma]{Shrinking lemma},'' just as we did for the finite case in \S\ 36.

\begin{lemma}
\label{lem:\thetheorem}
Let \(X\) be a paracompact Hausdorff space; let \({\{ {U}_{\alpha }\} }_{\alpha \in J}\) be an indexed family of open sets covering \(X\). Then there exists a locally finite indexed family \({\{ {V}_{\alpha }\} }_{\alpha \in J}\) of open sets covering \(X\) such that \({\bar{V}}_{\alpha } \subset {U}_{\alpha }\) for each \(\alpha\).
\end{lemma}

The condition that \({\bar{V}}_{\alpha } \subset {U}_{\alpha }\) for each \(\alpha\) is sometimes expressed by saying that the family \(\{ {\bar{V}}_{\alpha }\}\) is a \idx[precise refinement]{Precise refinement} of the family \(\{ {U}_{\alpha }\}\).

\begin{proof}
Let \(\mathcal{A}\) be the collection of all open sets \(A\) such that \(\bar{A}\) is contained in some element of the collection \(\{ {U}_{\alpha }\}\). Regularity of \(X\) implies that \(\mathcal{A}\) covers \(X\). Since \(X\) is paracompact, we can find a locally finite collection \(\mathcal{B}\) of open sets covering \(X\) that refines \(\mathcal{A}\). Let us index \(\mathcal{B}\) bijectively with some index set \(K\); then the general element of \(\mathcal{B}\) can be denoted \({B}_{\beta }\), for \(\beta \in K\), and \({\{ {B}_{\beta }\} }_{\beta \in K}\) is a locally finite indexed family. Since \(\mathcal{B}\) refines \(\mathcal{A}\), we can define a function \(f : K \rightarrow J\) by choosing, for each \(\beta\) in \(K\), an element \(f(\beta) \in J\) such that
\[
...
(Note that \({V}_{\alpha }\) is empty if there exists no index \(\beta\) such that \(f(\beta) = \alpha\).) For each element \({B}_{\beta }\) of the collection \({\mathcal{B}}_{\alpha }\) we have \({\bar{B}}_{\beta } \subset {U}_{\alpha }\) by definition. Because the collection \({\mathcal{B}}_{\alpha }\) is locally finite, \({\bar{V}}_{\alpha }\) equals the union of the closures of the elements of \({\mathcal{B}}_{\alpha }\), so that \({\bar{V}}_{\alpha } \subset {U}_{\alpha }\).

Finally, we check local finiteness. Given \(x \in X\), choose a neighborhood \(W\) of \(x\) such that \(W\) intersects \({B}_{\beta }\) for only finitely many values of \(\beta\), say \(\beta = {\beta }_{1},\ldots ,{\beta }_{k}\). Then \(W\) can intersect \({V}_{\alpha }\) only if \(\alpha\) is one of the indices \(f({\beta }_{1}),\ldots, f({\beta }_{K})\).
\end{proof}

\begin{theorem}
\label{thm:\thetheorem}
Let \(X\) be a paracompact Hausdorff space; let \({\{ {U}_{\alpha }\} }_{\alpha \in J}\) be an indexed open covering of \(X\). Then there exists a partition of unity on \(X\) dominated by \(\{ {U}_{\alpha }\}\).
\end{theorem}

\begin{proof}
We begin by applying the shrinking lemma twice, to find locally finite indexed families of open sets \(\{ {W}_{\alpha }\}\) and \(\{ {V}_{\alpha }\}\) covering \(X\), such that \({\bar{W}}_{\alpha } \subset {V}_{\alpha }\) and \({\bar{V}}_{\alpha } \subset {U}_{\alpha }\) for each \(\alpha\). Since \(X\) is normal, we may choose, for each \(\alpha\), a continuous function \({\psi }_{\alpha } : X \rightarrow [{0,1}]\) such that \({\psi }_{\alpha }({\bar{W}}_{\alpha }) = \{ 1\}\) and \({\psi }_{\alpha }({X - {V}_{\alpha }}) = \{ 0\}\). Since \({\psi }_{\alpha }\) is nonzero only at points of \({V}_{\alpha }\), we have

\[
({\text{ Support }{\psi }_{\alpha }}) \subset {\bar{V}}_{\alpha } \subset {U}_{\alpha }\text{.}
\]

Furthermore, the indexed family \(\{ {\bar{V}}_{\alpha }\}\) is locally finite (since an open set intersects \({\bar{V}}_{\alpha }\) only if it intersects \({V}_{\alpha }\) ); hence the indexed family \(\{ {\text{ Support }{\psi }_{\alpha }}\}\) is also locally finite. Note that because \(\{ {W}_{\alpha }\}\) covers \(X\), for any given \(x\) at least one of the functions \({\psi }_{\alpha }\) is positive at \(x\).
...
\item
 ...
 \end{enumerate}
\item Let \(X\) be paracompact. We proved a ``shrinking lemma'' for arbitrary indexed open coverings of \(X\). Here is an ``\idx[expansion lemma]{Expansion lemma}'' for arbitrary locally finite indexed families in \(X\). \textsl{Lemma} Let \({\{ {B}_{\alpha }\} }_{\alpha \in J}\) be a locally finite indexed family of subsets of the paracompact Hausdorff space \(X\). Then there is a locally finite indexed family \({\{ {U}_{\alpha }\} }_{\alpha \in J}\) of open sets in \(X\) such that \({B}_{\alpha } \subset {U}_{\alpha }\) for each \(\alpha\). 
\item
 \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
...
\item Let \(p : X \rightarrow Y\) be a perfect map. (See Exercise 7 of \S\ 31.)
 ...
 \end{enumerate}
\item Let \(G\) be a locally compact, connected topological group. Show that \(G\) is paracompact. [Hint. Let \({U}_{1}\) be a neighborhood of \(e\) having compact closure. In general, define \({U}_{n + 1} = {\bar{U}}_{n} \cdot {U}_{1}\). Show the union of the sets \({\bar{U}}_{n}\) is both open and closed in \(G\).] This result holds without assuming \(G\) is connected, but the proof requires more effort.
...
\[
{B}_{{C}_{i}}({x,{\epsilon }_{i}}) \subset ({U \cap {C}_{i}}).
\]

Choose \(m\) so that \(2/m < \min \{ {{\epsilon }_{1},\ldots ,{\epsilon }_{k}}\}\). Because the collection \({\mathcal{D}}_{m}\) covers \(X\), there must be an element \(D\) of \({\mathcal{D}}_{m}\) containing \(x\). Because \({\mathcal{D}}_{m}\) refines \({\mathcal{A}}_{m}\), there must be an element \({B}_{C}({y,1/m})\) of \({\mathcal{A}}_{m}\), for some \(C \in \mathcal{C}\) and some \(y \in C\), that contains \(D\). Because

\[
x \in D \subset {B}_{C}({y,1/m}) \text{,}
\]

the point \(x\) belongs to \(C\), so that \(C\) must be one of the sets \({C}_{1},\ldots ,{C}_{k}\). Say \(C = {C}_{i}\). Since \({B}_{C}({y,1/m})\) has diameter at most \(2/m < {\epsilon }_{i}\), it follows that
