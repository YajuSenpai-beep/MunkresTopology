...
\begin{lemma}
	Let \(\mathcal{A}\) be a \index{locally finite collection@Locally finite collection}\index{locally finite collection@Locally finite collection}\index{locally finite collection@Locally finite collection}locally finite collection of subsets of \(X\) . Then:
...
\end{proof}
\begin{definition}
	A collection \(\mathcal{B}\) of subsets of \(X\) is said to be \index{countably locally finite@Countably locally finite}\index{countably locally finite@Countably locally finite}\index{countably locally finite@Countably locally finite}countably locally finite if \(\mathcal{B}\) can be written as the countable union of collections \({\mathcal{B}}_{n}\) , each of which is locally finite.
\end{definition}
...
\item Find a \index{point-finite collection@Point-finite collection} point-finite open covering \(\mathcal{A}\) of \(\mathbb{R}\) that is not locally finite. (The collection \(\mathcal{A}\) is point-finite if each point of \(\mathbb{R}\) lies in only finitely many elements of \(\mathcal{A}\) .)
...
\item Consider \({\mathbb{R}}^{\omega }\) in the uniform topology. Given \(n\) , let \({\mathcal{B}}_{n}\) be the collection \index{Paracompactness!of \({\mathbb{R}}^{\omega }\)}of all subsets of \({\mathbb{R}}^{\omega }\) of the form \(\prod {A}_{i}\) , where \({A}_{i} = \mathbb{R}\) for \(i \leq  n\) and \({A}_{i}\) equals either \(\{ 0\}\) or \(\{ 1\}\) otherwise. Show that the collection \(\mathcal{B} = \bigcup {\mathcal{B}}_{n}\) is countably locally finite, but neither countable nor locally finite.
\end{enumerate}
...
\begin{proof}
	We gave this as an exercise in \(\$ {33}\) , we provide a proof here. Write \(A\) as the intersection \index{Stone-Čech compactification!of \({\mathbb{Z}}_{ + }\)}of the open sets \({U}_{n}\) , for \(n \in  {\mathbb{Z}}_{ + }\) For each \(n\) , choose a continuous function \({f}_{n} : X \rightarrow  [  {0,1}]\) such that \(f( x)  = 0\) for \(x \in  A\) and \(f( x)  = 1\) for \(x \in  X - {U}_{n}\) Define \(f( x)  = \sum {f}_{n}( x) /{2}^{n}\) The series converges uniformly, by comparison with \(\sum 1/{2}^{n}\) , so that \(f\) is continuous. Also, \(f\) vanishes on \(A\) and is positive on \(X - A\)
\end{proof}
\begin{theorem}[\index{nagata-smirnov metrization theorem@Nagata-Smirnov metrization theorem}]
...
Note that on the subspace \({[  0,1]  }^{J}\) \index{Paracompactness!of \({\mathbb{R}}^{J}\)}of \({\mathbb{R}}^{J}\) , the uniform metric equals the metric
...
\item A collection \(\mathcal{A}\) of subsets of \(X\) is said to be \index{Locally discrete} if each point of \(X\) has a neighborhood that intersects at most one element of \(\mathcal{A}\) . A collection \(\mathcal{B}\) is \index{Countably locally discrete} (or `` \(\sigma\) -\index{locally discrete@Locally discrete}\index{locally discrete@Locally discrete}\index{locally discrete@Locally discrete}locally discrete'') if it equals a countable union of locally discrete collections. Prove the following: \textsl{Theorem. (\index{bing metrization theorem@Bing metrization theorem}).} A space \(X\) is metrizable if and only if it is regular and has a basis that is \index{countably locally discrete@Countably locally discrete}\index{countably locally discrete@Countably locally discrete}\index{countably locally discrete@Countably locally discrete}countably locally discrete. 
\end{enumerate}
\section*{§ 41 Paracompactness}

The concept \index{Normality!of paracompact Hausdorff space}of \index{Compact Hausdorff space: is Baire space!paracompactness}\index{Compact Hausdorff space: is Baire space!paracompactness}paracompactness is one of the most useful generalizations of compactness that has been discovered in recent years. It is particularly useful for applications in topology and differential geometry We shall give just one application, a metrization theorem that we prove in the next section.
...
A space \(X\) is compact if every open covering \(\mathcal{A}\) of \(X\) has a finite \index{open refinement@Open refinement}\index{open refinement@Open refinement}\index{open refinement@Open refinement}open refinement \(\mathcal{B}\) that covers \(X\)
...
Some of the properties of a \index{paracompact space@Paracompact space}\index{paracompact space@Paracompact space}\index{paracompact space@Paracompact space}paracompact space are similar to those of a compact space. For instance, a subspace \index{Paracompactness!of closed subspace}of a paracompact space is not necessarily paracompact; but a closed subspace is paracompact. Also, a \index{Normality!of paracompact Hausdorff space}paracompact Hausdorff space is normal. In other ways, a paracompact space is not similar to a compact space; in particular, the product of two paracompact spaces need not be paracompact. We shall verify these facts shortly.
...
\(( 3)  \Rightarrow  ( 4)\) . Let \(\mathcal{A}\) be an open covering of \(X\) . Using(3), choose \(\mathcal{B}\) to be a refinement of \(\mathcal{A}\) that covers \(X\) and is locally finite (We can take \(\mathcal{B}\) to be a \index{closed refinement@Closed refinement}\index{closed refinement@Closed refinement}\index{closed refinement@Closed refinement}closed refinement if we like, but that is irrelevant.) We seek to expand each element \(B\) of \(\mathcal{B}\) slightly to an open set, making the expansion slight enough that the resulting collection of open sets will still be locally finite and will still refine \(\mathcal{A}\) .
...
\item Let \(p : X \rightarrow  Y\) be a perfect map. (See Exercise 7 of §31.)
  ...
  \end{enumerate}
\item Let \(G\) be a locally compact, connected topological group \index{Topological group!paracompactness}Show that \(G\) is para-compact [Hint. Let \({U}_{1}\) be a neighborhood of \(e\) having compact closure. In general, define \({U}_{n + 1} = {\bar{U}}_{n} \cdot  {U}_{1}\) . Show the union of the sets \({\bar{U}}_{n}\) is both open and closed in \(G\) .] This result holds without assuming \(G\) is connected, but the proof requires more effort.
...
\item
  \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
  \item Show that for each \(x \in  {S}_{\Omega }\) , the section \index{Paracompactness!of \({S}_{\Omega }\)}of \({S}_{\Omega }\) by \(x\) has a countable basis and hence is metrizable.
