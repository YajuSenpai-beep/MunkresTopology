...
where each collection \({\mathcal{B}}_{n}\) is locally finite. We say that such a collection \(\mathcal{B}\) is \index{Countably locally finite}countably locally finite. Surprisingly enough, this condition, along with regularity, is both necessary and sufficient for metrizability of \(X\) . This we shall we prove.

There is another concept in topology that involves the notion of local finiteness. It is a generalization of the concept of compactness called ``\index{Paracompactness}paracompactness.'' Although of fairly recent origin, it has proved useful in many parts of mathematics. We introduce it here so that we can give another set of necessary and sufficient conditions for a space \(X\) to be metrizable. It turns out that \(X\) is metrizable if and only if it is both paracompact and locally metrizable. This we prove in §42.
...
In this sections we prove some elementary properties of \index{Locally finite collection}locally finite collections and a crucial lemma about metrizable spaces.
...
\begin{definition}
	Let \(\mathcal{A}\) be a collection of subsets of the space \(X\) . A collection \(\mathcal{B}\) of subsets of \(X\) is said to be a \index{Refinement}refinement of \(\mathcal{A}\) (or is said to refine \(\mathcal{A}\) ) if for each element \(B\) of \(\mathcal{B}\) , there is an element \(A\) of \(\mathcal{A}\) containing \(B\) . If the elements of \(\mathcal{B}\) are open sets, we call \(\mathcal{B}\) an \index{Open refinement}open refinement of \(\mathcal{A}\) ; if they are closed sets, we call \(\mathcal{B}\) a \index{Closed refinement}closed refinement
\end{definition}
...
\item Find a \index{point-finite collection@Point-finite collection}Point-finite collection point-finite open covering \(\mathcal{A}\) of \(\mathbb{R}\) that is not locally finite. (The collection \(\mathcal{A}\) is point-finite if each point of \(\mathbb{R}\) lies in only finitely many elements of \(\mathcal{A}\) .)
...
\end{proof}
\begin{theorem}[\index{nagata-smirnov metrization theorem@Nagata-Smirnov metrization theorem}Nagata-Smirnov metrization theorem]
...
\item A collection \(\mathcal{A}\) of subsets of \(X\) is said to be \index{Locally discrete}locally discrete if each point of \(X\) has a neighborhood that intersects at most one element of \(\mathcal{A}\) . A collection \(\mathcal{B}\) is \index{Countably locally discrete}countably locally discrete (or `` \(\sigma\) -locally discrete'') if it equals a countable union of locally discrete collections. Prove the following: \textsl{Theorem. (\index{bing metrization theorem@Bing metrization theorem}Bing metrization theorem).} A space \(X\) is metrizable if and only if it is regular and has a basis that is countably locally discrete. 
\end{enumerate}
...
The condition that \({\widetilde{V}}_{\alpha } \subset  {U}_{\alpha }\) for each \(\alpha\) is sometimes expressed by saying that the family \(\left\{  {\widetilde{V}}_{\alpha }\right\}\) is a \index{Precise refinement}precise refinement of the family \(\left\{  {U}_{\alpha }\right\}\) .
