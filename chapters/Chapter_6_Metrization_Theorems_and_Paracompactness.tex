...
\begin{example}
\label{ex:\thesection.\theexample}
 The space \({\mathbb{R}}^{\omega }\) is paracompact in both the product and uniform topologies. This result follows from the fact that \({\mathbb{R}}^{\omega }\) is metrizable in these topologies. It is not known whether \({\mathbb{R}}^{\omega }\) is paracompact in the box topology. (See the comment in Exercise 5 of §32)
\end{example}
\end{centeredblock}
...
One of the most useful properties that a paracompact space \(X\) possesses has to do with the existence of partitions of unity on \(X\). We have already seen the finite version of this notion in §36; we discuss the general case now. Recall that if \(\phi : X \rightarrow \mathbb{R}\), the support of \(\phi\) is the closure of the set of those \(x\) for which \(\phi (x) \neq 0\).
...
\section*{Exercises}
\begin{enumerate}[itemsep=0.4em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=\arabic*., ref=\arabic*]
\item Compare \hyperref[thm:42.1]{Theorem 42.1} with Exercises 7 and 8 of §34.
\item
 \begin{enumerate}[itemsep=0.2em, parsep=0pt, topsep=0pt, partopsep=0pt, leftmargin=*, label=(\alph*), align=left]
