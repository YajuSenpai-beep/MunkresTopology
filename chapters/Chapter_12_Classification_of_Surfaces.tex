...
\begin{center}
	\munkrescomm{0.5603}{%
		\node (F)  at ( 20, -155) {$F$};
		\node (qF) at (245,  -45) {$q(F)$};
		\node (qQ) at (732.5,  -45.0) {$q(F)/[q(F),q(F)]$};
		\node (pF) at (398, -207) {$p(F)$};
		\node (pP) at (732.5, -262.0) {$p(F)/p(N)$};
		\draw[->] (qF) -- node[above,pos=.5] {$s$} (qQ);
		\draw[->] (F)  -- node[above,pos=.5] {$q$} (qF);
		\draw[->] (F)  -- node[below,pos=.5] {$p$} (pF);
		\draw[->] (qF) -- node[below,pos=.5] {$u$} (pP);
		\draw[->] (pF) -- node[above,pos=.5] {$v$} (qQ);
		\draw[->] ([xshift=-9pt]pP.east) -- node[right] {$\phi$} ([xshift=-9pt]qQ.east);
		\draw[->] ([xshift=-18pt]qQ.east) -- node[left] {$\psi$} ([xshift=-18pt]pP.east);
		\draw[->] (pF) -- node[below,pos=.5] {$r$} (pP);
	}
	\label{fig:com_75_1}
\end{center}
\par\nopagebreak 
...
\begin{center}
	\munkrescomm{0.3895}{%
		\node (Be) at ( 60,  -70) {$\mathbf{B}_{\epsilon}-\mathbf{0}$};
		\node (B)  at (880,  -70) {$\mathbf{B}-\mathbf{0}$};
		\node (V)  at (530, -320) {$V-\mathbf{0}$};
		\draw[->] (Be) -- node[above,pos=.5] {$i$} (B);
		\draw[->] (Be) -- node[below left=1pt,pos=.5] {$j$} (V);
		\draw[->] (V)  -- node[above right=1pt,pos=.5] {$k$} (B);
	}
	\label{fig:com_78_35}
\end{center}
\par\nopagebreak 
...
\begin{center}
	\munkrescomm{0.4008}{%
		\node (Xe) at ( 60,  -64) {$\mathbf{X}_{\epsilon}-\mathbf{0}$};
		\node (X)  at (880,  -64) {$\mathbf{X}-\mathbf{0}$};
		\node (W)  at (530, -320) {$W-\mathbf{0}$};
		\draw[->] (Xe) -- node[above,pos=.5] {$i$} (X);
		\draw[->] (Xe) -- node[below left=1pt,pos=.5] {$j$} (W);
		\draw[->] (W)  -- node[above right=1pt,pos=.5] {$k$} (X);
	}
	\label{fig:com_78_55}
\end{center}
\par\nopagebreak 
