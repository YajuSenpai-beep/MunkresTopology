...
\begin{center}
	\munkrescomm{0.5603}{%
		...
		\node (qQ) at (720,  -45) {$q(F)/[q(F),q(F)]$};
		\node (pF) at (398, -207) {$p(F)$};
		\node (pP) at (745, -262) {$p(F)/p(N)$};
...
\begin{center}
	\munkrescomm{0.3895}{%
		...
		\draw[->] (Be) -- node[below left=-2pt,pos=.5] {$j$} (V);
		\draw[->] (V)  -- node[above right=-3pt,pos=.5] {$k$} (B);
	}
	\label{fig:com_78_35}
\end{center}
\par\nopagebreak 
...
\begin{center}
	\munkrescomm{0.4008}{%
		...
		\draw[->] (Xe) -- node[below left=-2pt,pos=.5] {$j$} (W);
		\draw[->] (W)  -- node[above right=-3pt,pos=.5] {$k$} (X);
	}
	\label{fig:com_78_55}
\end{center}
\par\nopagebreak 
