...
When we pass to the cosets in the abelian group \({\pi }_{1}/[ {{\pi }_{1},{\pi }_{1}}]\), we see that \(\hat{g}\) induces the identity map.
...
% 交换图 com_75_1：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.5603}{%
		\node (F)  at ( 21.4, -152.5) {$F$};
		\node (qF) at (248.9,  -42.8) {$q(F)$};
		\node (qQ) at (674.1,  -42.8) {$q(F)/[q(F),q(F)]$};
		\node (pF) at (248.9, -259.8) {$p(F)$};   % 原书：p(F) 与 q(F) 严格同列（x 已修）；且 p(F) 与 p(F)/p(N) **同一行**，故 y 取与 pP 相同（-259.8）。原先 y=-204.8 偏高约 55 单位。
		\node (pP) at (735.4, -259.8) {$p(F)/p(N)$};
		\draw[->] (qF) -- node[inner sep=0pt,above=3.5pt,pos=.5] {$s$} (qQ);
		\draw[->] (F)  -- node[inner sep=0pt,above=3.5pt,pos=.5] {$q$} (qF);
		\draw[->] (F)  -- node[inner sep=0pt,below=3.5pt,pos=.5] {$p$} (pF);
		% u/v 都在各自线的**下方**（原书如此）；pos 取 .3 让两标签分开
		% （都取 .5 时两标签在中点附近几乎重合）
		\draw[->] (qF) -- node[inner sep=0pt,below=3.5pt,pos=.22] {$u$} (pP);
		\draw[->] (pF) -- node[inner sep=0pt,below=3.5pt,pos=.22] {$v$} (qQ);
		% ⚠ 这两支竖箭头原写作 `[xshift=-9pt]pP.east` —— `east` 已是节点右缘，
		%   再往左偏移等于**把箭头画进节点文字里**（实测箭头穿过 ")]" 和 "N"），
		%   而且两支只差 9pt、箭头互相重叠。原书里它们在两个右侧节点**文字之右**的
		%   空白处左右并列。故改用显式坐标：φ 在左、ψ 在右（原书如此；原先左右相反），
		%   纵向跨度取两节点之间的空隙。
		\draw[->] (737, -68) -- node[inner sep=0pt,left=3pt] {$\phi$} (737, -232);   % 原书 φ 朝下；x=737 实测（原写 880 偏右 143）
		\draw[->] (767, -232) -- node[inner sep=0pt,right=3pt] {$\psi$} (767, -68);   % 原书 ψ 朝上；x=767 实测（原写 908 偏右）
		\draw[->] (pF) -- node[inner sep=0pt,below=3.5pt,pos=.5] {$r$} (pP);
	}
	\label{fig:com_75_1}
\end{center}
\par\nopagebreak 
...
% 交换图 com_78_35：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.3895}{%
		% 原书这三个串是**粗斜体**（回原书 40 倍放大核对：B、V 都带倾斜且笔画明显加粗，
		% 只有 0 是粗正体）；\mathbf 在本项目的字体下给的是**正体且不加粗**，
		% 故改用 \boldsymbol（\boldsymbol{0} 仍是粗正体，与原书一致）。
		\node (Be) at ( 62.3,  -64.6) {$\boldsymbol{B_{\epsilon}-0}$};
		\node (B)  at (880.9,  -64.6) {$\boldsymbol{B-0}$};
		\node (V)  at (479.6, -313.0) {$\boldsymbol{V-0}$};   % x 由 533.1 左移：原书 V−0 墨迹中心在两上节点中心的中点（偏离 +0.7% 顶边跨度），533.1 时偏离 +5.8%；字形同 B：原书为粗斜体
		\draw[->] (Be) -- node[inner sep=0pt,above=3.5pt,pos=.5] {$i$} (B);
		\draw[->] (Be) -- node[inner sep=0pt,below left=3pt,pos=.5] {$j$} (V);
		\draw[->] (V)  -- node[inner sep=0pt,below right=3pt,pos=.5] {$k$} (B);   % k 放箭头下方（V->B 是上行右斜线，下方即右下侧）
	}
	\label{fig:com_78_35}
\end{center}
\par\nopagebreak 
...
% 交换图 com_78_55：几何取自原书实测（坐标 = 墨迹宽/1000，原书 y 向下故取负）
% 尺寸/字号推导见 TopologyBook.sty 的 \munkrescomm
\par\nopagebreak
\begin{center}
	\munkrescomm{0.4008}{%
		\node (Xe) at ( 61.4,  -59.5) {$\boldsymbol{X_{\epsilon}-0}$};
		\node (X)  at (881.1,  -59.5) {$\boldsymbol{X-0}$};
		\node (W)  at (480.6, -313.2) {$\boldsymbol{W-0}$};   % x 由 530.8 左移：同 com_78_35，原书 W−0 居中（偏离 +1.0% 顶边跨度），530.8 时偏离 +5.8%；字形同 X：原书为粗斜体
		\draw[->] (Xe) -- node[inner sep=0pt,above=3.5pt,pos=.5] {$i$} (X);
		\draw[->] (Xe) -- node[inner sep=0pt,below left=3pt,pos=.5] {$j$} (W);
		\draw[->] (W)  -- node[inner sep=0pt,below right=3pt,pos=.5] {$k$} (X);   % k 放箭头下方
	}
	\label{fig:com_78_55}
\end{center}
\par\nopagebreak 
