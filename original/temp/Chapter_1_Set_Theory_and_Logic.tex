...
The equality symbol = is used throughout this book to mean logical identity. Thus, when we write \(a = b\) , we mean that " \(a\) " and " \(b\) " are symbols for the same object. This is what one means in arithmetic, for example, when one writes \(\frac{2}{4} = \frac{1}{2}\) . Simularly, the equation \(A = B\) states that " \(A\) " and " \(B\) " are symbols for the same set; that is, \(A\) and \(B\) consist of precisely the same objects.
...
Now the only way in which the statement " \(P \Rightarrow  Q\) " can fail to be correct is if the hypothesis \(P\) is true and the conclusion \(Q\) is false. Otherwise it is correct. Similarly, the only way in which the statement (not \(Q\) ) \(\Rightarrow\) (not \(P\) ) can fail to be correct is if the hypothesis "not \(Q\) " is true and the conclusion "not \(P\) " is false. This is the same as saying that \(Q\) is false and \(P\) is true. And this, in turn, is precisely the situation in which \(P \Rightarrow  Q\) fails to be correct. Thus, we see that the two statements are either both correct or both incorrect; they are logically equivalent. Therefore, we shall accept a proof of the statement "not \(Q \Rightarrow\) not \(P\) " as a proof of the statement " \(P \Rightarrow  Q\) ."
...
(*)
\[
\text{ For every }x \in  A\text{ , statement }P\text{ holds. }
\]
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_7_189_1051_1140_250_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.7\textwidth]{images/bo_d7h36d491nqc73828lu0_7_335_1757_852_388_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7h36d491nqc73828lu0_8_563_629_392_354_0.jpg}
\end{center}
\hspace*{3em} 
...
We now have another way to form new sets from old ones. Given a set \(A\) , we can consider sets whose elements are subsets of \(A\) . In particular, we can consider the set of all subsets of \(A\) . This set is sometimes denoted by the symbol \(\mathcal{P}\left( A\right)\) and is called the power set of \(A\) (for reasons to be explained later).
...
There is yet another way of forming new sets from old ones; it involves the notion of an "ordered pair" of objects. When you studied analytic geometry, the first thing you did was to convince yourself that after one has chosen an \(x\) -axis and a \(y\) -axis in the plane, every point in the plane can be made to correspond to a unique ordered pair \(\left( {x,y}\right)\) of real numbers. (In a more sophisticated treatment of geometry, the plane is more likely to be defined as the set of all ordered pairs of real numbers!)

The notion of ordered pair carries over to general sets. Given sets \(A\) and \(B\) , we define their cartesian product \(A \times  B\) to be the set of all ordered pairs \(\left( {a,b}\right)\) for which \(a\) is an element of \(A\) and \(b\) is an element of \(B\) . Formally,
...
it defines the ordered pair \(\left( {a,b}\right)\) as a collection of sets. If \(a \neq  b\) , this definition says that \(\left( {a,b}\right)\) is a collection containing two sets, one of which is a one-element set and the other a two-element set. The first coordinate of the ordered pair is defined to be the element belonging to both sets, and the second coordinate is the element belonging to only one of the sets. If \(a = b\) , then \(\left( {a,b}\right)\) is a collection containing only one set \(\{ a\}\) , since \(\{ a,b\}  = \; \{ a,a\}  = \{ a\}\) in this case. Its first coordinate and second coordinate both equal the element in this single set.
...
Let us make a comment on notation. It is an unfortunate fact that the notation \(\left( {a,b}\right)\) is firmly established in mathematics with two entirely different meanings. One meaning, as an ordered pair of objects, we have just discussed. The other meaning is the one you are familiar with from analysis; if \(a\) and \(b\) are real numbers, the symbol \(\left( {a,b}\right)\) is used to denote the interval consisting of all numbers \(x\) such that \(a < x < b\) . Most of the time, this conflict in notation will cause no difficulty because the meaning will be clear from the context. Whenever a situation occurs where confusion is possible, we shall adopt a different notation for the ordered pair \(\left( {a,b}\right)\) , denoting it by the symbol
...
(b) For at least one \(a \in  A\) , it is true that \({a}^{2} \notin  B\) .
...
7. Given sets \(A,B\) , and \(C\) , express each of the following sets in terms of \(A,B\) , and \(C\) , using the symbols \(U, \cap\) , and - .
...
\[
F = \{ x \mid  x \in  A\text{ and }\left( {x \in  B \Rightarrow  x \in  C}\right) \} \text{ . }
\]
...
(d) \(\left\{  {\left( {x,y}\right)  \mid  x}\right.\) is not an integer and \(y\) is an integer \(\}\) .
...
\({}^{ \dagger  }\) Analysts are apt to use the word "range" to denote what we have called the "image set" of \(f\) They avoid giving the set \(B\) a name.

\HRule

Definition. If \(f : A \rightarrow  B\) and if \({A}_{0}\) is a subset of \(A\) , we define the restriction of \(f\) to \({A}_{0}\) to be the function mapping \({A}_{0}\) into \(B\) whose rule is
...
EXAMPLE I. Let \(\mathbb{R}\) denote the real numbers and let \({\widetilde{\mathbb{R}}}_{ + }\) denote the nonnegative reals. Consider the functions
...
\[
g \cdot  {\widetilde{\mathbb{R}}}_{ + } \rightarrow  \mathbb{R}\;\text{ defined by }\;g\left( x\right)  = {x}^{2},
\]

\(h.\mathbb{R} \rightarrow  {\widetilde{\mathbb{R}}}_{ + }\;\) defined by \(\;h\left( x\right)  = {x}^{2},\)

\(k \cdot  {\widetilde{\mathbb{R}}}_{ + } \rightarrow  {\widetilde{\mathbb{R}}}_{ + }\;\) defined by \(\;k\left( x\right)  = {x}^{2}.\)

The function \(g\) is different from the function \(f\) because their rules are different subsets of \(\mathbb{R} \times  \mathbb{R}\) ; it is the restriction of \(f\) to the set \({\mathbb{R}}_{ + }\) . The function \(h\) is also different from \(f\) , even though their rules are the same set, because the range specified for \(h\) is different from the range specified for \(f\) . The function \(k\) is different from all of these. These functions are pictured in Figure 2 I

\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_14_168_1076_1183_405_0.jpg}
\end{center}
\hspace*{3em} 
...
Definition. Given functions \(f : A \rightarrow  B\) and \(g : B \rightarrow  C\) , we define the composite \(g \circ  f\) of \(f\) and \(g\) as the function \(g \circ  f : A \rightarrow  C\) defined by the equation \(\left( {g \circ  f}\right) \left( a\right)  = \; g\left( {f\left( a\right) }\right)\) .
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_15_219_204_1081_319_0.jpg}
\end{center}
\hspace*{3em} 

Figure 2.2

EXAMPLE 2. The composite of the function \(f.\mathbb{R} \rightarrow  \mathbb{R}\) given by \(f\left( x\right)  = 3{x}^{2} + 2\) and the function \(g : \mathbb{R} \rightarrow  \mathbb{R}\) given by \(g\left( x\right)  = {5x}\) is the function \(g \circ  f : \mathbb{R} \rightarrow  \mathbb{R}\) given by
...
If \(f\) is bijective, there exists a function from \(B\) to \(A\) called the inverse of \(f\) . It is denoted by \({f}^{-1}\) and is defined by letting \({f}^{-1}\left( b\right)\) be that unique element \(a\) of \(A\) for which \(f\left( a\right)  = b\) . Given \(b \in  B\) , the fact that \(f\) is surjective implies that there exists such an element \(a \in  A\) ; the fact that \(f\) is injective implies that there is only one such element \(a\) . It is easy to see that if \(f\) is bijective, \({f}^{-1}\) is also bijective.

EXAMPLE 3. Consider again the functions \(f,g,h\) , and \(k\) of Figure 2.1. The function \(f : \mathbb{R} \rightarrow  \mathbb{R}\) given by \(f\left( x\right)  = {x}^{2}\) is neither injective nor surjective. Its restriction \(g\) to the nonnegative reals is injective but not surjective. The function \(h : \mathbb{R} \rightarrow  {\mathbb{R}}_{ + }\) obtained from \(f\) by changing the range is surjective but not injective. The function \(k{\bar{\mathbf{R}}}_{ + } \rightarrow  {\overline{\mathbf{R}}}_{ + }\) obtained from \(f\) by restricting the domain and changing the range is both injective and surjective, so it has an inverse. Its inverse is, of course, what we usually call the square-root function.
...
Lemma 2.1. Let \(f : A \rightarrow  B\) . If there are functions \(g : B \rightarrow  A\) and \(h : B \rightarrow  A\) such that \(g\left( {f\left( a\right) }\right)  = a\) for every \(a\) in \(A\) and \(f\left( {h\left( b\right) }\right)  = b\) for every \(b\) in \(B\) , then \(f\) is bijective and \(g = h = {f}^{-1}\) .

Definition. Let \(f : A \rightarrow  B\) . If \({A}_{0}\) is a subset of \(A\) , we denote by \(f\left( {A}_{0}\right)\) the set of all images of points of \({A}_{0}\) under the function \(f\) ; this set is called the image of \({A}_{0}\) under \(f\) . Formally,

\[
f\left( {A}_{0}\right)  = \left\{  {b \mid  b = f\left( a\right) \text{ for at least one }a \in  {A}_{0}}\right\}  .
\]

On the other hand, if \({B}_{0}\) is a subset of \(B\) , we denote by \({f}^{-1}\left( {B}_{0}\right)\) the set of all elements of \(A\) whose images under \(f\) lie in \({B}_{0}\) ; it is called the preimage of \({B}_{0}\) under \(f\) (or the "counterimage," or the "inverse image," of \({B}_{0}\) ). Formally,
...
EXAMPLE 4 Consider the function \(f : \mathbb{R} \rightarrow  \mathbb{R}\) given by \(f\left( x\right)  = 3{x}^{2} + 2\) (Figure 2.3) Let \(\left\lbrack  {a,b}\right\rbrack\) denote the closed interval \(a \leq  x \leq  b\) . Then
...
\begin{center}
\includegraphics[max width=0.6\textwidth]{images/bo_d7h36d491nqc73828lu0_17_399_498_714_898_0.jpg}
\end{center}
\hspace*{3em} 
...
2. Let \(f : A \rightarrow  B\) and let \({A}_{i} \subset  A\) and \({B}_{i} \subset  B\) for \(\mathrm{i} = 0\) and \(i = 1\) . Show that \({f}^{ - }\) preserves inclusions, unions, intersections, and differences of sets:
...
5. In general, let us denote the identity function for a set \(C\) by \({i}_{C}\) . That is, define \({i}_{C} : C \rightarrow  C\) to be the function given by the rule \({i}_{C}\left( x\right)  = x\) for all \(x \in  C\) . Given \(f : A \rightarrow  B\) , we say that a function \(g : B \rightarrow  A\) is a left inverse for \(f\) if \(g \circ  f = {i}_{A}\) ; and we say that \(h : B \rightarrow  A\) is a right inverse for \(f\) if \(f \circ  h = {i}_{B}\) .
...
EXAMPLE I. Let \(P\) denote the set of all people in the world, and define \(D \subset  P \times  P\) by the equation
...
\[
S = \{ \left( {x,y}\right)  \mid  \text{ the parents of }x\text{ are the parents of }y\} .
\]
...
Proof. Let \(E\) be the equivalence class determined by \(x\) , and let \({E}^{\prime }\) be the equivalence class determined by \({x}^{\prime }\) . Suppose that \(E \cap  {E}^{\prime }\) is not empty; let \(\mathrm{y}\) be a point of \(E \cap  {E}^{\prime }\) . See Figure 3.1. We show that \(E = {E}^{\prime }\) .

\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7h36d491nqc73828lu0_20_569_457_382_243_0.jpg}
\end{center}
\hspace*{3em} 
...
Given an equivalence relation on a set \(A\) , let us denote by \(\varepsilon\) the collection of all the equivalence classes determined by this relation. The preceding lemma shows that distinct elements of \(\varepsilon\) are disjoint. Furthermore, the union of the elements of \(\varepsilon\) equals all of \(A\) because every element of \(A\) belongs to an equivalence class. The collection \(\varepsilon\) is a particular example of what is called a partition of \(A\) :
...
EXAMPLE 3 Define two points in the plane to be equivalent if they lie at the same distance from the orgin. Reflexivity, symmetry, and transitivity hold trivially. The collection \(\varepsilon\) of equivalence classes consists of all circles centered at the origin, along with the set consisting of the origin alone.
...
EXAMPLE 6 Let \({\mathcal{L}}^{\prime }\) be the collection of all straight lines in the plane. Then \({\mathcal{L}}^{\prime }\) is not a partition of the plane, for distinct elements of \({\mathcal{L}}^{\prime }\) are not necessarily disjoint; two lines may intersect without being equal.
...
EXAMPLE 7. Consider the relation on the real line consisting of all pairs \(\left( {x,y}\right)\) of real numbers such that \(x < y\) . It is an order relation, called the "usual order relation," on the real line. A less familiar order relation on the real line is the following: Define \({xCy}\) if \({x}^{2} < {y}^{2}\) , or if \({x}^{2} = {y}^{2}\) and \(x < y\) . You can check that this is an order relation.
...
As the tilde, \(\sim\) , is the generic symbol for an equivalence relation, the "less than" symbol, <, is commonly used to denote an order relation. Stated in this not ation, the properties of an order relation become
...
Definition. If \(X\) is a set and \(<\) is an order relation on \(X\) , and if \(a < b\) , we use the notation \(\left( {a,b}\right)\) to denote the set
...
Definition. Suppose that \(A\) and \(B\) are two sets with order relations \({C}_{A}\) and \({C}_{B}\) respectively. We say that \(A\) and \(B\) have the same order type if there is a bijective correspondence between them that preserves order; that is, if there exists a bijective function \(f : A \rightarrow  B\) such that

\[
{a}_{1}{ < }_{A}{a}_{2} \Rightarrow  f\left( {a}_{1}\right) { < }_{B}f\left( {a}_{2}\right) .
\]

EXAMPLE 9. The interval \(\left( {-1,1}\right)\) of real numbers has the same order type as the set \(\mathbb{R}\) of real numbers itself, for the function \(f \cdot  \left( {-1,1}\right)  \rightarrow  \mathbb{R}\) given by
...
EXAMPLE 10. The subset \(A = \{ 0\}  \cup  \left( {1,2}\right)\) of \(\mathbb{R}\) has the same order type as the subset
...
\begin{center}
\includegraphics[max width=0.6\textwidth]{images/bo_d7h36d491nqc73828lu0_23_420_196_673_754_0.jpg}
\end{center}
\hspace*{3em} 
...
EXAMPLE 12 Consider the set \(\left( {0,1}\right)\) of real numbers and the set \({\mathbb{Z}}_{ + }\) of positive integers, both in their usual orders; give \({\mathbb{Z}}_{ + } \times  \lbrack 0,1)\) the dictionary order. This set has the same order type as the set of nonnegative reals; the function
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_24_206_138_1103_618_0.jpg}
\end{center}
\hspace*{3em} 
...
EXAMPLE 13. Consider the set \(A = \left( {-1,1}\right)\) of real numbers in the usual order. Assuming the fact that the real numbers have the least upper bound property, it follows that the set \(A\) has the least upper bound property. For, given any subset of \(A\) having an upper bound in \(A\) , it follows that its least upper bound (in the real numbers) must be in \(A\) . For example, the subset \(\left\{  {-1/{2n} \mid  n \in  {\mathbb{Z}}_{ + }}\right\}\) of \(A\) , though it has no largest element, does have a least upper bound in \(A\) , the number 0 .

On the other hand, the set \(B = \left( {-1,0}\right)  \cup  \left( {0,1}\right)\) does not have the least upper bound property. The subset \(\left\{  {-1/{2n} \mid  n \in  {\mathbb{Z}}_{ + }}\right\}\) of \(B\) is bounded above by any element of \(\left( {0,1}\right)\) , but it has no least upper bound in \(B\) .
...
(b) Show that the equation \(g\left( y\right)  = {2y}/\left\lbrack  {1 + {\left( 1 + 4{y}^{2}\right) }^{1/2}}\right\rbrack\) defines a function \(g : \mathbb{R} \rightarrow  \left( {-1,1}\right)\) that is both a left and a nght inverse for \(f\) .
...
\[
\left\lbrack  {0,1}\right\rbrack   = \{ x \mid  0 \leq  x \leq  1\rbrack
\]
...
(b) Does [0, 1] \(\times  \left\lbrack  {0,1}\right\rbrack\) in the dictionary order have the least upper bound property? What about \(\left\lbrack  {0,1}\right\rbrack   \times  \lbrack 0,1)\) ? What about \(\lbrack 0,1) \times  \left\lbrack  {0,1}\right\rbrack\) ?
...
There exists a unique element of \(\mathbb{R}\) called one, different from 0 and denoted by1, such that \(x \cdot  1 = x\) for all \(x \in  \mathbb{R}\) .
...
\section*{A Mixed Algebraic and Order Property}
...
We define a number \(x\) to be positive if \(x > 0\) , and to be negative if \(x < 0\) . We denote the positive reals by \({\mathbb{R}}_{ + }\) and the nonnegative reals (for reasons to be explained later) by \({\overline{\mathbb{R}}}_{ + }\) . Properties (l)-(6) are familiar properties in modern algebra. Any set with two binary operations satisfying (1)-(5) is called by algebraists a field; if the field has an order relation satisfying (6), it is called an ordered field.
...
Now there is nothing in this list as it stands to tell us what an integer is. We now define the integers, using only properties ( l )-(6).

Definition. A subset \(A\) of the real numbers is said to be inductive if it contains the number 1, and if for every \(x\) in \(A\) , the number \(x + 1\) is also in \(A\) . Let \(A\) be the collection of all inductive subsets of \(\mathbb{R}\) . Then the set \({\mathbb{Z}}_{ + }\) of positive integers is defined by the equation

\[
{\mathbb{Z}}_{ + } = \mathop{\bigcap }\limits_{{A \in  A}}A
\]

Note that the set \({\mathbb{R}}_{ + }\) of positive real numbers is inductive, for it contains1and the statement \(x > 0\) implies the statement \(x + 1 > 0\) . Therefore, \({\mathbb{Z}}_{ + } \subset  {\mathbb{R}}_{ + }\) , so the elements of \({\mathbb{Z}}_{ + }\) are indeed positive, as the choice of terminology suggests. Indeed, one sees readily that I is the smallest element of \({\mathbb{Z}}_{ + }\) , because the set of all real numbers \(x\) for which \(x \geq  1\) is inductive.

The basic properties of \({\mathbb{Z}}_{ + }\) , which follow readily from the definition, are the following:

(l) \({\mathbb{Z}}_{ + }\) is inductive.

(2) (Pnnciple of induction). If \(A\) is an inductive set of positive integers, then \(A = \; {\mathbb{Z}}_{ + }\) .
...
Now we prove two properties of the positive integers that may not be quite so famuliar, but are quite useful. They may be thought of as alternative versions of the induction principle.
...
for \(n \in  {\mathbb{Z}}_{ + }\) . (See \(\text{ § }7\) for a discussion of the process of inductive definition.) Show that for \(n,m \in  {\mathbb{Z}}_{ + }\) and \(a,b \in  \mathbb{R}\) ,
...
EXAMPLE 1. We now have two definitions for the symbol \(A \times  B\) . One definition is, of course, the one given earlier, under which \(A \times  B\) denotes the set of all ordered pairs \(\left( {a,b}\right)\) such that \(a \in  A\) and \(b \in  B\) . The second definition, just given, defines \(A \times  B\) as the set of all functions \(\mathbf{x} : \{ 1,2\}  \rightarrow  A \cup  B\) such that \(\mathbf{x}\left( 1\right)  \in  A\) and \(\mathbf{x}\left( 2\right)  \in  B\) . There is an obvious bijective correspondence between these two sets, under which the ordered pair \(\left( {a,b}\right)\) corresponds to the function \(\mathbf{x}\) defined by \(\mathbf{x}\left( 1\right)  = a\) and \(\mathbf{x}\left( 2\right)  = b\) . Since we commonly denote this function \(x\) in "tuple notation" by the symbol \(\left( {a,b}\right)\) , the notation itself suggests the correspondence. Thus for the cartesian product of two sets, the general definition of cartesian product reduces essentially to the earlier one.

EXAMPLE 2. How does the cartesian product \(A \times  B \times  C\) differ from the cartesian products \(A \times  \left( {B \times  C}\right)\) and \(\left( {A \times  B}\right)  \times  {C}^{2}\) Very little. There are obvious bijective correspondences between these sets, indicated as follows
...
we also call such a function a sequence, or an infinite sequence, of elements of \(X\) . If \(x\) is an \(\omega\) -tuple, we often denote the value of \(x\) at \(i\) by \({x}_{i}\) rather than \(x\left( i\right)\) , and call it the ith coordinate of \(\mathbf{x}\) . We denote \(\mathbf{x}\) itself by the symbol
...
Nothing in these definitions requires the sets \({A}_{t}\) to be different from one another. Indeed, they may all equal the same set \(X\) In that case, the cartesian product \({A}_{1} \times \; \cdots  \times  {A}_{m}\) is just the set of all \(m\) -tuples of elements of \(X\) , which we denote by \({X}^{m}\) . Similarly, the product \({A}_{1} \times  {A}_{2} \times  \cdots\) is just the set of all \(\omega\) -tuples of elements of \(X\) , which we denote by \({X}^{\omega }\) .
...
(b) Given the indexed family \(\left\{  {{A}_{1},{A}_{2},\ldots }\right\}\) , let \({B}_{i} = {A}_{{2i} - 1} \times  {A}_{2i}\) for each positive integer \(i\) . Show there is bijective correspondence of \({A}_{1} \times  {A}_{2} \times  \cdots\) with \({B}_{1} \times  {B}_{2} \times  \cdots\)

3. Let \(A = {A}_{1} \times  {A}_{2} \times  \cdots\) and \(B = {B}_{1} \times  {B}_{2} \times  \cdots\) .

(a) Show that if \({B}_{t} \subset  {A}_{t}\) for all \(i\) , then \(B \subset  A\) . (Structly speaking, if we are given a function mapping the index set \({\mathbb{Z}}_{ + }\) into the union of the sets \({B}_{i}\) , we must change its range before it can be considered as a function mapping \({\mathbb{Z}}_{ + }\) into the union of the sets \({A}_{i}\) . We shall ignore this technicality when dealing with cartesian products).
...
(d) What is the relation between the set \(A \cup  B\) and the cartesian product of the sets \({A}_{i} \cup  {B}_{i}\) ? What is the relation between the set \(A \cap  B\) and the cartesian product of the sets \({A}_{i} \cap  {B}_{t}\) ?
...
(a) \(\left\{  {\mathbf{x} \mid  {x}_{i}}\right.\) is an integer for all \(\left. i\right\}\) .

(b) \(\left\{  {\mathbf{x} \mid  {x}_{i} \geq  i}\right.\) for all \(\left. i\right\}\) .

(c) \(\left\{  {\mathbf{x} \mid  {x}_{i}}\right.\) is an integer for all \(i \geq  {100}\}\) .

(d) \(\left\{  {\mathbf{x} \mid  {x}_{2} = {x}_{3}}\right\}\) .
...
for some positive integer \(n\) . In the former case, we say that \(A\) has cardinality0; in the latter case, we say that \(A\) has cardinality \(n\) .
...
Even empirical demonstration would be difficult for such a large value of \(n\) . One might, for instance, construct an experiment by taking a freight car full of marbles and hinng 10 different people to count them independently. If one thinks of the physical problems involved, it seems likely that the counters would not all arrive at the same answer. Of course, the conclusion one could draw is that at least one person made a mistake. But that would mean assuming the correctness of the result one was trying to demonstrate empirically An alternative explanation could be that there do exist bijective correspondences between the given set of marbles and two different sections of the positive integers
...
Lemma 6.1. Let \(n\) be a positive integer. Let \(A\) be a set; let \({a}_{0}\) be an element of \(A\) . Then there exists a bijective correspondence \(f\) of the set \(A\) with the set \(\{ 1,\ldots ,n + 1\}\) if and only if there exists a bijective correspondence \(g\) of the set \(A - \left\{  {a}_{0}\right\}\) with the set \(\{ 1,\ldots ,n\}\) .
...
\begin{center}
\includegraphics[max width=0.9\textwidth]{images/bo_d7h36d491nqc73828lu0_38_237_1155_1053_337_0.jpg}
\end{center}
\hspace*{3em} 
...
(2) There is a surjective function from a section of the posttive integers onto \(B\) .

(3) There is an injective function from \(B\) into a section of the positive integers.

Proof. \(\;\left( 1\right)  \Rightarrow  \left( 2\right)\) . Since \(B\) is nonempty, there is, for some \(n\) , a bijective function \(f : \{ 1,\ldots ,n\}  \rightarrow  B\) .
...
EXAMPLE 2. The product \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) is countably infinite. If we represent the elements of the product \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) by the integer points in the first quadrant, then the left-hand portion of Figure 7.1 suggests how to "count" the points, that is, how to put them in bijective correspondence with the positive integers. A picture is not a proof, of course, but this picture suggests a proof. First, we define a bijection \(f : {\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + } \rightarrow  A\) , where \(A\) is the subset of \({\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + }\) consisting of pairs \(\left( {x,y}\right)\) for which \(y \leq  x\) , by the equation
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_42_179_916_1153_562_0.jpg}
\end{center}
\hspace*{3em} 
...
Proof. \(\;\left( 1\right)  \Rightarrow  \left( 2\right)\) . Suppose that \(B\) is countable. If \(B\) is countably infinite, there is a bijection \(f : {\mathbb{Z}}_{ + } \rightarrow  B\) by definition, and we are through. If \(B\) is finite, there is a bijection \(h : \{ 1,\ldots ,n\}  \rightarrow  B\) for some \(n \geq  1\) . (Recall that \(B \neq  \varnothing\) .) We can extend \(h\) to a surjection \(f : {\mathbb{Z}}_{ + } \rightarrow  B\) by defining
...
Proof. We define a bijection \(h : {\mathbb{Z}}_{ + } \rightarrow  C\) . We proceed by induction. Define \(h\left( 1\right)\) to be the smallest element of \(C\) ; it exists because every nonempty subset \(C\) of \({\mathbb{Z}}_{ + }\) has a smallest element. Then assuming that \(h\left( 1\right) ,\ldots ,h\left( {n - 1}\right)\) are defined, define
...
(*)
\[
h\left( 1\right)  = \text{ smallest element of }C,
\]

\[
h\left( i\right)  = \text{ smallest element of }\left\lbrack  {C - h\left( {\{ 1,\ldots ,i - 1\} }\right) }\right\rbrack  \text{ for all }i > 1.
\]
...
for example, is self-contradictory; although \(h\left( i\right)\) necessarily is an element of the set \(h\left( {\{ 1,\ldots ,i + 1\} }\right)\) , this formula says that it does not belong to the set. Another example is the classic paradox:
...
EXAMPLE 3. The set \({\mathbb{Q}}_{ + }\) of positive rational numbers is countably infinite. For we can define a surjection \(g \cdot  {\mathbb{Z}}_{ + } \times  {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{Q}}_{ + }\) by the equation
...
(If we write the numbers \({x}_{{n}_{l}}\) in a rectangular array, the particular elements \({x}_{nn}\) appear as the diagonal entries in this array; we choose \(y\) so that its \(n\) th coordinate differs from the diagonal entry \({x}_{nn}\) .)
...
Proof. In general, if \(B\) is a nonempty set, the existence of an injective map \(f : B \rightarrow \; C\) implies the existence of a surjective map \(g.C \rightarrow  B\) ; one defines \(g\left( c\right)  = {f}^{-1}\left( c\right)\) for each \(c\) in the image set of \(f\) , and defines \(g\) arbitrarily on the rest of \(C\) .
...
with rational coefficients \({a}_{i}\) . Assuming that each polynomial equation has only finitely many roots, show that the set of algebraic numbers is countable.
...
(c) The set \(C = \mathop{\bigcup }\limits_{{n \in  {\mathbb{Z}}_{ + }}}{B}_{n}\) .

(d) The set \(D\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  {\mathbb{Z}}_{ + }\) .

(e) The set \(E\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) .

(f) The set \(F\) of all functions \(f : {\mathbb{Z}}_{ + } \rightarrow  \{ 0,1\}\) that are "eventually zero." [We say that \(f\) is eventually zero if there is a positive integer \(N\) such that \(f\left( n\right)  = 0\) for all \(n \geq  N\rbrack\)
...
(b) Show that the formula (*) of part (a) does not determine \(h\) uniquely. [Hint: If \(h\) is a positive function satisfying (*), let \(f\left( i\right)  = h\left( i\right)\) for \(i \neq  3\) , and let \(f\left( 3\right)  =  - h\left( 3\right)\) .]
...
\[
h\left( \mathrm{i}\right)  = \text{ smallest element of }\left\lbrack  {C - h\left( {\{ 1,\ldots ,i - 1\} }\right) }\right\rbrack  \;\text{ for }i > 1.
\]
...
\[
f\left( i\right)  = \text{ smallest element of }\left\lbrack  {C - f\left( {\{ 1,\ldots ,i - 1\} }\right) }\right\rbrack  ,
\]

\[
g\left( i\right)  = \text{ smallest element of }\left\lbrack  {C - g\left( {\{ 1,\ldots ,i - 1\} }\right) }\right\rbrack  .
\]
...
Proof. By Lemma 8.1, there exists for each \(n\) a function that maps \(\{ 1,\ldots ,n\}\) into \(C\) and satisfies \(\left( *\right)\) for all \(i\) in its domain. Given \(n\) , Lemma 8.2 shows that this function is unique; two such functions having the same domain must be equal. Let \({f}_{n}\) : \(\{ 1,\ldots ,n\}  \rightarrow  C\) denote this unique function.
...
\[
h\left( i\right)  = \rho \left( {h\mid \{ 1,\ldots ,i - 1\} }\right) \;\text{ for }i > 1.
\]

The formula (*) is called a recursion formula for \(h\) . It specifies \(h\left( 1\right)\) , and it expresses the value of \(h\) at \(i > 1\) in terms of the values of \(h\) for positive integers less than \(i\) .

EXAMPLE I. Let us show that Theorem 8.3 is a special case of this theorem. Given the infinite subset \(C\) of \({\mathbb{Z}}_{ + }\) , let \({a}_{0}\) be the smallest element of \(C\) , and define \(\rho\) by the equation

\[
\rho \left( f\right)  = \text{ smallest element of }\left\lbrack  {C - \text{ (image set of }f)}\right\rbrack  \text{ . }
\]

Because \(C\) is infinite and \(f\) is a function mapping a finite set into \(C\) , the image set of \(f\) is not all of \(C\) , therefore, \(\rho\) is well defined. By Theorem 8.4 there exists a function \(h : {\mathbb{Z}}_{ + } \rightarrow \; C\) such that \(h\left( 1\right)  = {a}_{0}\) , and for \(i > 1\) ,

\[
h\left( i\right)  = \rho \left( {h\mid \{ 1,\ldots ,i - 1\} }\right)
\]
...
EXAMPLE 2. Given \(a \in  \mathbb{R}\) , we "defined" \({a}^{n}\) , in the exercises of \(\text{ § }4\) , by the recursion formula
...
Let \(A\) be the set of real numbers; choose \(\rho\) so that Theorem 8.4 applies to define this sum ngorously. We sometimes denote the sum \(\mathop{\sum }\limits_{{k = 1}}^{n}{b}_{k}\) by the symbol \({b}_{1} + {b}_{2} + \cdots  + {b}_{n}.\)
...
Use Theorem 8.4 to define this product rigorously. We sometimes denote the product \(\mathop{\prod }\limits_{{k = 1}}^{n}{b}_{k}\) by the symbol \({b}_{1}{b}_{2}\cdots {b}_{n}\) .
...
\begin{center}
\includegraphics[max width=0.9\textwidth]{images/bo_d7h36d491nqc73828lu0_54_240_1465_1036_275_0.jpg}
\end{center}
\hspace*{3em} 

Figure 9.1

The implication \(\left( 2\right)  \Rightarrow  \left( 3\right)\) is just the contrapositive of Corollary 6.3, so it has already been proved. To prove that \(\left( 3\right)  \Rightarrow  \left( 1\right)\) , we assume that \(A\) is infinite and construct "by induction" an injective function \(f : {\mathbb{Z}}_{ + } \rightarrow  A\) .
...
\[
f\left( i\right)  = \text{ an arbitrary element of }\left\lbrack  {A - f\left( {\{ 1,\ldots ,i - 1\} }\right) }\right\rbrack  \;\text{ for }i > 1.
\]
...
Because \(B\) contains at least one element \(x\) , the set \({B}^{\prime }\) contains at least the element \(\left( {B,x}\right)\) , so it is nonempty.
...
4. There was a theorem in \(\text{ § }7\) whose proof involved an infinite number of arbitrary choices. Which one was it? Rewrite the proof so as to make explicit the use of the choice axiom. (Several of the earlier exercises have used the choice axiom also.)
...
EXAMPLE I. Consider the set \(\{ 1,2\}  \times  {\mathbb{Z}}_{ + }\) in the dictionary ordernng. Schematically, it can be represented as one infinite sequence followed by another infinite sequence:
...
\begin{center}
\includegraphics[max width=0.8\textwidth]{images/bo_d7h36d491nqc73828lu0_60_303_1103_906_629_0.jpg}
\end{center}
\hspace*{3em} 
...
Thus, the dictionary order at least does not give a well-ordernng of the set \({\left( {\mathbb{Z}}_{ + }\right) }^{\omega }\) . Is there some other order relation on this set that is a well-ordernng? No one has ever constructed a specific well-ordering of \({\left( {\mathbb{Z}}_{ + }\right) }^{\omega }\) . Nevertheless, there is a famous theorem that says such a well-ordering exists:
...
Definition. Let \(X\) be a well-ordered set Given \(\alpha  \in  X\) , let \({S}_{\alpha }\) denote the set
...
Theorem (The principle of transfinite induction). If \(J\) is a well-ordered set and \({J}_{0}\) is an inductive subset of \(J\) , then \({J}_{0} = J\) .

8. (a) Let \({A}_{1}\) and \({A}_{2}\) be disjoint sets, well-ordered by \({ < }_{1}\) and \({ < }_{2}\) , respectively. Define an order relation on \({A}_{1} \cup  {A}_{2}\) by letting \(a < b\) either if \(a,b \in  {A}_{1}\) and \(a{ < }_{1}b\) , or if \(a,b \in  {A}_{2}\) and \(a{ < }_{2}b\) , or if \(a \in  {A}_{1}\) and \(b \in  {A}_{2}\) . Show that this is a well-ordering
...
(d) Show by transfinite induction that for every \(\beta  \in  J\) , there exists a function \({h}_{\beta } : {S}_{\beta } \rightarrow  C\) satisfying (*). [Hint: If \(\beta\) has an immediate predecessor \(\alpha\) , then \({S}_{\beta } = {S}_{\alpha } \cup  \{ \alpha \}\) . If not, \({S}_{\beta }\) is the union of all \({S}_{\alpha }\) with \(\alpha  < \beta\) .]
...
\({}^{ \dagger  }\) This section will be assumed in Chapters 5 and 14
...
EXAMPLE 1 If \(\mathcal{A}\) is any collection of sets, the relation "is a proper subset of" is a strict partial order on \(\mathcal{A}\) . Suppose that \(\mathcal{A}\) is the collection of all circular regions (interiors of circles) in the plane. One maximal simply ordered subcollection of \(\mathcal{A}\) consists of all circular regions with centers at the origin Another maximal simply ordered subcollection consists of all circular regions bounded by circles tangent from the right to the \(y\) -axis at the ongin See Figure 11 1.

\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7h36d491nqc73828lu0_66_163_887_1190_499_0.jpg}
\end{center}
\hspace*{3em} 

Figure 11.1

EXAMPLE 2. If \(\left( {{x}_{0},{y}_{0}}\right)\) and \(\left( {{x}_{1},{y}_{1}}\right)\) are two points of the plane \({\mathbb{R}}^{2}\) , define

\[
\left( {{x}_{0},{y}_{0}}\right)  \prec  \left( {{x}_{1},{y}_{1}}\right)
\]

If \({y}_{0} = {y}_{1}\) and \({x}_{0} < {x}_{1}\) This is a partial ordering of \({\mathbb{R}}^{2}\) under which two points are comparable only if they lie on the same horizontal line The maximal simply ordered sets are the honzontal lines in \({\mathbb{R}}^{2}\)
...
Proof. Let \(X\) be a set, let \(c\) be a fixed choice function for the nonempty subsets of \(X\) . If \(T\) is a subset of \(X\) and \(<\) is a relation on \(T\) , we say that \(\left( {T, < }\right)\) is a tower in \(X\) if \(<\) is a well-ordering of \(T\) and if for each \(x \in  T\) ,
...
8. Using Exercises I-4, construct an uncountable well-ordered set, as follows. Let A be the collection of all pairs \(\left( {A, < }\right)\) , where \(A\) is a subset of \({\mathbb{Z}}_{ + }\) and \(<\) is a well-ordering of \(A\) . (We allow \(A\) to be empty.) Define \(\left( {A, < }\right)  \sim  \left( {{A}^{\prime },{ < }^{\prime }}\right)\) if \(\left( {A, < }\right)\) and \(\left( {{A}^{\prime },{ < }^{\prime }}\right)\) have the same order type. It is trivial to show this is an equivalence relation. Let \(\left\lbrack  \left( {A, < }\right) \right\rbrack\) denote the equivalence class of \(\left( {A, < }\right)\) ; let \(E\) denote the collection of these equivalence classes. Define
